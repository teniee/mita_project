...
class TestDaysUntilExhausted:
    ...
    def test_days_until_exhausted_zero_pace_returns_none(self):
        ...
        plans = [
            _plan("entertainment", _d(2026, 3, d), "50", "0") for d in range(1, 32)
        ]
        result = compute_forecast(
            daily_plans=plans,
            goals=[],
...
class TestGoalForecasts:
    ...
    def test_goal_on_track_with_sufficient_contribution(self):
        ...
        result = compute_forecast(
            daily_plans=[],
            goals=[_goal("1200", "300", contribution="200", target_date=far_future)],
            year=2026,
            month=3,
            today=_d(2026, 3, 15),
        )
    ...
    def test_goal_not_on_track_shortfall_calculated(self):
        ...
        result = compute_forecast(
            daily_plans=[],
            goals=[_goal("1200", "0", contribution="50", target_date=far_future)],
            year=2026,
            month=3,
            today=_d(2026, 3, 15),
        )
    ...
    def test_goal_no_contribution_set_is_not_on_track(self):
        ...
        result = compute_forecast(
            daily_plans=[],
            goals=[_goal("1000", "100", contribution=None, target_date=_d(2026, 9, 1))],
            year=2026,
            month=3,
            today=_d(2026, 3, 15),
        )
...
class TestDecimalPrecision:
    ...
    def test_safe_daily_limit_precision_two_decimals(self):
        ...
        assert result.safe_daily_limit == result.safe_daily_limit.quantize(
            Decimal("0.01")
        )
...
class TestFullScenario:
    ...
    def test_realistic_mid_month_danger(self):
        ...
        result = compute_forecast(
            daily_plans=plans,
            goals=[_goal("500", "50", contribution="50", target_date=_d(2026, 6, 1))],
            year=2026,
            month=3,
            today=_d(2026, 3, 10),
        )
    ...
    def test_on_track_scenario_all_green(self):
        ...
        plans = (
            [_plan("groceries", _d(2026, 3, d), "70", "70") for d in range(1, 16)]
            + [_plan("dining_out", _d(2026, 3, d), "30", "30") for d in range(1, 16)]
            + [_plan("groceries", _d(2026, 3, d), "70", "0") for d in range(16, 32)]
            + [_plan("dining_out", _d(2026, 3, d), "30", "0") for d in range(16, 32)]
        )

        result = compute_forecast(
            daily_plans=plans,
            goals=[_goal("600", "200", contribution="200", target_date=_d(2026, 5, 1))],
            year=2026,
            month=3,
            today=_d(2026, 3, 15),
        )
