...
class TestCalendarGeneration:
    ...
    def test_calendar_returns_list(self, sample_user_data_with_breakdown):
        ...
        calendar_data = build_monthly_budget(
            sample_user_data_with_breakdown, year, month
        )
    ...
    def test_calendar_spread_pattern(self, sample_user_data_with_breakdown):
        ...
        coffee_days = [
            d for d in calendar_data if "coffee" in d.get("planned_budget", {})
        ]
    ...
    def test_calendar_clustered_pattern(self, sample_user_data_with_breakdown):
        ...
        dining_days = [
            d for d in calendar_data if "dining out" in d.get("planned_budget", {})
        ]
...
class TestBudgetRedistribution:
    ...
    def test_redistribution_reduces_overspending(self):
        ...
        before_over = sum(
            max(Decimal(0), d["total"] - d["limit"]) for d in test_calendar.values()
        )
        ...
        after_over = sum(
            max(Decimal(0), d["total"] - d["limit"]) for d in updated_calendar.values()
        )
