...
from app.api.auth.schemas import GoogleAuthIn, LoginIn, RegisterIn, TokenOut
from app.api.auth.services import authenticate_google
from app.api.dependencies import get_current_user
from app.core.async_session import get_async_db
from app.core.error_decorators import ErrorHandlingMixin, handle_auth_errors
...
async def refresh_token_standardized(
    ...
    try:
        ...
        await log_security_event_async(
            ...
            details={
                "error": str(e),
                "token_prefix": refresh_token[:20] if refresh_token else "",
            },
        )
...
async def emergency_register_legacy(request: Request):
    ...
    try:
        ...
        with Session() as session:
            ...
            logger.info(
                f"🚨 EMERGENCY REGISTRATION SUCCESS: {email[:3]}*** in {total_time:.2f}s"
            )
...
async def register_fast_legacy(request: Request):
    ...
    try:
        ...
        with Session() as session:
            ...
            total_time = time.time() - start_time
            logger.info(
                f"Fast registration success: {email[:3]}*** in {total_time:.2f}s"
            )
...
async def register_full(
    payload: RegisterIn, request: Request, db: AsyncSession = Depends(get_async_db)
):
    ...
    await log_security_event_async(
        ...
        {
            "email_hash": (
                payload.email[:3] + "***@" + payload.email.split("@")[1]
                if "@" in payload.email
                else "invalid"
            ),
    ...
    try:
        ...
        if payload:
            ...
            if not await blacklist_token(token, reason="rotation"):
                logger.error(
                    f"Failed to blacklist old refresh token for user {payload.get('sub')}"
                )
        ...
        try:
            ...
            await log_security_event_async(
                "legacy_token_refresh",
                {"user_id": user_id, "token_type": "legacy", "success": True},
                request=request,
                user_id=user_id,
            )
        ...
        await log_security_event_async(
            ...
            {"user_id": user_id, "error": str(e)[:200], "success": False},
            request=request,
            user_id=user_id,
        )
        raise HTTPException(
...
async def revoke_token(
    ...
    try:
        ...
        await log_security_event_async(
            ...
            {"user_id": str(current_user.id), "error": str(e)[:200], "success": False},
            request=request,
            user_id=str(current_user.id),
        )
        raise HTTPException(
...
async def validate_current_token(
    ...
    return success_response(
        {"user_id": str(current_user.id), "token_validation": validation}
    )
...
@router.post(
    "/password-reset/request", dependencies=[Depends(check_password_reset_rate_limit)]
)
async def request_password_reset(
    ...
    await log_security_event_async(
        ...
        {
            "email_hash": (
                validated_email[:3] + "***@" + validated_email.split("@")[1]
                if "@" in validated_email
                else "invalid"
            ),
            "user_agent": request.headers.get("User-Agent", "")[:100],
            "client_ip": request.client.host if request.client else "unknown",
    ...
    try:
        ...
        if user:
            ...
            reset_token, token_hash = PasswordResetTokenManager.generate_reset_token(
                str(user.id)
            )
            ...
            user.password_reset_attempts = (
                getattr(user, "password_reset_attempts", 0) + 1
            )
            ...
            await log_security_event_async(
                ...
                {
                    "user_id": str(user.id),
                    "email_hash": validated_email[:3]
                    + "***@"
                    + validated_email.split("@")[1],
                    "job_id": job_id,
                    "success": True,
                },
                request=request,
            ...
            logger.info(
                f"Password reset email queued for {validated_email[:3]}*** (job: {job_id})"
            )
        else:
            ...
            logger.info(
                f"Password reset requested for non-existent email: {validated_email[:3]}***"
            )
...
async def confirm_password_reset(
    ...
    try:
        ...
        if not user:
            ...
            raise AuthenticationError(
                "Invalid password reset request", ErrorCode.AUTH_INVALID_CREDENTIALS
            )
        ...
        if not hasattr(user, "password_reset_token") or not user.password_reset_token:
            ...
            raise AuthenticationError(
                "Invalid or expired password reset token", ErrorCode.AUTH_TOKEN_INVALID
            )
        ...
        if hasattr(user, "password_reset_expires") and user.password_reset_expires:
            if datetime.utcnow() > user.password_reset_expires:
                ...
                raise AuthenticationError(
                    "Password reset token has expired", ErrorCode.AUTH_TOKEN_EXPIRED
                )

        # Verify token
        if not PasswordResetTokenManager.verify_reset_token(
            ...
            raise AuthenticationError(
                "Invalid password reset token", ErrorCode.AUTH_TOKEN_INVALID
            )
        ...
        from app.services.email_service import EmailPriority, EmailType
        ...
        raise AuthenticationError("Password reset failed", ErrorCode.AUTH_TOKEN_INVALID)
...
async def test_registration_isolated(request: Request):
    ...
    try:
        ...
        logger.info(
            f"🔍 TEST-REG [{step_results['request_id']}] Starting minimal registration test"
        )
        ...
        try:
            ...
            logger.info(
                f"🔍 TEST-REG [{step_results['request_id']}] Step 1 PASSED: Request parsing"
            )
            ...
            logger.error(
                f"🔍 TEST-REG [{step_results['request_id']}] Step 1 FAILED: {e}"
            )
            return {
                "success": False,
                "test_results": step_results,
                "failed_at": "request_parsing",
            }
        ...
        try:
            ...
            logger.info(
                f"🔍 TEST-REG [{step_results['request_id']}] Step 2 PASSED: Basic validation"
            )
            ...
            logger.error(
                f"🔍 TEST-REG [{step_results['request_id']}] Step 2 FAILED: {e}"
            )
            return {
                "success": False,
                "test_results": step_results,
                "failed_at": "basic_validation",
            }
        ...
        try:
            ...
            logger.info(
                f"🔍 TEST-REG [{step_results['request_id']}] Step 3 PASSED: Database connection"
            )
            ...
            logger.error(
                f"🔍 TEST-REG [{step_results['request_id']}] Step 3 FAILED: {e}"
            )
            return {
                "success": False,
                "test_results": step_results,
                "failed_at": "database_connection",
            }
        ...
        try:
            ...
            logger.info(
                f"🔍 TEST-REG [{step_results['request_id']}] Step 4 PASSED: Password hashing"
            )
            ...
            logger.error(
                f"🔍 TEST-REG [{step_results['request_id']}] Step 4 FAILED: {e}"
            )
            return {
                "success": False,
                "test_results": step_results,
                "failed_at": "password_hashing",
            }
        ...
        try:
            with Session() as session:
                ...
                logger.info(
                    f"🔍 TEST-REG [{step_results['request_id']}] Step 5 PASSED: Database operations"
                )
            ...
            logger.error(
                f"🔍 TEST-REG [{step_results['request_id']}] Step 5 FAILED: {e}"
            )
            return {
                "success": False,
                "test_results": step_results,
                "failed_at": "database_operations",
            }
        ...
        try:
            ...
            logger.info(
                f"🔍 TEST-REG [{step_results['request_id']}] Step 6 PASSED: Token generation"
            )
            ...
            logger.error(
                f"🔍 TEST-REG [{step_results['request_id']}] Step 6 FAILED: {e}"
            )
            return {
                "success": False,
                "test_results": step_results,
                "failed_at": "token_generation",
            }
        ...
        try:
            ...
            logger.info(
                f"🔍 TEST-REG [{step_results['request_id']}] Step 7 PASSED: Response generation"
            )
            logger.info(
                f"🔍 TEST-REG [{step_results['request_id']}] ALL STEPS COMPLETED SUCCESSFULLY"
            )
            ...
            logger.error(
                f"🔍 TEST-REG [{step_results['request_id']}] Step 7 FAILED: {e}"
            )
            return {
                "success": False,
                "test_results": step_results,
                "failed_at": "response_generation",
            }

    except Exception as e:
        logger.error(
            f"🔍 TEST-REG [{step_results['request_id']}] UNEXPECTED ERROR: {e}"
        )
        return {
            "success": False,
            "test_results": step_results,
...
async def emergency_diagnostics(db: AsyncSession = Depends(get_async_db)):
    ...
    try:
        ...
        try:
            ...
            test_tokens = create_token_pair(
                {"sub": "test_user"}, user_role="basic_user"
            )
...
async def test_password_hashing_isolated(request: Request):
    ...
    try:
        ...
        logger.info(
            f"🔐 HASH-TEST [{test_id}] Completed: {passed_tests}/{total_tests} tests passed"
        )
...
async def test_database_operations_isolated(request: Request):
    ...
    try:
        ...
        total_tests = sum(
            1 for test in results["tests"].values() if test["status"] != "skipped"
        )
        ...
        logger.info(
            f"🗄️ DB-TEST [{test_id}] Completed: {passed_tests}/{total_tests} tests passed"
        )
...
async def test_response_generation_isolated(request: Request):
    ...
    try:
        ...
        try:
            ...
            logger.error(
                f"📤 RESPONSE-TEST [{test_id}] FastAPI compatibility FAILED: {e}"
            )
        ...
        logger.info(
            f"📤 RESPONSE-TEST [{test_id}] Completed: {passed_tests}/{total_tests} tests passed"
        )
...
async def get_security_status(
    ...
    from app.core.password_security import (
        get_password_performance_stats,
    ...
    return success_response(
        {
            ...
            "password_security": {
                ...
                "security_compliant": bcrypt_validation["valid"]
                and password_stats["bcrypt_rounds"] >= 12,
            },
        ...
        return success_response(
            {
                ...
                "security_compliance": {
                    "meets_industry_standard": config_validation["configuration"][
                        "rounds"
                    ]
                    >= 12,
...
async def admin_revoke_user_tokens(
    ...
    try:
        revoked_count = await revoke_user_tokens(
            user_id=user_id, reason=reason, revoked_by=str(current_user.id)
        )

        logger.info(
            f"Admin {current_user.id} revoked {revoked_count} tokens for user {user_id}"
        )
...
async def admin_revoke_token_by_jti(
    ...
    try:
        ...
        success = await blacklist_service.revoke_token_by_jti(
            jti=jti, reason=blacklist_reason, revoked_by=str(current_user.id)
        )
        ...
        return success_response(
            {
                "blacklist_metrics": {
                    ...
                    "last_cleanup": (
                        metrics.last_cleanup.isoformat()
...
async def verify_reset_token(
    ...
    try:
        ...
        return success_response(
            {"valid": True, "message": "Reset token is valid", "user_id": user_id}
        )
...
async def google_auth(
    request: Request, google_data: GoogleAuthIn, db: AsyncSession = Depends(get_db)
):
    """
