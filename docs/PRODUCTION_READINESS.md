...
| Android build vs production | ✅ **built and recorded** (Main CI run [29321271177](https://github.com/teniee/mita_project/actions/runs/29321271177), green): `mobile_app/build/app/outputs/flutter-apk/app-debug.apk`, 167,012,569 bytes, SHA-256 `10c4af352251dc1da62fce7a23871cd01a43e56f217bb5eba2104f762e8e0e7b`, applicationId `mita.finance`, versionCode 1 / versionName 1.0 / pubspec `1.0.0+1`, built with `--dart-define=API_BASE_URL=<railway> --dart-define=ENV=production` (debug logging off). Artifact `mita-debug-apk` (ID 8306243842, 7-day retention). Default `AppConfig.baseUrl` is the production URL; HTTPS-only, no localhost/stale URLs, no bundled secrets | `.github/workflows/main-ci.yml` |
