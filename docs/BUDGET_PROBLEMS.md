...
**Что было:** `UserContext` создавался но не применялся. `except Exception:` глотал ошибки без следа. `user_context_applied: True` возвращалось всегда — ложь. `confidence: 0.8` одинаков для обоих путей.

**Что сделано:**
- Добавлен `logger = logging.getLogger(__name__)` в модуль
- Успешный путь (dynamic): `logger.info("behavioral_allocation.dynamic_context_applied", extra={user_id, income_tier, monthly_income, region, categories_count})`
- Fallback путь: `logger.warning("behavioral_allocation.fallback_to_hardcoded", extra={user_id, income_tier, fallback_reason}, exc_info=True)` — исключение больше не молчит
- Новые поля в ответе API:
  - `allocation_method`: `"dynamic"` или `"hardcoded_fallback"` — для агрегации в Sentry/Grafana
  - `income_tier`: классифицированный тир пользователя (e.g. `"middle"`)
  - `confidence`: `0.85` для dynamic, `0.50` для fallback — отражает реальное качество

**Как измерять % использования:** Фильтр в Sentry/Grafana по полю `allocation_method`. `"dynamic"` vs `"hardcoded_fallback"` — точная метрика без дополнительной инфраструктуры.

**Тесты:** 19 тестов (`tests/test_behavioral_allocator_context.py`) — все проходят ✅
...
| 5 | Behavioral allocator врал | 🟡 Важно | ✅ Исправлено + структурированное логирование |
...
**Тесты:** 305 passing, 8 failing (`test_category_mapping_fix.py` ×4, `test_income_location_budget_allocation.py` ×2, `test_revolutionary_feature_end_to_end.py` ×1, `test_scheduled_expense_service.py::test_impact_includes_pending_expenses` ×1 — не связаны с бюджетными проблемами).
