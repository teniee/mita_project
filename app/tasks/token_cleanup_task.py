...
async def cleanup_token_blacklist():
    ...
    try:
        ...
        if metrics.average_check_time_ms > 50:
            logger.warning(
                f"Blacklist check performance degraded: {metrics.average_check_time_ms:.2f}ms"
            )
            log_security_event(
                "blacklist_performance_warning",
                {
                    "average_check_time_ms": metrics.average_check_time_ms,
                    "threshold_ms": 50,
                },
        ...
        if metrics.redis_errors > 10:
            ...
            log_security_event(
                "blacklist_redis_errors",
                {"redis_errors": metrics.redis_errors, "threshold": 10},
            )
...
async def cleanup_security_audit_logs():
    ...
    try:
        ...
        log_security_event(
            "audit_log_cleanup",
            {"cutoff_date": cutoff_date.isoformat(), "cleanup_type": "automated"},
        )
...
async def run_daily_cleanup():
    ...
    logger.info(
        f"Daily maintenance completed: {success_count}/{len(tasks)} tasks successful"
    )
...
def schedule_blacklist_tasks():
    ...
    return {"hourly_monitoring": "scheduled", "daily_cleanup": "scheduled"}
