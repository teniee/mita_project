...
from app.api.auth_api import router as auth_router_legacy  # legacy auth
...
from app.api.category_goal_api import router as category_goal_router
...
from app.api.transactions.routes import (
    router as transactions_router,  # standard transactions router
)
...
async def log_requests(request: Request, call_next):
    ...
    logging.info(
        (
            ...
            f"{duration*1000:.2f}ms with status {response.status_code}"
        )
    )
    return response


# Include public routers (keep prefixes unique)
app.include_router(
    auth_router_legacy,
    prefix="/api/v0/auth",
    tags=["auth_legacy"],
...
app.include_router(
    ocr_google_router,
    prefix="/api/v0/ocr/google",
    tags=["ocr_legacy"],
)
app.include_router(
    ocr_image_router,
    prefix="/api/v0/ocr/image",
    tags=["ocr_legacy"],
)
app.include_router(
    ocr_router,
...
app.include_router(
    summary_router,
    prefix="/api/v0/calendar/summary",
...
app.include_router(
    calendar_ext_router,
    prefix="/api/v0/calendar/extensions",
    tags=["calendar_legacy"],
)
app.include_router(
...
private_routers_list = [
    ...
    (category_goal_router, "/api/category-goal", ["CategoryGoals"]),
    (email_router, "/api/email", ["Email"]),
    (assistant_router, "/api/assistant", ["Assistant"]),
]
