...
from app.api.health.external_services_routes import (
    router as external_services_health_router,
)
...
from app.core.async_session import close_database, init_database
...
def configure_sentry():
    ...
    sentry_sdk.init(
        ...
        integrations=[
            ...
            RqIntegration(),
            SqlalchemyIntegration(auto_enable=True),
            AsyncPGIntegration(),
            RedisIntegration(),
            HttpxIntegration(),
            sentry_logging,
    ...
    if "request" in event:
        ...
        if "query_string" in event["request"]:
            ...
            if query_string and any(
                key in query_string.lower() for key in sensitive_keys
            ):
...
from app.core.openapi_documentation import get_standardized_openapi_schema
...
API endpoints are rate-limited to ensure fair usage. Rate limit information is provided in response headers.
    """.strip(),
    contact={"name": "MITA Finance API Support", "email": "api-support@mita.finance"},
...
async def detailed_health_check():
    ...
    try:
        db_healthy = await asyncio.wait_for(
            check_database_health(), timeout=5.0
        )  # Aligned with Docker HEALTHCHECK --timeout (5-10s)
    ...
    if not _sentry_initialized and settings.ENVIRONMENT == "production":
        overall_status = "degraded"
    if (
        not config_status["jwt_secret_configured"]
        or not config_status["database_configured"]
    ):
...
app.add_middleware(
    ResponseValidationMiddleware, validate_success_responses=settings.DEBUG
)
...
async def optimized_audit_middleware(request: Request, call_next):
    ...
    try:
        ...
        should_log = (
            ...
            or response.status_code >= 400  # Errors
            or request.url.path.startswith("/api/auth/")  # Authentication endpoints
            or request.url.path.startswith("/api/users/")  # User management
            or request.url.path.startswith("/api/transactions/")  # Financial operations
        )
        ...
        if should_log:
            ...
            asyncio.create_task(
                audit_logger.log_request_response(
                    request=request,
                    response=response,
...
private_routers_list = [
    ...
    (
        ...
        "",
        ["Health"],
    ),  # No /api prefix for health endpoints
]
...
async def standardized_exception_handler(
    request: Request, exc: StandardizedAPIException
):
    ...
    try:
        ...
        async def init_critical_services():
            ...
            try:
                ...
                logging.warning(
                    "⚠️ Database init timed out after 15s - will retry on first request"
                )
            except Exception as e:
        ...
        try:
            ...
            logging.warning(
                "⚠️ Audit system init timed out - will initialize on first use"
            )
        ...
        logging.info(
            f"🎯 Fast startup complete: {ready_services}/{total_services} services ready"
        )
