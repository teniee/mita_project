        ...
        async with get_async_db_context() as db:
            ...
            for index in self.composite_indexes:
                try:
                    ...
                    error_msg = (
                        f"Failed to create composite index {index['name']}: {str(e)}"
                    )
                    results["errors"].append(error_msg)
                    logger.error(error_msg)
            ...
            for index in self.fulltext_indexes:
                try:
                    ...
                    error_msg = (
                        f"Failed to create full-text index {index['name']}: {str(e)}"
                    )
                    results["errors"].append(error_msg)
                    logger.error(error_msg)
    ...
    async def analyze_index_usage(self) -> Dict[str, Any]:
        ...
        async with get_async_db_context() as db:
            ...
            try:
                ...
                return {
                    "index_usage": [
                        dict(row._asdict()) for row in usage_result.fetchall()
                    ],
                    "index_sizes": [
                        dict(row._asdict()) for row in size_result.fetchall()
                    ],
                    "unused_indexes": [
                        dict(row._asdict()) for row in unused_result.fetchall()
                    ],
                }
            except Exception as e:
    ...
    async def get_slow_queries(self, limit: int = 10) -> List[Dict[str, Any]]:
        ...
        async with get_async_db_context() as db:
            ...
            try:
                ...
                logger.warning(
                    f"Could not retrieve slow queries (pg_stat_statements may not be enabled): {str(e)}"
                )
                return []
