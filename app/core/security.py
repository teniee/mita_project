...
from app.core.password_security import get_bcrypt_rounds
from app.core.password_security import hash_password_sync as centralized_hash_password
...
pwd_context = CryptContext(
    schemes=["bcrypt"], deprecated="auto", bcrypt__rounds=get_bcrypt_rounds()
)
...
class SecurityConfig:
    ...
    DEFAULT_RATE_LIMIT = (
        300  # requests per hour for general endpoints (increased from 100)
    )
    API_RATE_LIMIT = (
        2000  # API calls per hour for authenticated users (increased from 1000)
    )
    ANONYMOUS_RATE_LIMIT = (
        120  # More reasonable limit for anonymous users (increased from 50)
    )
...
class SecurityUtils:
    ...
    def validate_password_strength(password: str) -> None:
        ...
        if not any(c.islower() for c in password):
            raise ValidationException(
                "Password must contain lowercase letters", field="password"
            )
        if not any(c.isupper() for c in password):
            raise ValidationException(
                "Password must contain uppercase letters", field="password"
            )
        if not any(c.isdigit() for c in password):
            raise ValidationException("Password must contain numbers", field="password")
        if not any(not c.isalnum() for c in password):
            raise ValidationException(
                "Password must contain special characters", field="password"
            )
    ...
    def scan_for_sql_injection(input_string: str) -> bool:
        ...
        for pattern in SecurityConfig.SQL_INJECTION_PATTERNS:
            if re.search(pattern, input_lower, re.IGNORECASE):
                logger.warning(
                    f"SQL injection attempt detected: {SecurityUtils.hash_sensitive_data(input_string)}"
                )
                return True
    ...
    def scan_for_xss(input_string: str) -> bool:
        ...
        for pattern in SecurityConfig.XSS_PATTERNS:
            if re.search(pattern, input_string, re.IGNORECASE):
                logger.warning(
                    f"XSS attempt detected: {SecurityUtils.hash_sensitive_data(input_string)}"
                )
                return True
...
class AdvancedRateLimiter:
    ...
    def _get_client_identifier(self, request: Request) -> str:
        ...
        for header in ip_headers:
            ...
            if header_value:
                ...
                if (
                    re.match(r"^(?:[0-9]{1,3}\.){3}[0-9]{1,3}$", real_ip)
                    or ":" in real_ip
                ):  # IPv4 or IPv6
                    break
                real_ip = None
        ...
        if self.redis:
            try:
                ...
                if not results or len(results) < 3:
                    raise ValueError(
                        f"Redis pipeline returned insufficient results: {results}"
                    )
                ...
                if oldest_score and len(oldest_score) > 0:
                    try:
                        ...
                        if (
                            isinstance(oldest_score[0], (list, tuple))
                            and len(oldest_score[0]) > 1
                        ):
                ...
                if self.fail_secure_mode:
                    ...
                    logger.error(
                        f"Rate limiting unavailable - failing secure for key: {key[:20]}..."
                    )
                    raise RateLimitException(
                        "Service temporarily unavailable. Please try again later."
                    )
    ...
    def check_rate_limit(
        ...
        if user_key:
            user_count, user_ttl, user_exceeded = self._sliding_window_counter(
                user_key,
                window_seconds,
                limit * 2,  # Higher limit for authenticated users
            )

        # Check for progressive penalties
        penalty_multiplier = self._check_progressive_penalties(
            client_id, identifier_suffix
        )
        ...
        if is_limited:
            ...
            raise RateLimitException(
                f"Rate limit exceeded. Try again in {max(client_ttl, user_ttl)} seconds."
            )
    ...
    def _check_progressive_penalties(self, client_id: str, endpoint: str) -> float:
        ...
        penalty_key = (
            f"penalties:{endpoint}:{SecurityUtils.hash_sensitive_data(client_id)}"
        )
    ...
    def _log_rate_limit_violation(
        self, request: Request, client_id: str, user_id: Optional[str], details: dict
    ) -> None:
    ...
    def check_auth_rate_limit(
        self, request: Request, email: str, endpoint_type: str = "login"
    ) -> None:
        ...
        limits = {
            "login": {
                "limit": 8,
                "window": 900,
            },  # 8 attempts per 15 minutes (increased from 5)
            "register": {
                "limit": 5,
                "window": 3600,
            },  # 5 registrations per hour (increased from 3)
            "password_reset": {
                "limit": 3,
                "window": 1800,
            },  # 3 resets per 30 minutes (increased from 2)
            "token_refresh": {
                "limit": 20,
                "window": 300,
            },  # 20 refreshes per 5 minutes (increased from 10)
        }
        ...
        email_count, email_ttl, email_exceeded = self._sliding_window_counter(
            email_key,
            config["window"],
            config["limit"] // 2,  # Stricter email-based limit
        )
    ...
    def _check_suspicious_auth_patterns(
        self, client_id: str, email_hash: str, endpoint_type: str
    ) -> None:
            ...
            if unique_emails_count > 10:  # More than 10 different emails in 1 hour
                ...
                log_security_event(
                    "suspicious_auth_pattern",
                    {
                        ...
                        "endpoint": endpoint_type,
                        "client_hash": SecurityUtils.hash_sensitive_data(client_id)[
                            :16
                        ],
                ...
                suspicious_key = (
                    f"suspicious:{SecurityUtils.hash_sensitive_data(client_id)}"
                )
    ...
    def get_rate_limit_status(
        self, request: Request, endpoint_type: str, user_id: Optional[str] = None
    ) -> dict:
        ...
        if self.redis:
            try:
                api_count = (
                    self.redis.zcard(api_key) if self.redis.exists(api_key) else 0
                )
                statuses["api_requests"] = {
                    "count": api_count,
                    "limit": SecurityConfig.API_RATE_LIMIT,
                }
            except Exception:
                statuses["api_requests"] = {
                    "count": 0,
        ...
        if endpoint_type.startswith("auth_"):
            ...
            if self.redis:
                try:
                    auth_count = (
                        self.redis.zcard(auth_key) if self.redis.exists(auth_key) else 0
                    )
                    statuses[f"{auth_type}_attempts"] = {
                        "count": auth_count,
                        "limit": SecurityConfig.LOGIN_RATE_LIMIT,
                    }
                except Exception:
                    statuses[f"{auth_type}_attempts"] = {
                        "count": 0,
        ...
        for pattern in suspicious_ua_patterns:
            if re.search(pattern, user_agent, re.IGNORECASE):
                self.log_security_event(
                    ...
                    {
                        "user_agent": SecurityUtils.hash_sensitive_data(user_agent),
                        "pattern": pattern,
                    },
                    request,
                    "high",
            ...
            try:
                ...
                if count > 50:  # More than 50 requests in 10 seconds
                    self.log_security_event(
                        "rapid_requests",
                        {
                            "requests_per_10s": count,
                            "client": SecurityUtils.hash_sensitive_data(client_id),
                        },
                        request,
                        "medium",
                    )
...
class SecureInputValidator:
    ...
    def validate_and_sanitize_input(
        data: Any, max_depth: int = SecurityConfig.MAX_OBJECT_DEPTH
    ) -> Any:
    ...
    def _sanitize_string(value: str) -> str:
        ...
        if len(value) > SecurityConfig.MAX_STRING_LENGTH:
            raise ValidationException(
                f"String too long (max {SecurityConfig.MAX_STRING_LENGTH})"
            )
    ...
    def _sanitize_dict(data: dict, max_depth: int) -> dict:
        ...
        if len(data) > SecurityConfig.MAX_ARRAY_LENGTH:
            raise ValidationException(
                f"Dictionary too large (max {SecurityConfig.MAX_ARRAY_LENGTH} keys)"
            )
        ...
        for key, value in data.items():
            ...
            sanitized_value = SecureInputValidator.validate_and_sanitize_input(
                value, max_depth
            )
    ...
    def _sanitize_list(data: list, max_depth: int) -> list:
        ...
        if len(data) > SecurityConfig.MAX_ARRAY_LENGTH:
            raise ValidationException(
                f"Array too large (max {SecurityConfig.MAX_ARRAY_LENGTH} items)"
            )

        return [
            SecureInputValidator.validate_and_sanitize_input(item, max_depth)
            for item in data
        ]
...
class FileUploadSecurity:
    ...
    def validate_file_upload(filename: str, file_content: bytes) -> None:
        ...
        if file_extension not in SecurityConfig.ALLOWED_FILE_EXTENSIONS:
            raise ValidationException(
                f"File type not allowed. Allowed types: {', '.join(SecurityConfig.ALLOWED_FILE_EXTENSIONS)}"
            )

        # Check file size
        if len(file_content) > SecurityConfig.MAX_FILE_SIZE:
            raise ValidationException(
                f"File too large. Maximum size: {SecurityConfig.MAX_FILE_SIZE // (1024*1024)}MB"
            )
...
def get_security_health_status() -> dict:
    ...
    if redis_client:
        try:
            ...
            health_status["redis_info"] = {
                ...
                "used_memory_human": redis_client.info().get(
    ...
    if redis_client:
        try:
            ...
            suspicious_keys = (
                len(redis_client.keys("suspicious:*")) if redis_client else 0
            )
            blacklist_keys = (
                len(redis_client.keys("blacklist:*")) if redis_client else 0
            )
...
def require_api_rate_limit(
    request: Request, rate_limiter: AdvancedRateLimiter = Depends(get_rate_limiter)
):
    """Dependency to enforce API rate limiting"""
    rate_limiter.check_rate_limit(
        request, SecurityConfig.API_RATE_LIMIT, 3600, "api"
    )  # 1 hour window
...
def require_auth_endpoint_protection(
    request: Request = None,
    ...
    def dependency(request: Request):
        ...
        rate_limiter.check_rate_limit(
            request, SecurityConfig.ANONYMOUS_RATE_LIMIT, 3600, "auth", None
        )
...
def comprehensive_auth_security(
    ...
    def dependency(
        ...
        if rate_limiter.is_client_suspicious(request):
            logger.warning(
                f"Blocking request from suspicious client: {rate_limiter._get_client_identifier(request)[:20]}..."
            )
        ...
        # Apply rate limiting
        rate_limiter.check_rate_limit(
            request, limit, window, f"auth_{endpoint_type}", None
        )
...
def monitor_security(
    request: Request, monitor: SecurityMonitor = Depends(get_security_monitor)
):
...
def lightweight_auth_security():
    ...
    def dependency(
        ...
        try:
            ...
            logger.warning(
                f"Non-critical security check failed during registration: {e}"
            )
            pass
    ...
    def create_access_token(
        ...
        if expires_delta:
            ...
            expire = datetime.now(timezone.utc) + timedelta(
                hours=SecurityConfig.JWT_EXPIRY_HOURS
            )
