...
from app.core.rate_limiter import RateLimitRule, rate_limiter
...
class MiddlewareHealthMonitor:
    ...
    def __init__(self):
        ...
        self.thread_pool = ThreadPoolExecutor(
            max_workers=10, thread_name_prefix="health_check"
        )
        ...
        try:
            ...
            if rate_limiter.redis_client:
                try:
                    ...
                    if (
                        redis_response_time
                        > self.performance_thresholds["redis_response_critical"]
                    ):
                        redis_status = "critical"
                    elif (
                        redis_response_time
                        > self.performance_thresholds["redis_response_warning"]
                    ):
            ...
            return HealthMetric(
                ...
                threshold_critical=self.performance_thresholds[
    ...
    async def check_jwt_service_health(self) -> HealthMetric:
        ...
        try:
            ...
            refresh_payload = validate_jwt_token(
            ...
            if total_jwt_time > 1000:  # More than 1 second for JWT operations
                ...
                status = HealthStatus.DEGRADED
                message = (
                    f"JWT configuration warnings: {', '.join(jwt_config['warnings'])}"
                )
            else:
                status = HealthStatus.HEALTHY
            ...
            return HealthMetric(
                ...
                details={
                    ...
                    "refresh_token_claims": (
                        len(refresh_payload) if refresh_payload else 0
                    ),
                },
            )
    ...
    async def check_database_session_health(
        ...
        try:
            ...
            if (
                connectivity_time
                > self.performance_thresholds["database_connection_critical"]
            ):
                issues.append(
                    f"Critical database connectivity time: {connectivity_time:.1f}ms"
                )
            elif (
                connectivity_time
                > self.performance_thresholds["database_connection_warning"]
            ):
            ...
            if (
                any("Critical" in issue for issue in issues)
                or very_long_queries > 0
                or blocked_queries > 5
            ):
                status = HealthStatus.CRITICAL
            elif issues:
                status = HealthStatus.DEGRADED
            ...
            message = (
                "Database session healthy"
                if status == HealthStatus.HEALTHY
                else f"Database issues: {', '.join(issues)}"
            )
            ...
            return HealthMetric(
                ...
                threshold_warning=self.performance_thresholds[
        ...
        try:
            ...
            try:
                ...
                if os.path.exists(audit_log_dir):
                    ...
                    log_details["recent_log_files"] = recent_logs[
                        :5
    ...
    def check_system_performance_health(self) -> HealthMetric:
        ...
        try:
            ...
            message = (
                ...
                if status == HealthStatus.HEALTHY
                else f"Performance issues: {', '.join(issues)}"
            )
    ...
    async def check_input_validation_health(self) -> HealthMetric:
        ...
        try:
            from app.core.input_validation import (  # noqa: F401
                InputValidator,
                ValidationRule,
            )
            ...
            if (
                passed_tests == total_tests and validation_time < 100
            ):  # All tests pass and fast
                status = HealthStatus.HEALTHY
    ...
    async def run_comprehensive_middleware_health_check(
        self, session: AsyncSession
        ...
        try:
            ...
            response_times = [
                m.response_time_ms for m in all_metrics.values() if m.response_time_ms
            ]
            performance_summary = {
                "total_response_time_ms": overall_response_time,
                "average_component_response_time_ms": (
                    sum(response_times) / len(response_times) if response_times else 0
                ),
                "max_component_response_time_ms": (
                    max(response_times) if response_times else 0
                ),
                "min_component_response_time_ms": (
                    min(response_times) if response_times else 0
                ),
                "components_checked": len(all_metrics),
                "healthy_components": sum(
                    1 for m in all_metrics.values() if m.status == HealthStatus.HEALTHY
                ),
                "degraded_components": sum(
                    1 for m in all_metrics.values() if m.status == HealthStatus.DEGRADED
                ),
                "critical_components": sum(
                    1 for m in all_metrics.values() if m.status == HealthStatus.CRITICAL
                ),
            ...
            for name, metric in all_metrics.items():
                if metric.status in [HealthStatus.CRITICAL, HealthStatus.UNHEALTHY]:
                    alerts.append(
                        f"ALERT: {name} is {metric.status.value}: {metric.message}"
                    )
                ...
                if (
                    name == "rate_limiter"
                    and metric.response_time_ms
                    and metric.response_time_ms > 1000
                ):
                    recommendations.append(
                        "Consider optimizing Redis connection or increasing Redis resources"
                    )
                elif (
                    name == "database_session"
                    and metric.response_time_ms
                    and metric.response_time_ms > 2000
                ):
                    recommendations.append(
                        "Database performance issues detected - check connection pool and query performance"
                    )
                elif (
                    name == "system_performance"
                    and "cpu_percent" in metric.details
                    and metric.details["cpu_percent"] > 80
                ):
                    recommendations.append(
                        "High CPU usage detected - consider scaling up or optimizing application"
                    )
                elif (
                    name == "jwt_service"
                    and metric.response_time_ms
                    and metric.response_time_ms > 500
                ):
                    recommendations.append(
                        "JWT operations slow - consider JWT secret optimization or caching"
                    )

            # Check for patterns that could cause 8-15+ second timeouts
            if performance_summary["max_component_response_time_ms"] > 5000:
                alerts.append(
                    "CRITICAL: Component response time exceeds 5 seconds - timeout risk high"
                )
                recommendations.append(
                    "URGENT: Investigate slow component - this could cause user-facing timeouts"
                )

            if performance_summary["average_component_response_time_ms"] > 2000:
                alerts.append(
                    "WARNING: Average component response time above 2 seconds"
                )
                recommendations.append(
                    "Monitor middleware performance - approaching timeout thresholds"
                )
    ...
    def get_health_trends(self, hours: int = 24) -> Dict[str, Any]:
        ...
        return {
            ...
            "average_response_time_ms": sum(r.response_time_ms for r in recent_reports)
            / len(recent_reports),
