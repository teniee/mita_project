...
class APIKeyValidator:
    ...
    async def validate_firebase_credentials(
    ...
    async def validate_upstash_redis(
        self, redis_url: str
    ) -> Tuple[bool, Optional[str]]:
...
class APIKeyManager:
        ...
        try:
            ...
            if info_file.exists():
                ...
                for key_name, info_dict in data.items():
                    ...
                    info_dict["last_validated"] = datetime.fromisoformat(
                        info_dict["last_validated"]
                    )

                    if info_dict.get("last_rotation"):
                        info_dict["last_rotation"] = datetime.fromisoformat(
                            info_dict["last_rotation"]
                        )
                    if info_dict.get("expires_at"):
                        info_dict["expires_at"] = datetime.fromisoformat(
                            info_dict["expires_at"]
                        )
    ...
    async def validate_all_keys(self) -> Dict[str, Dict[str, Any]]:
        ...
        for key_name, key_value in api_keys.items():
            try:
                ...
                # Validate the key
                valid, error = await self._validate_key(
                    service_type, key_value, key_name
                )
    ...
    async def _validate_key(
        self, service_type: ServiceType, key_value: str, key_name: str
    ) -> Tuple[bool, Optional[str]]:
        ...
        try:
            if service_type == ServiceType.OPENAI:
                ...
                if "ACCESS_KEY" in key_name:
                    ...
                    if secret_key:
                        return await self.validator.validate_aws_credentials(
    ...
    def get_key_health_status(self) -> Dict[str, Any]:
        ...
        active_keys = len(
            [k for k in self.key_info.values() if k.status == APIKeyStatus.ACTIVE]
        )
        invalid_keys = len(
            [k for k in self.key_info.values() if k.status == APIKeyStatus.INVALID]
        )
        ...
        return {
            ...
            "last_validation": max(
                (k.last_validated for k in self.key_info.values()), default=datetime.min
            ).isoformat(),
            "key_details": {
                name: info.to_dict() for name, info in self.key_info.items()
            },
        }
