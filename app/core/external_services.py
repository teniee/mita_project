...
class OpenAIService:
    ...
    def __init__(self):
        ...
        logger.info(
            f"OpenAI service configured: enabled={self.enabled}, model={self.model}"
        )
...
class RedisService:
    ...
    def __init__(self):
        ...
        logger.info(
            f"Redis service configured: enabled={self.enabled}, using_upstash={bool(self.upstash_url)}"
        )
    ...
    def get_config(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "rest_api_available": bool(
                self.upstash_rest_url and self.upstash_rest_token
            ),
        }
...
class FirebaseService:
    ...
    async def send_push_notification(
        self, token: str, title: str, body: str, data: Dict[str, str] = None
    ) -> bool:
...
class AWSService:
    ...
    def __init__(self):
        ...
        logger.info(
            f"AWS service configured: enabled={self.enabled}, region={self.region}"
        )
...
class ExternalServicesManager:
            ...
            try:
                ...
                results[service_name] = {
                    ...
                    "status": (
                        "healthy"
                        if connected
                        else "unhealthy" if service.enabled else "disabled"
                    ),
                }
    ...
    def get_service_health(self) -> Dict[str, Any]:
        ...
        enabled_services = len(
            [s for s in self.health_status.values() if s.get("enabled", False)]
        )
        healthy_services = len(
            [s for s in self.health_status.values() if s.get("status") == "healthy"]
        )
