...
class TaskMetrics:
            ...
            with self._metrics_lock:
                ...
                for queue_name, queue_stats in stats.items():
                    if isinstance(queue_stats, dict) and "length" in queue_stats:
                        self.queue_depth.labels(queue=queue_name).set(
                            queue_stats["length"]
                        )
    ...
    def time_task(self, queue: str, function: str):
        ...
        try:
            ...
            with self._metrics_lock:
                self.task_duration.labels(queue=queue, function=function).observe(
                    duration
                )
                self.task_counter.labels(
                    queue=queue, function=function, status="success"
                ).inc()
        except Exception as e:
            # Task failed
            duration = time.time() - start_time
            with self._metrics_lock:
                ...
                self.task_counter.labels(
                    queue=queue, function=function, status="failed"
                ).inc()
                self.error_counter.labels(
                    error_type=type(e).__name__, function=function, queue=queue
                ).inc()
            raise
    ...
    def record_task_wait_time(self, queue: str, priority: str, wait_seconds: float):
        ...
        with self._metrics_lock:
            self.task_wait_duration.labels(queue=queue, priority=priority).observe(
                wait_seconds
            )
    ...
    def record_financial_operation(
        ...
        with self._metrics_lock:
            self.financial_operation_counter.labels(
                operation_type=operation_type, status=status, user_tier=user_tier
            ).inc()

            if duration_seconds is not None:
                self.financial_operation_duration.labels(
                    operation_type=operation_type, user_tier=user_tier
    ...
    def record_ai_analysis(self, analysis_type: str, status: str):
        ...
        with self._metrics_lock:
            ...
            if analysis_type in ["comprehensive", "personalized"]:
                self.revenue_impact.labels(
                    operation="ai_analysis", impact_type="high_cost"
                ).inc()
        ...
        try:
            ...
            return {
                ...
                "financial_operations": {
                    ...
                    "ai_analysis_queue_depth": stats.get("default", {}).get(
                        "length", 0
                    ),
                    "budget_queue_depth": stats.get("normal", {}).get("length", 0),
                },
                "system_status": (
                    "healthy" if total_queued < 50 and total_failed < 10 else "degraded"
                ),
            }
...
def record_financial_operation(
    ...
    """Record financial operation metrics."""
    return get_task_metrics().record_financial_operation(
        operation_type, status, user_tier, duration_seconds
    )
