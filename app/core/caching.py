...
class RedisCache:
    ...
    def _initialize_redis(self):
        ...
        try:
            ...
            redis_url = getattr(settings, "UPSTASH_REDIS_URL", None) or getattr(
                settings, "REDIS_URL", None
            )

            if not redis_url:
                logger.warning(
                    "No Redis URL configured for caching. Cache will be disabled."
                )
            ...
            if redis_url == "" or not (
                ...
                logger.warning(
                    f"Invalid Redis URL format for caching: '{redis_url}' - using in-memory cache"
                )
                self.redis_client = None
                return
            ...
            logger.info(
                f"Redis cache initialized with external provider: {redis_url[:20]}..."
            )
    ...
    async def get(self, key: str) -> Optional[Any]:
        ...
        try:
            ...
            # nosec B301 - deserializes only app-written cache entries
            cache_entry = pickle.loads(data)  # nosec B301
    ...
    async def _update_access_metadata(self, key: str):
        ...
        try:
            ...
            await self.redis_client.hset(
                f"meta:{key}", "last_accessed", datetime.now(timezone.utc).isoformat()
            )
        except Exception:
...
class MultiTierCache:
    ...
    def __init__(self):
        ...
        self.hit_stats = {"memory_hits": 0, "redis_hits": 0, "misses": 0}
    ...
    def get_hit_ratio(self) -> Dict[str, float]:
        ...
        return {
            ...
            "total": (
                (self.hit_stats["memory_hits"] + self.hit_stats["redis_hits"])
                / total_requests
            )
            * 100,
        }
...
class QueryCache:
    ...
    def _generate_cache_key(self, query: str, params: Dict[str, Any] = None) -> str:
        ...
        return (
            f"query:{hashlib.md5(key_data.encode(), usedforsecurity=False).hexdigest()}"
        )
    ...
    async def cache_result(
        self, query: str, result: Any, params: Dict[str, Any] = None, ttl: int = 300
    ):
        """Cache query result"""
        cache_key = self._generate_cache_key(query, params)
...
async def check_cache_health() -> Dict[str, Any]:
    ...
    health_status = {
        ...
        "redis_cache": (
            "healthy" if stats["redis_cache"].get("connected", False) else "degraded"
        ),
        "overall": "healthy",
    }
...
def _generate_cache_recommendations(stats: Dict[str, Any]) -> List[str]:
    ...
    if total_hit_ratio < 50:
        recommendations.append(
            "Low cache hit ratio - consider increasing TTL or cache size"
        )

    memory_util = stats.get("memory_cache", {}).get("utilization", 0)
    if memory_util > 80:
        recommendations.append(
            "High memory cache utilization - consider increasing max_size"
        )

    if not stats.get("redis_cache", {}).get("connected", False):
        recommendations.append(
            "Redis cache not available - consider fixing Redis connection"
        )
