...
class TransactionManager:
    ...
    async def execute_with_retry(
        ...
        **kwargs,
        ...
        raise TransactionError(
            "Unexpected error in retry logic", original_error=last_error
        )
    ...
    async def _handle_transaction_error(self, error: Exception, transaction_type: str):
        ...
        if isinstance(error, IntegrityError):
            ...
            )  # Data integrity issues are important but not critical
...
class BulkOperationManager:
    ...
    async def bulk_insert(
        ...
        logger.info(
            f"Starting bulk insert of {total_records} records for {model_class.__name__}"
        )
        ...
        for i in range(0, total_records, self.batch_size):
            ...
            try:
                ...
                error_msg = (
                    f"Batch {batch_number} failed with integrity error: {str(e)}"
                )
                ...
                if not ignore_conflicts:
                    ...
                    failed_inserts = (
                        failed_inserts - len(batch) + individual_results["failed"]
                    )
                else:
        ...
        result = {
            ...
            "success_rate": (
                (successful_inserts / total_records) * 100 if total_records > 0 else 0
            ),
            "errors": errors,
        }
    ...
    async def _handle_individual_inserts(
        self, session: AsyncSession, model_class: Any, batch: List[Dict[str, Any]]
    ) -> Dict[str, int]:
    ...
    async def check_database_health(self) -> Dict[str, Any]:
        ...
        try:
            async with get_async_db_context() as session:
                ...
                if all(
                    [
                        health_data["connection_test"],
                        health_data["query_test"],
                        health_data["transaction_test"],
                    ]
                ):
