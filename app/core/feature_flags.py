...
class FeatureFlagManager:
    ...
    def is_enabled(
        self, flag_key: str, user_id: Optional[int] = None, default: bool = False
    ) -> bool:
        """
        ...
        try:
            ...
            if not flag:
                logger.warning(
                    f"Feature flag '{flag_key}' not found, using default: {default}"
                )
                return default
    ...
    def get_value(
        self, flag_key: str, user_id: Optional[int] = None, default: Any = None
    ) -> Any:
        """
    ...
    def set_flag_value(
        self, flag_key: str, value: Any, user_id: Optional[int] = None
    ) -> bool:
        """
        ...
        try:
            if self.redis_conn:
                ...
                self.redis_conn.setex(redis_key, self.cache_ttl, json.dumps(value))
    ...
    def get_all_flags(
        self, environment: Optional[FeatureFlagEnvironment] = None
    ...
    def _get_flag(self, flag_key: str) -> Optional[FeatureFlag]:
        ...
        try:
            if self.redis_conn:
                ...
                if data:
                    ...
                    if flag_dict.get("created_at"):
                        flag_dict["created_at"] = datetime.fromisoformat(
                            flag_dict["created_at"]
                        )
                    if flag_dict.get("updated_at"):
                        flag_dict["updated_at"] = datetime.fromisoformat(
                            flag_dict["updated_at"]
                        )
                    # Convert enums
                    flag_dict["flag_type"] = FeatureFlagType(flag_dict["flag_type"])
                    flag_dict["environments"] = [
                        FeatureFlagEnvironment(env) for env in flag_dict["environments"]
                    ]
            ...
            logger.error(
                f"Error getting feature flag definition '{flag_key}': {str(e)}"
            )
            return None
        ...
        try:
            if self.redis_conn:
                ...
                self.redis_conn.setex(
                    redis_key, 7 * 24 * 3600, json.dumps(flag_dict)  # 7 days TTL
                )

        except Exception as e:
            logger.error(
                f"Error storing feature flag definition '{flag.key}': {str(e)}"
            )
            ...
            if self.redis_conn:
                ...
                while True:
                    cursor, keys = self.redis_conn.scan(
                        cursor=cursor, match="feature_flag_def:*", count=100
                    )
    ...
    def _is_flag_enabled_for_environment(
        self, flag: FeatureFlag, environment: FeatureFlagEnvironment
    ) -> bool:
        """Check if a flag is enabled for the specified environment."""
        return flag.enabled and (
            FeatureFlagEnvironment.ALL in flag.environments
            or environment in flag.environments
        )

    def _evaluate_percentage_flag(
        self, percentage: Union[int, float], user_id: int
    ) -> bool:
...
def is_feature_enabled(
    flag_key: str, user_id: Optional[int] = None, default: bool = False
) -> bool:
    """
...
def get_feature_value(
    flag_key: str, user_id: Optional[int] = None, default: Any = None
) -> Any:
    """
