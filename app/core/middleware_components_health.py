    ...
    async def check_security_event_logging(self) -> HealthMetric:
        ...
        try:
            ...
            if log_time > 2000:  # More than 2 seconds to log
                ...
                status = HealthStatus.DEGRADED
                message = (
                    f"Security logging file system issues: {log_file_health['message']}"
                )
            else:
                status = HealthStatus.HEALTHY
    ...
    async def check_connection_pool_health(self, session: AsyncSession) -> HealthMetric:
        ...
        try:
            ...
            basic_result = await session.execute(
                text("SELECT 1, NOW() as current_time")
            )
            basic_result.first()
            ...
            message = (
                ...
                else f"Connection pool issues: {', '.join(issues)}"
            )
    ...
    async def check_query_performance(self, session: AsyncSession) -> HealthMetric:
        ...
        try:
            ...
            try:
                ...
                query_tests.append(
                    {"type": "table_scan", "time_ms": None, "error": "Failed"}
                )
            ...
            try:
                ...
                query_tests.append(
                    {"type": "index_stats", "time_ms": None, "error": "Failed"}
                )
            ...
            total_query_time = sum(
                test["time_ms"] for test in query_tests if test["time_ms"]
            )
            max_query_time = max(
                (test["time_ms"] for test in query_tests if test["time_ms"]), default=0
            )
            ...
            if len(slow_query_results) > 0:
                avg_slow_time = sum(row.mean_time for row in slow_query_results) / len(
                    slow_query_results
                )
                issues.append(
                    f"{len(slow_query_results)} slow queries averaging {avg_slow_time:.1f}ms"
                )
            ...
            message = (
                ...
                if status == HealthStatus.HEALTHY
                else f"Query performance issues: {', '.join(issues)}"
            )
...
class CacheMiddlewareHealthChecker:
    ...
    def _init_redis(self):
        ...
        try:
            ...
            logger.warning(
                f"Redis client initialization failed for health check: {str(e)}"
            )
    ...
    async def check_redis_cache_health(self) -> HealthMetric:
        ...
        try:
            ...
            if (
                max_memory > 0 and (used_memory / max_memory) > 0.9
            ):  # More than 90% memory usage
                issues.append(
                    f"High Redis memory usage: {(used_memory / max_memory) * 100:.1f}%"
                )
            ...
            if (
                hit_ratio < 80 and (keyspace_hits + keyspace_misses) > 1000
            ):  # Low hit ratio with significant traffic
            ...
            message = (
                "Redis cache healthy"
                if status == HealthStatus.HEALTHY
                else f"Redis issues: {', '.join(issues)}"
            )
