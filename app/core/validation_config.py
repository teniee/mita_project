...
class SecurityConfig:
    ...
    session_timeout_minutes: int = (
        120  # Match ACCESS_TOKEN_EXPIRE_MINUTES for consistent session duration
    )
...
class ValidationConfigManager:
            ...
            try:
                ...
                if region_code in [r.value for r in Region]:
                    ...
                    if region in self.configs:
                        ...
                        self.configs[region] = self._create_config_from_data(
                            region, config_data
                        )
    ...
    def _create_config_from_data(
        self, region: Region, data: Dict[str, Any]
    ) -> RegionConfig:
    ...
    def export_config(self, region: Region, format: str = "yaml") -> Optional[str]:
        ...
        config_dict = {
            ...
            "security": {
                ...
                "suspicious_amount_threshold": str(
                    config.security.suspicious_amount_threshold
                ),
            },
    ...
    def validate_region_compatibility(self, region: Region) -> Dict[str, Any]:
        ...
        if not config.security.require_digits:
            warnings.append(
                "Requiring digits in passwords is recommended for financial applications"
            )

        # Check currency limits
        if config.currency.max_transaction <= config.currency.min_amount:
            errors.append(
                "Maximum transaction amount must be greater than minimum amount"
            )
