...
from app.core.middleware_health_monitor import HealthStatus, MiddlewareHealthReport
...
class HealthAlertManager:
    ...
    def _initialize_alert_rules(self) -> List[AlertRule]:
        ...
        return [
            ...
            AlertRule(
                ...
                condition=lambda report: report.overall_status
            ...
            AlertRule(
                ...
                condition=lambda report: (
                    "database_session" in report.metrics
                    and report.metrics["database_session"].status
            ...
            AlertRule(
                ...
                condition=lambda report: (
                    "database_session" in report.metrics
                    and report.metrics["database_session"].response_time_ms
                    and report.metrics["database_session"].response_time_ms > 3000
                ),
                severity=AlertSeverity.HIGH,
                channels=[AlertChannel.EMAIL, AlertChannel.WEBHOOK],
                cooldown_minutes=10,
            ...
            AlertRule(
                ...
                condition=lambda report: (
                    "rate_limiter" in report.metrics
                    and report.metrics["rate_limiter"].status
            ...
            AlertRule(
                ...
                condition=lambda report: (
                    "jwt_service" in report.metrics
                    and report.metrics["jwt_service"].status == HealthStatus.UNHEALTHY
                ),
                severity=AlertSeverity.HIGH,
                channels=[AlertChannel.EMAIL, AlertChannel.WEBHOOK],
                cooldown_minutes=5,
            ...
            AlertRule(
                ...
                condition=lambda report: (
                    "redis_cache" in report.metrics
                    and report.metrics["redis_cache"].status == HealthStatus.UNHEALTHY
                    and "redis_configured" in report.metrics["redis_cache"].details
                    and report.metrics["redis_cache"].details["redis_configured"]
                    is not False
                ),
            ...
            AlertRule(
                ...
                condition=lambda report: (
                    "system_performance" in report.metrics
                    and report.metrics["system_performance"].status
                    == HealthStatus.CRITICAL
                ),
                severity=AlertSeverity.HIGH,
                channels=[AlertChannel.EMAIL, AlertChannel.WEBHOOK],
    ...
    def _setup_notification_channels(self) -> Dict[AlertChannel, Dict[str, Any]]:
        ...
        if hasattr(settings, "ALERT_EMAIL_SMTP_HOST"):
            channels[AlertChannel.EMAIL] = {
                ...
                "from_email": getattr(
                    settings, "ALERT_EMAIL_FROM", "alerts@mita.finance"
                ),
                "to_emails": getattr(
                    settings, "ALERT_EMAIL_TO", "ops@mita.finance"
                ).split(","),
        ...
        if hasattr(settings, "ALERT_SLACK_WEBHOOK_URL"):
            channels[AlertChannel.SLACK] = {
                ...
                "username": getattr(
                    settings, "ALERT_SLACK_USERNAME", "MITA Health Monitor"
                ),
            }
    ...
    async def process_health_report(
        self, health_report: MiddlewareHealthReport
    ) -> List[Alert]:
    ...
    def _generate_alert_message(
        self, rule: AlertRule, health_report: MiddlewareHealthReport
    ) -> str:
...
Support: support@mita.finance
    ...
    async def _send_webhook_alert(self, alert: Alert):
        ...
        try:
            response = requests.post(  # nosec B113 - timeout=config['timeout'] below
                config["url"],
                json=payload,
    ...
    async def _send_pagerduty_alert(self, alert: Alert):
        ...
        payload = {
            ...
            "payload": {
                ...
                "severity": (
                    "critical" if alert.severity == AlertSeverity.CRITICAL else "error"
                ),
                "component": alert.component,
                "custom_details": {
                    "alert_id": alert.id,
    ...
    def acknowledge_alert(self, alert_id: str, acknowledger: str) -> bool:
        ...
        with self.lock:
            if alert_id in self.active_alerts:
                self.active_alerts[alert_id].acknowledgements.append(
                    f"{acknowledger} at {datetime.now(timezone.utc).isoformat()}"
                )
        ...
        with self.lock:
            if alert_id in self.active_alerts:
                ...
                alert.acknowledgements.append(
                    f"Resolved by {resolver} at {alert.resolved_at.isoformat()}"
                )
