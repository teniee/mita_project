...
class SimpleRateLimiter:
    ...
    async def _redis_sliding_window(
        self, redis_client, key: str, window_seconds: int, limit: int
    ) -> Tuple[int, int, bool]:
    ...
    async def check_rate_limit(
        ...
        try:
            if redis_client:
                # Use Redis
                current_count, time_until_reset, is_exceeded = (
                    await self._redis_sliding_window(
                ...
                if user_key:
                    user_count, user_reset, user_exceeded = (
                        await self._redis_sliding_window(
                            redis_client,
                            user_key,
                            window_seconds,
                ...
                # Use memory fallback
                current_count, time_until_reset, is_exceeded = (
                    self._memory_sliding_window(client_key, window_seconds, limit)
                )
        ...
        if is_exceeded:
            logger.warning(
                f"Rate limit exceeded for {endpoint}: {current_count}/{limit} requests"
            )
...
async def check_password_reset_rate_limit(request: Request):
    ...
    await rate_limiter.check_rate_limit(
        request, 3, 1800, "auth_password_reset"
    )  # 3 per 30 minutes
    return True
...
async def check_token_refresh_rate_limit(request: Request):
    ...
    await rate_limiter.check_rate_limit(
        request, 20, 300, "auth_token_refresh"
    )  # 20 per 5 minutes
    return True
...
async def check_behavioral_insights_rate_limit(request: Request):
    ...
    await rate_limiter.check_rate_limit(
        request, 10, 60, "analytics_behavioral_insights"
    )  # 10 per minute
    return True
...
async def check_seasonal_patterns_rate_limit(request: Request):
    ...
    await rate_limiter.check_rate_limit(
        request, 5, 60, "analytics_seasonal_patterns"
    )  # 5 per minute
    return True
...
async def check_aggregate_rate_limit(request: Request):
    ...
    await rate_limiter.check_rate_limit(
        request, 20, 60, "analytics_aggregate"
    )  # 20 per minute
    return True
...
async def check_anomalies_rate_limit(request: Request):
    ...
    await rate_limiter.check_rate_limit(
        request, 20, 60, "analytics_anomalies"
    )  # 20 per minute
    return True
