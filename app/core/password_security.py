...
BCRYPT_ROUNDS = getattr(
    settings, "BCRYPT_ROUNDS_PRODUCTION", 12
)  # Industry standard for financial applications
BCRYPT_PERFORMANCE_TARGET_MS = getattr(
    settings, "BCRYPT_PERFORMANCE_TARGET_MS", 500
...
BCRYPT_DEV_ROUNDS = getattr(
    settings, "BCRYPT_ROUNDS_DEVELOPMENT", 10
)  # Development balance of security and speed
...
async def hash_password_async(password: str) -> str:
    ...
    try:
        password_hash = await loop.run_in_executor(
            thread_pool, hash_password_sync, password
        )
        return password_hash
    except Exception as e:
...
async def verify_password_async(password: str, hashed_password: str) -> bool:
    ...
    try:
        is_valid = await loop.run_in_executor(
            thread_pool, verify_password_sync, password, hashed_password
        )
        return is_valid
    except Exception as e:
...
def hash_password(password: str) -> str:
    ...
    """
    logger.debug(
        "Using legacy hash_password function - consider upgrading to hash_password_sync/async"
    )
...
def get_password_performance_stats() -> dict:
    ...
    if stats["total_verifications"] > 0:
        stats["avg_verify_time_ms"] = (
            stats["total_verify_time"] / stats["total_verifications"]
        )
    else:
...
def test_password_performance() -> dict:
    ...
    if test_results["times_ms"]:
        test_results["average_ms"] = sum(test_results["times_ms"]) / len(
            test_results["times_ms"]
        )
        test_results["meets_target"] = (
            test_results["average_ms"] <= BCRYPT_PERFORMANCE_TARGET_MS
        )
...
def validate_bcrypt_configuration() -> dict:
    ...
    if validation_result["valid"]:
        if validation_result["warnings"]:
            logger.warning(
                f"Password security validation passed with warnings: {validation_result['warnings']}"
            )
        else:
            logger.info("Password security validation passed successfully")
    else:
        logger.error(
            f"Password security validation failed: {validation_result['issues']}"
        )
