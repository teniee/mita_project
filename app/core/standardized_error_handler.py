...
class AuthorizationError(StandardizedAPIException):
    ...
    def __init__(
        ...
        error_code: str = ErrorCode.AUTH_INSUFFICIENT_PERMISSIONS,
        **kwargs,
    ):
...
class ValidationError(StandardizedAPIException):
    ...
    def __init__(
        ...
        error_code: str = ErrorCode.VALIDATION_INVALID_FORMAT,
        **kwargs,
    ):
        super().__init__(
            message, error_code, status.HTTP_422_UNPROCESSABLE_ENTITY, **kwargs
        )
...
class BusinessLogicError(StandardizedAPIException):
    ...
    def __init__(
        self,
        message: str,
        error_code: str = ErrorCode.BUSINESS_INVALID_OPERATION,
        **kwargs,
    ):
        super().__init__(
            message, error_code, status.HTTP_422_UNPROCESSABLE_ENTITY, **kwargs
...
class DatabaseError(StandardizedAPIException):
    ...
    def __init__(
        ...
        error_code: str = ErrorCode.DATABASE_QUERY_ERROR,
        **kwargs,
    ):
        super().__init__(
            message, error_code, status.HTTP_500_INTERNAL_SERVER_ERROR, **kwargs
        )
...
class ErrorResponse:
    ...
    def create(
        ...
        if isinstance(error, StandardizedAPIException):
            ...
            error, (ValidationError, PydanticValidationError, RequestValidationError)
        ):
            ...
            response = {
                "success": False,
                "error": {
                    "code": error_code_mapping.get(
                        error.status_code, ErrorCode.SYSTEM_INTERNAL_ERROR
                    ),
                    "message": (
                        error.detail
...
class StandardizedErrorHandler:
    ...
    def log_error(
        ...
        if request:
            try:
                ...
                log_context["context_error"] = (
                    f"Failed to capture request context: {str(ctx_error)}"
                )
        ...
        if isinstance(
            error, (AuthenticationError, AuthorizationError, ValidationError)
        ):
            ...
            logger.warning(
                f"Business logic error [{error_id}]: {error}", extra=log_context
            )
        elif isinstance(error, RateLimitError):
            logger.warning(
                f"Rate limit exceeded [{error_id}]: {error}", extra=log_context
            )
        elif isinstance(error, StarletteHTTPException) and error.status_code < 500:
            logger.info(
                f"HTTP {error.status_code} [{error_id}]: {error}", extra=log_context
            )
        else:
            # Server errors - log with full stack trace
            logger.error(
                f"Server error [{error_id}]: {error}", extra=log_context, exc_info=True
            )
    ...
    def create_response(
        ...
        # Create headers
        headers = {}
        if isinstance(error, (StandardizedAPIException, RateLimitError)) and hasattr(
            error, "retry_after"
        ):
            if error.retry_after:
    ...
    if isinstance(error, IntegrityError):
        ...
        if (
            ...
            return BusinessLogicError(
                "Resource already exists", ErrorCode.RESOURCE_ALREADY_EXISTS
            )
        ...
        return ResourceNotFoundError("Resource", "The requested resource was not found")

    elif isinstance(error, MultipleResultsFound):
        return DatabaseError(
            "Multiple results found where one expected", ErrorCode.DATABASE_QUERY_ERROR
        )

    elif isinstance(error, (DisconnectionError, SQLTimeoutError)):
        return DatabaseError(
            "Database connection issue", ErrorCode.DATABASE_CONNECTION_ERROR
        )
...
def validate_email(email: str) -> str:
    ...
    if not isinstance(email, str):
        raise ValidationError(
            "Email must be a string", ErrorCode.VALIDATION_INVALID_FORMAT
        )
    ...
    if not email:
        raise ValidationError(
            "Email address is required", ErrorCode.VALIDATION_REQUIRED_FIELD
        )
...
def validate_password(password: str) -> None:
    ...
    if not isinstance(password, str):
        raise ValidationError(
            "Password must be a string", ErrorCode.VALIDATION_INVALID_FORMAT
        )
