...
class SentryContextManager:
            ...
            try:
                ...
                scope.set_context(
                    "compliance",
                    {
                        ...
                        "retention_period": (
                            "7_years" if financial_ctx.pci_scope else "1_year"
                        ),
                ...
                yield {
                    ...
                    "start_time": start_time,
                    "scope": scope,
                    "set_result": lambda result: scope.set_extra(
                        "operation_result", str(type(result).__name__)
                    ),
    ...
    def set_financial_user_context(self, user_ctx: UserContext):
        ...
        set_context(
            "user_details",
            {
                ...
                "last_login": (
                    user_ctx.last_login.isoformat() if user_ctx.last_login else None
                ),
            },
        )
    ...
    def export_compliance_report(
        self, output_file: str, user_id: Optional[str] = None, days_back: int = 30
    ):
        """Export compliance report for audit purposes"""

        audit_data = self.get_audit_trail(user_id=user_id, hours_back=days_back * 24)
    ...
    def _generate_audit_summary(
        ...
        return {
            ...
            "most_active_user": (
                max(user_activity.items(), key=lambda x: len(x[1]))[0]
                if user_activity
                else None
            ),
...
def set_transaction(
    transaction_id: str, user_id: str, amount: float, currency: str, **kwargs
):
