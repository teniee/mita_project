    ...
    async def _record_failure(self):
        ...
        async with self._lock:
            ...
            if (
                ...
                logger.warning(
                    f"Circuit breaker '{self.name}' transitioned to OPEN after {self.stats.consecutive_failures} failures"
                )
            ...
            for attempt in range(self.config.max_retry_attempts):
                try:
                    ...
                    if attempt < self.config.max_retry_attempts - 1:
                        ...
                        logger.error(
                            f"Circuit breaker '{self.name}' all {self.config.max_retry_attempts} attempts failed"
                        )
                        raise
                except Exception as e:
                    # Non-triggering exceptions are re-raised immediately
                    logger.error(
                        f"Circuit breaker '{self.name}' non-retriable error: {str(e)}"
                    )
                    raise
    ...
    def get_health_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "overall_health": (
                ...
                else ("degraded" if degraded_services > 0 else "critical")
            ),
