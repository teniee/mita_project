...
class MitaTaskQueue:
        ...
        **kwargs,
        ...
        try:
            ...
            task_result = TaskResult(
                task_id=job.id, status=TaskStatus.QUEUED, metadata=metadata
            )
    ...
    def clean_old_jobs(self, max_age_hours: int = 48) -> Dict[str, int]:
        ...
        logger.info(
            f"Cleaned {cleaned['completed']} completed and {cleaned['failed']} failed jobs"
        )
        return cleaned
    ...
    def _store_task_result(self, task_id: str, task_result: TaskResult) -> None:
        ...
        try:
            self.redis_conn.setex(
                key, self.result_ttl, json.dumps(task_result.to_dict())
            )
        except Exception as e:
...
def get_task_queue():
    ...
    if _task_queue is None:
        try:
            ...
            logger.warning(
                f"Failed to initialize task queue: {e} - tasks will be skipped"
            )
