...
from pydantic import condecimal, field_validator, validator
...
class InputSanitizer:
    ...
    @classmethod
    def sanitize_string(
        cls, value: str, max_length: int = None, allow_html: bool = False
    ) -> str:
    ...
    def sanitize_amount(
        ...
        try:
            ...
            quantized_amount = decimal_amount.quantize(
                Decimal("0.01"), rounding=ROUND_HALF_UP
            )

            # Additional validation for suspicious patterns
            if decimal_amount != quantized_amount and abs(
                decimal_amount - quantized_amount
            ) > Decimal("0.001"):
                logger.warning(
                    f"Suspicious precision in {field_name}: {decimal_amount}"
                )
    ...
    def sanitize_zip_code(cls, zip_code: str, country: str = "US") -> str:
        ...
        if country == "US":
            ...
            if not re.match(r"^\d{5}(-\d{4})?$", zip_code):
                raise ValidationError(
                    "Invalid US ZIP code format (use 12345 or 12345-6789)"
                )
...
class EnhancedUserValidator(BaseValidator):
    ...
    country: Optional[str] = Field(
        None, min_length=2, max_length=2, description="Country code (US only)"
    )
    state: Optional[str] = Field(
        None, min_length=2, max_length=2, description="State code"
    )
    ...
    timezone: Optional[str] = Field(
        None, min_length=3, max_length=50, description="User timezone"
    )
...
class EnhancedTransactionValidator(BaseValidator):
    ...
    category: str = Field(
        ..., min_length=1, max_length=50, description="Transaction category"
    )
    ...
    def validate_description(cls, v):
        ...
        for pattern in suspicious_patterns:
            if re.search(pattern, sanitized.lower()):
                logger.warning(
                    f"Suspicious transaction description: {sanitized[:50]}..."
                )
...
class EnhancedBudgetValidator(BaseValidator):
    ...
    period: str = Field(
        ..., pattern=r"^(monthly|weekly|yearly)$", description="Budget period"
    )
    category: str = Field(
        ..., min_length=1, max_length=50, description="Budget category"
    )
    amount: condecimal(max_digits=12, decimal_places=2, gt=0) = Field(
    ...
    description: Optional[str] = Field(
        None, max_length=500, description="Budget description"
    )
    ...
    is_flexible: Optional[bool] = Field(
        True, description="Can budget be adjusted automatically"
    )
    ...
    def validate_start_date(cls, v):
        ...
        if start_date < date.today() - timedelta(days=30):
            raise ValueError(
                "Budget start date cannot be more than 30 days in the past"
            )

        if start_date > date.today() + timedelta(days=365):
            raise ValueError(
                "Budget start date cannot be more than 1 year in the future"
            )
    ...
    def validate_end_date(cls, v, info):
        ...
        start_date = (
            info.data.get("start_date", date.today()) if info.data else date.today()
        )
...
class EnhancedGoalValidator(BaseValidator):
    ...
    monthly_contribution: Optional[
        condecimal(max_digits=10, decimal_places=2, ge=0)
    ] = Field(None, description="Planned monthly contribution")
    ...
    def validate_target_date(cls, v):
        ...
        if target_date > date.today() + timedelta(days=365 * 20):
            raise ValueError(
                "Goal target date cannot be more than 20 years in the future"
            )
...
class EnhancedValidationMiddleware:
    ...
    def _pre_validation_security_check(
        self, data: Dict[str, Any], user_id: str = None, ip_address: str = None
    ):
        ...
        for pattern in injection_patterns:
            if re.search(pattern, data_str):
                self._log_security_event(
                    ...
                    details={"pattern": pattern, "data_preview": data_str[:200]},
                    user_id=user_id,
                    ip_address=ip_address,
                    severity="high",
                )
    ...
    def _post_validation_security_check(
        self,
                ...
                if isinstance(value, (int, float, Decimal)):
                    ...
                    if amount > Decimal("100000"):  # $100,000
                        self._log_security_event(
                            ...
                            details={"field": key, "amount": str(amount)},
                            user_id=user_id,
                            ip_address=ip_address,
                        )
...
# Legacy support (intentionally shadows the pydantic import above)
validator = EnhancedValidationMiddleware()  # noqa: F811
    ...
    def decorator(func):
        async def wrapper(*args, **kwargs):
            ...
            if "user" in kwargs:
                ...
                user_id = getattr(
                    kwargs["current_user"], "id", str(kwargs["current_user"])
                )
            ...
            if data:
                try:
                    validation_middleware = (
                        enhanced_validator if enhanced else validator
                    )
...
def validate_user_input(
    data: Dict[str, Any], user_id: str = None, ip_address: str = None
) -> Dict[str, Any]:
