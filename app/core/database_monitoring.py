...
class DatabaseQueryMonitor:
                ...
                if severity:
                    ...
                    logger.warning(
                        ...
                        extra={
                            ...
                            "performance_impact": (
                                "high" if execution_time > 5.0 else "medium"
                            ),
                        },
                    )
    ...
    def get_query_statistics(self, limit: int = 10) -> Dict[str, Any]:
        ...
        with self.lock:
            ...
            return {
                "total_queries": sum(
                    stats["count"] for stats in self.query_stats.values()
                ),
    ...
    def get_connection_metrics(self) -> Dict[str, Any]:
        ...
        with self.lock:
            ...
            return {
                ...
                "pool_utilization": (
                    (latest.checked_out / latest.pool_size) * 100
                    if latest.pool_size > 0
                    else 0
                ),
                "connection_efficiency": (
                    (latest.active_connections / latest.total_connections) * 100
...
class OptimizedDatabaseEngine:
        ...
        try:
            ...
            if readonly_url:
                ...
                if readonly_url.startswith("postgresql://"):
                    readonly_url = readonly_url.replace(
                        "postgresql://", "postgresql+asyncpg://", 1
                    )
        ...
        try:
            for engine_name, engine in self.engines.items():
                ...
                def on_connect(dbapi_conn, connection_record):
                    logger.debug(
                        f"New database connection established: {id(dbapi_conn)}"
                    )
                ...
                def on_checkin(dbapi_conn, connection_record):
                    if hasattr(connection_record, "checkout_time"):
                        checkout_duration = (
                            time.time() - connection_record.checkout_time
                        )
                        if checkout_duration > 10:  # Log long-running connections
                            logger.warning(
                                f"Long connection checkout: {checkout_duration:.2f}s"
                            )
    ...
    def _generate_optimization_suggestions(self, query: QueryMetrics) -> List[str]:
        ...
        if query.execution_time > 5.0:
            suggestions.append(
                "Query execution time is very high - consider adding database indexes"
            )

        if "JOIN" in query_text and query.execution_time > 2.0:
            suggestions.append(
                "Complex joins detected - verify indexes on join columns"
            )
    ...
    if query_stats.get("total_queries", 0) > 0:
        ...
        if len(query_stats.get("recent_slow_queries", [])) > 5:
            recommendations.append(
                "High number of slow queries detected - review database indexes"
            )
...
def _calculate_health_score(stats: Dict[str, Any]) -> int:
    ...
    if query_stats.get("slowest_queries"):
        avg_time = sum(q["avg_time"] for q in query_stats["slowest_queries"]) / len(
            query_stats["slowest_queries"]
        )
        if avg_time > 2.0:
            score -= 20
        elif avg_time > 1.0:
