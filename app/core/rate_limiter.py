...
class RateLimiter:
    ...
    def __init__(self):
        ...
        self.default_rules = {
            "global": RateLimitRule(
                "global", 1500, 3600, per="ip"
            ),  # 1500 per hour per IP (increased from 1000)
            "auth": RateLimitRule(
                "auth", 12, 300, per="ip"
            ),  # 12 auth attempts per 5 min per IP (increased from 5)
            "api": RateLimitRule(
                "api", 200, 300, per="user"
            ),  # 200 API calls per 5 min per user (increased from 100)
            "upload": RateLimitRule(
                "upload", 20, 3600, per="user"
            ),  # 20 uploads per hour per user (increased from 10)
            "heavy": RateLimitRule(
                "heavy", 10, 60, per="user"
            ),  # 10 heavy operations per minute per user (increased from 5)
        }

        # Security rules for suspicious behavior (SOFTENED for better UX)
        self.security_rules = {
            "failed_auth": RateLimitRule(
                "failed_auth", 6, 900, per="ip"
            ),  # 6 failed auth per 15 min (increased from 3)
            "suspicious": RateLimitRule(
                "suspicious", 20, 3600, per="ip"
            ),  # 20 suspicious requests per hour (increased from 10)
            "brute_force": RateLimitRule(
                "brute_force", 2, 3600, per="ip"
            ),  # 2 brute force attempts per hour (increased from 1)
        }
    ...
    async def check_rate_limit(
        self, key: str, rule: RateLimitRule, request: Request = None
    ...
    def _build_key(
        self, base_key: str, rule: RateLimitRule, request: Request = None
    ) -> str:
    ...
    async def _redis_sliding_window(
        self, key: str, rule: RateLimitRule, now: float, window_start: float
    ) -> Tuple[bool, Dict[str, Any]]:
    ...
    async def _memory_sliding_window(
        ...
        # Remove old requests
        store["requests"] = [
            req_time for req_time in store["requests"] if req_time > window_start
        ]
    ...
    async def _redis_token_bucket(
        ...
        return allowed, {
            ...
            "reset_time": int(
                now + ((rule.requests - tokens) * rule.window / rule.requests)
            ),
            "retry_after": (
                0 if allowed else int((1.0 - tokens) * rule.window / rule.requests)
            ),
            "window": rule.window,
        }
    ...
    async def _memory_token_bucket(
        ...
        return allowed, {
            ...
            "reset_time": int(
                now + ((rule.requests - bucket["tokens"]) * rule.window / rule.requests)
            ),
            "retry_after": (
                0
                if allowed
                else int((1.0 - bucket["tokens"]) * rule.window / rule.requests)
            ),
            "window": rule.window,
        }
        ...
        if self.redis_client:
            ...
            if window_key not in self.memory_store:
                self.memory_store[window_key] = {
                    "count": 0,
                    "expires": window_start + rule.window,
                }
