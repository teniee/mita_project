...
class SecretValue:
    ...
    def __post_init__(self):
        ...
        logger.info(
            f"Secret accessed: {self.metadata.name} (category: {self.metadata.category.value})"
        )
...
class UnifiedSecretManager:
    ...
    def _init_aws_provider(self):
        ...
        try:
            ...
            self.providers[SecretProvider.AWS_SECRETS_MANAGER] = session.client(
                "secretsmanager"
            )
    ...
    def _init_gcp_provider(self):
        ...
        try:
            ...
            if project_id:
                self.providers[SecretProvider.GCP_SECRET_MANAGER] = (
        ...
        for secret_name, secret_config in secret_configs.items():
            try:
                metadata = SecretMetadata(
                    ...
                    provider=SecretProvider(
                        secret_config.get("provider", "environment")
                    ),
                    version=secret_config.get("version", "1"),
                    created_at=datetime.now(timezone.utc),
                    expires_at=None,
                    rotation_interval_days=secret_config.get(
    ...
    async def _retrieve_from_provider(
        self, name: str, metadata: SecretMetadata, version: Optional[str] = None
    ) -> str:
        ...
        try:
            ...
            if secret_key in secret.data:
                ...
                raise SecretNotFoundError(
                    f"Secret key not found in Kubernetes secret: {secret_key}"
                )
    ...
    async def _try_fallback_providers(
        ...
        for provider in fallback_order:
            if provider in self.providers:
                try:
                    logger.warning(
                        f"Trying fallback provider {provider.value} for secret {name}"
                    )
    ...
    async def store_secret(
        self, name: str, value: str, metadata: Optional[SecretMetadata] = None
    ) -> bool:
        """
        ...
        try:
            if metadata.provider == SecretProvider.AWS_SECRETS_MANAGER:
                ...
                raise SecretManagerError(
                    f"Storage not supported for provider: {metadata.provider}"
                )

            # Invalidate cache
            cache_keys_to_remove = [
                k for k in self.secret_cache.keys() if k.startswith(f"{name}:")
            ]
            for key in cache_keys_to_remove:
                del self.secret_cache[key]

            self._audit_log(
                "store_success", name, {"provider": metadata.provider.value}
            )
            return True
    ...
    async def _check_compliance(self, name: str, metadata: SecretMetadata) -> None:
        ...
        if metadata.access_count > 1000:  # Threshold for investigation
            logger.warning(
                f"High access count for secret {name}: {metadata.access_count}"
            )
    ...
    def _get_request_id(self) -> str:
        ...
        return hashlib.md5(
            f"{datetime.now(timezone.utc)}{secrets.token_hex(8)}".encode(),
...
def create_secret_manager(config_path: Optional[str] = None) -> UnifiedSecretManager:
    ...
    if config_path and os.path.exists(config_path):
        ...
        config = {
            ...
            "kubernetes": {"namespace": os.getenv("NAMESPACE", "default")},
            "secrets": {
                "jwt_secret": {
                    "category": "critical",
                    "provider": "kubernetes",
