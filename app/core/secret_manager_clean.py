...
class SecretManager:
    ...
    def _validate_secret(self, secret_name: str, value: str) -> None:
        ...
        if config.min_length and len(value) < config.min_length:
            errors.append(
                f"Secret '{secret_name}' must be at least {config.min_length} characters"
            )
        ...
        if self.environment == "production":
            for indicator in placeholder_indicators:
                if indicator in value:
                    errors.append(
                        f"Secret '{secret_name}' contains placeholder value: {indicator}"
                    )
        ...
        if "jwt" in secret_name.lower():
            if len(value) < 32:
                errors.append(
                    f"JWT secret '{secret_name}' must be at least 32 characters for security"
                )
        ...
        if secret_name == "database_url":
            if not any(
                protocol in value
                for protocol in ["postgresql://", "postgres://", "sqlite://"]
            ):
                errors.append(
                    f"Database URL '{secret_name}' must contain valid protocol"
                )
        ...
        if secret_name == "openai_api_key":
            if not value.startswith("sk-"):
                if self.environment == "production":
                    errors.append(
                        f"OpenAI API key '{secret_name}' must start with 'sk-' in production"
                    )

        if errors:
            raise SecretValidationError(
                f"Secret validation failed: {'; '.join(errors)}"
            )
    ...
    def validate_all_secrets(self) -> Dict[str, Any]:
        ...
        for secret_name, config in self.SECRET_CONFIGS.items():
            try:
                ...
                if (
                    not value
                    and config.required_in_production
                    ...
                    results["warnings"].append(
                        f"Optional secret '{secret_name}' is missing (ok in {self.environment})"
                    )
                ...
                results["errors"].append(
                    f"Error validating secret '{secret_name}': {str(e)}"
                )
                results["valid"] = False
    ...
    def rotate_secret(self, secret_name: str, new_value: str) -> bool:
        ...
        if self.environment == "production":
            self.logger.warning(
                f"Secret rotation for '{secret_name}' should be handled by external secret manager"
            )
            return False
        ...
        try:
            ...
            self.logger.info(
                f"Secret '{secret_name}' rotated successfully in {self.environment}"
            )
            return True
        except SecretValidationError as e:
