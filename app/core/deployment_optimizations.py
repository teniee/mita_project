...
class DeploymentOptimizer:
    ...
    def get_uvicorn_config(platform: str) -> Dict[str, Any]:
        ...
        config = {
            "host": "0.0.0.0",  # nosec B104 - containerized deployment must bind all interfaces
        ...
        if platform == "aws_lambda":
            ...
            config.update({"timeout_keep_alive": 0, "access_log": False, "workers": 1})

        return config

    @staticmethod
    def apply_database_optimizations(
        platform: str, engine_config: Dict[str, Any]
    ) -> Dict[str, Any]:
