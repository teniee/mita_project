...
from app.core.secret_manager import SecretCategory, UnifiedSecretManager
...
class SecretRotationManager:
    ...
    def _load_rotation_policies(self) -> Dict[SecretCategory, RotationPolicy]:
        ...
        for category, default_policy in default_policies.items():
            ...
            policies[category] = RotationPolicy(
                category=category,
                rotation_interval_days=category_config.get(
                    "rotation_interval_days", default_policy.rotation_interval_days
                ),
                max_retries=category_config.get(
                    "max_retries", default_policy.max_retries
                ),
                validation_timeout_seconds=category_config.get(
                    "validation_timeout_seconds",
                    default_policy.validation_timeout_seconds,
                ),
                notification_recipients=category_config.get(
                    "notification_recipients", default_policy.notification_recipients
                ),
                requires_approval=category_config.get(
                    "requires_approval", default_policy.requires_approval
                ),
                emergency_contacts=category_config.get(
                    "emergency_contacts", default_policy.emergency_contacts
                ),
            )
    ...
    def register_validation_function(
        self, secret_name: str, validation_func: Callable[[str], bool]
    ):
        """
    ...
    async def schedule_rotation(
        self, secret_name: str, force: bool = False
    ) -> RotationTask:
        """
        ...
        if not policy:
            raise ValueError(
                f"No rotation policy found for category: {secret_metadata.category}"
            )
        ...
        if force:
            ...
            scheduled_time = last_rotation + timedelta(
    ...
    async def execute_rotation(self, task: RotationTask) -> RotationResult:
        ...
        try:
            ...
            new_secret_value = await self.secret_manager.rotate_secret(
                task.secret_name, force=True
            )
            ...
            if await self._validate_secret(
                task.secret_name, new_secret_value, policy.validation_timeout_seconds
            ):
                ...
                logger.error(
                    f"Validation failed for rotated secret: {task.secret_name}"
                )
    ...
    async def _validate_secret(
        self, secret_name: str, secret_value: str, timeout_seconds: int
    ) -> bool:
        """
        ...
        try:
            ...
            if secret_name in self.validation_functions:
                ...
                result = await asyncio.wait_for(
                    asyncio.create_task(
                        self._run_validation(validation_func, secret_value)
                    ),
    ...
    async def _run_validation(
        self, validation_func: Callable, secret_value: str
    ) -> bool:
    ...
    async def _default_validation(self, secret_name: str, secret_value: str) -> bool:
        ...
        if "jwt" in secret_name.lower() or "token" in secret_name.lower():
            ...
            return (
                len(secret_value) >= 12
                and any(c.isupper() for c in secret_value)
                and any(c.islower() for c in secret_value)
                and any(c.isdigit() for c in secret_value)
            )
    ...
    async def _rollback_secret(self, task: RotationTask) -> bool:
        ...
        try:
            ...
            if not task.old_version:
                logger.error(
                    f"No previous version available for rollback: {task.secret_name}"
                )
                return False
        ...
        try:
            ...
            if self.smtp_config and recipients:
                await self._send_email_notification(
                    event_type, message, recipients, context
                )
            ...
            if (
                ...
                and event_type in ["rotation_failure", "rollback_required"]
                and context.get("task", {}).get("category") == "critical"
            ):
...
MITA DevOps Team
    ...
    async def run_scheduled_rotations(self):
        ...
        due_tasks = [
            task for task in self.pending_tasks if task.scheduled_time <= current_time
        ]
        ...
        for task in due_tasks:
            ...
            if result in [RotationResult.SUCCESS, RotationResult.ROLLBACK_SUCCESS]:
                ...
                result == RotationResult.FAILED and task.retry_count >= task.max_retries
            ):
                self.completed_tasks.append(task)
                self.pending_tasks.remove(task)
    ...
    async def emergency_rotate_all(
        self, reason: str, categories: Optional[List[SecretCategory]] = None
        ...
        emergency_secrets = [
            name
            for name, metadata in self.secret_manager.secret_definitions.items()
        ...
        for secret_name in emergency_secrets:
            try:
                ...
                if result != RotationResult.SUCCESS:
                    logger.error(
                        f"Emergency rotation failed for {secret_name}: {result}"
                    )
    ...
    async def get_rotation_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "next_rotation": min(
                [task.scheduled_time for task in self.pending_tasks], default=None
            ),
            "recent_failures": [
                task.secret_name
...
def create_rotation_manager(
    secret_manager: UnifiedSecretManager, config_path: Optional[str] = None
