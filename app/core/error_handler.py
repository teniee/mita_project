...
class ErrorHandler:
    ...
    def log_error(
        ...
        if request:
            try:
                ...
                context["request_error"] = (
                    f"Failed to serialize request context: {str(e)}"
                )
        ...
        if isinstance(
            ...
            logger.error(
                f"Server error {error_id}: {error}", extra=context, exc_info=True
            )
...
def validate_amount(
    amount: float, min_amount: float = 0.01, max_amount: float = 1000000
) -> float:
...
class InputValidator:
        ...
        if "date" in data:
            try:
                if isinstance(data["date"], str):
                    validated["date"] = datetime.fromisoformat(
    ...
    def validate_budget_data(data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if "monthly_income" in data:
            validated["monthly_income"] = validate_amount(
                data["monthly_income"], min_amount=0, max_amount=10000000
            )

        if "savings_target" in data:
            validated["savings_target"] = validate_amount(
                data["savings_target"], min_amount=0
            )

        if "categories" in data and isinstance(data["categories"], dict):
            validated_categories = {}
            for category, amount in data["categories"].items():
                validated_categories[validate_category(category)] = validate_amount(
                    amount, min_amount=0
                )
