...
class TaskErrorHandler:
    ...
    def handle_task_failure(
        ...
        try:
            ...
            logger.error(
                f"Error handler itself failed: {str(handler_error)}", exc_info=True
            )
            return {
    ...
    def get_error_statistics(self, time_window_hours: int = 24) -> Dict[str, Any]:
        ...
        try:
            ...
            for error in error_records:
                ...
                stats["errors_by_function"][func_name] = (
                    stats["errors_by_function"].get(func_name, 0) + 1
                )
                ...
                stats["errors_by_queue"][queue_name] = (
                    stats["errors_by_queue"].get(queue_name, 0) + 1
                )
        ...
        try:
            ...
            while True:
                cursor, keys = self.redis_conn.scan(
                    cursor=cursor, match="task_error:*", count=100
                )
                ...
                for key in keys:
                    try:
                        ...
                        logger.warning(
                            f"Failed to process error record {key}: {str(e)}"
                        )
    ...
    def _classify_error_severity(
        ...
        if any(
            ...
            for pattern in ["critical", "fatal", "system"]
        ):
            return ErrorSeverity.HIGH
    ...
    def _is_error_retryable(self, error_type: str, function_name: str) -> bool:
        ...
        if (
            retry_policy.non_retryable_errors
            and error_type in retry_policy.non_retryable_errors
        ):
            return False
        ...
        if (
            ...
            and error_type in retry_policy.retryable_errors
        ):
            return True
    ...
    def _should_retry_task(
        self, task_error: TaskError, retry_policy: RetryPolicy
    ) -> bool:
    ...
    def _calculate_retry_delay(
        ...
        if retry_policy.strategy == RetryStrategy.IMMEDIATE:
            ...
            delay = retry_policy.base_delay * (
                retry_policy.backoff_factor**task_error.retry_count
            )
    ...
    def _schedule_retry(
        self, job: Job, delay: int, retry_policy: RetryPolicy
    ) -> Dict[str, Any]:
    ...
    def _move_to_dead_letter_queue(
        self, task_error: TaskError, job: Job
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"moved_to_dlq": True, "dlq_type": dlq_type}
    ...
    def _update_error_statistics(self, task_error: TaskError) -> None:
        ...
        try:
            ...
            self.redis_conn.hincrby(
                f"error_stats:{today}", f"by_type:{task_error.error_type}", 1
            )

            # By function
            self.redis_conn.hincrby(
                f"error_stats:{today}", f"by_function:{task_error.function_name}", 1
            )

            # By severity
            self.redis_conn.hincrby(
                f"error_stats:{today}", f"by_severity:{task_error.severity.value}", 1
            )
    ...
    def _check_alert_conditions(self, task_error: TaskError) -> None:
        ...
        try:
            ...
            today = datetime.now(timezone.utc).date().isoformat()
            total_errors = int(
                self.redis_conn.hget(f"error_stats:{today}", "total_errors") or 0
            )

            if total_errors > 100:  # More than 100 errors in a day
                self._send_error_alert(
                    task_error, f"High error rate: {total_errors} errors today"
                )
                    ...
                    summaries.append(
                        {
                            ...
                            "function_name": job.func_name,
                            "failed_at": (
                                job.meta.get("failed_at") if job.meta else None
                            ),
                            "error_type": (
                                job.meta.get("error_type") if job.meta else None
                            ),
                            "retry_count": (
                                job.meta.get("retry_count", 0) if job.meta else 0
                            ),
                        }
                    )
                except Exception as e:
    ...
    def _calculate_error_trends(self, error_records: List[TaskError]) -> Dict[str, Any]:
        ...
        return {
            "trend": trend,
            "hourly_distribution": {
                h.isoformat(): count for h, count in hourly_counts.items()
            },
            "peak_hour": (
                max(hourly_counts, key=hourly_counts.get).isoformat()
                if hourly_counts
                else None
            ),
        }
