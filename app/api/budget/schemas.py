...
class BudgetRequest(BaseModel):
    ...
    user_answers: Dict[str, Any] = Field(
        ..., description="User questionnaire responses"
    )
    ...
    def validate_user_answers(cls, v):
        ...
        for key, value in v.items():
            if isinstance(value, str):
                sanitized_answers[key] = InputSanitizer.sanitize_string(
                    value, max_length=1000
                )
            elif isinstance(value, (int, float)) and "income" in key.lower():
                # Validate income-related fields
                sanitized_answers[key] = float(
                    InputSanitizer.sanitize_amount(
                        value,
...
class BudgetCategoryIn(BaseModel):
    ...
    period: str = Field(
        "monthly", pattern=r"^(weekly|monthly|yearly)$", description="Budget period"
    )
...
class BudgetAdjustmentIn(BaseModel):
    ...
    reason: Optional[str] = Field(
        None, max_length=500, description="Reason for adjustment"
    )
    effective_date: Optional[date] = Field(
        None, description="When adjustment takes effect"
    )
