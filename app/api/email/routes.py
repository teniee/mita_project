...
from app.services.email_service import EmailPriority, EmailType, get_email_service
...
class SendEmailRequest(BaseModel):
    ...
    to_email: str = Field(
        ..., min_length=5, max_length=255, description="Recipient email address"
    )
...
async def get_email_queue_status(
    ...
    try:
        ...
        return StandardizedResponse.success(
            message="Email queue status retrieved", data=queue_status
        )
...
async def get_email_job_status(
    ...
    try:
        ...
        if not job_status:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail="Email job not found"
            )

        return StandardizedResponse.success(
            message="Email job status retrieved", data=job_status
        )
...
async def test_email_template(
    ...
    try:
        ...
        subject, html_content = email_service.template_manager.render_template(
            email_type=email_type, variables=variables
        )
        ...
        await log_security_event_async(
            ...
            user_id=str(current_user.id),
            request=request,
            details={"email_type": email_type.value, "template_rendered": True},
        )
...
async def list_email_templates(
    ...
    try:
        ...
        return StandardizedResponse.success(
            ...
            data={"templates": template_info, "total_templates": len(templates)},
        )
