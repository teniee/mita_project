...
async def get_dashboard(
    ...
    try:
            ...
            transactions_data.append(
                {
                    ...
                    "icon": category_icons.get(
                        (txn.category or "other").lower(), "attach_money"
                    ),
                    "color": category_colors.get(
                        (txn.category or "other").lower(), "#9E9E9E"
                    ),
                }
            )
        ...
        if total_spent_this_month > 0:
            spending_rate = (
                (float(total_spent_this_month) / monthly_income) * 100
                if monthly_income > 0
                else 0
            )
                ...
                goals_data.append(
                    {
                        ...
                        "target_amount": (
                            float(goal.target_amount) if goal.target_amount else 0.0
                        ),
                        "saved_amount": (
                            float(goal.saved_amount) if goal.saved_amount else 0.0
                        ),
                        "progress": progress,
                        ...
                        "remaining_amount": float(goal.target_amount or 0)
        ...
        try:
            ...
            result = await db.execute(
                ...
                .order_by(ChallengeParticipation.progress_percentage.desc())
            ...
            logger.error(
                f"Error loading challenges for dashboard: {str(e)}", exc_info=True
            )
...
async def get_quick_stats(
    ...
    try:
        ...
        daily_average = (
            float(monthly_spending) / days_in_month if days_in_month > 0 else 0.0
        )
        ...
        top_category = {
            "name": (
                (top_category_result[0] or "other").title()
        ...
        try:
            ...
            if all_goals:
                goals_stats["total_target_amount"] = sum(
                    float(g.target_amount) if g.target_amount else 0.0
                    for g in all_goals
                )
                goals_stats["total_saved_amount"] = sum(
                    float(g.saved_amount) if g.saved_amount else 0.0 for g in all_goals
                )
                goals_stats["average_progress"] = sum(
                    float(g.progress or 0) for g in all_goals
                ) / len(all_goals)
        except Exception as e:
