...
class IncomeData(BaseModel):
    ...
    monthly_income: float = Field(
        ..., gt=0, description="Monthly income must be greater than 0"
    )
    additional_income: Optional[float] = Field(
        0, ge=0, description="Additional income (optional)"
    )
...
class SpendingHabits(BaseModel):
    ...
    dining_out_per_month: Optional[int] = Field(
        0, ge=0, description="Dining out frequency per month"
    )
    entertainment_per_month: Optional[int] = Field(
        0, ge=0, description="Entertainment frequency per month"
    )
    clothing_per_month: Optional[int] = Field(
        0, ge=0, description="Clothing shopping frequency per month"
    )
    travel_per_year: Optional[int] = Field(
        0, ge=0, description="Travel frequency per year"
    )
    coffee_per_week: Optional[int] = Field(
        0, ge=0, description="Coffee purchases per week"
    )
    transport_per_month: Optional[int] = Field(
        0, ge=0, description="Transportation frequency per month"
    )
    # Legacy fields for backward compatibility
    entertainment_budget: Optional[float] = Field(
        0, ge=0, description="Entertainment budget (deprecated)"
    )
    shopping_frequency: Optional[int] = Field(
        0, ge=0, description="Shopping frequency per month (deprecated)"
    )
...
class GoalsData(BaseModel):
    ...
    savings_goal_amount_per_month: Optional[float] = Field(
        0, ge=0, description="Monthly savings goal"
    )
    savings_goal_type: Optional[str] = Field(
        None, description="Primary savings goal type"
    )
    has_emergency_fund: Optional[bool] = Field(
        False, description="User has emergency fund goal"
    )
    all_goals: Optional[list] = Field(None, description="All selected financial goals")
    # Legacy fields for backward compatibility
    emergency_fund_target: Optional[float] = Field(
        0, ge=0, description="Emergency fund target (deprecated)"
    )
    debt_payoff_goal: Optional[float] = Field(
        0, ge=0, description="Debt payoff goal (deprecated)"
    )
...
class OnboardingSubmitRequest(BaseModel):
    ...
    fixed_expenses: Dict[str, float] = Field(
        ..., description="Fixed expenses by category"
    )
    spending_habits: Optional[Union[SpendingHabits, Dict]] = Field(
        None, description="Spending habits (optional)"
    )
    goals: Optional[Union[GoalsData, Dict]] = Field(
        None, description="Financial goals (optional)"
    )
    ...
    def validate_fixed_expenses(cls, v: Dict[str, float]) -> Dict[str, float]:
        ...
        for category, amount in v.items():
            ...
            if len(category) > 100:
                raise ValueError(
                    f"Category name '{category}' too long (max 100 characters)"
                )
            ...
            if amount > 1_000_000:
                raise ValueError(
                    f"Expense amount for '{category}' exceeds reasonable limit"
                )
