...
from app.core.feature_flags import FeatureFlagEnvironment, get_feature_flag_manager
...
async def get_all_feature_flags(
    environment: Optional[str] = None, user=Depends(get_current_user)
):
    """
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Error retrieving feature flags: {str(e)}"
        )


@router.get("/{flag_key}", response_model=Dict[str, Any])
async def get_feature_flag(flag_key: str, user=Depends(get_current_user)):
    """
...
async def set_feature_flag_value(
    flag_key: str, request: FeatureFlagValueRequest, user=Depends(get_current_user)
):
    """
    ...
    try:
        ...
        success = feature_manager.set_flag_value(
            flag_key, request.value, request.user_id
        )

        if not success:
            raise HTTPException(
                status_code=500, detail=f"Failed to update feature flag '{flag_key}'"
            )
...
async def check_feature_flag(
    flag_key: str, user_id: Optional[int] = None, user=Depends(get_current_user)
):
    """
