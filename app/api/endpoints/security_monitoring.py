...
@router.get("/metrics")
async def get_security_metrics(current_user: User = Depends(require_admin_access)):
    """
    ...
    try:
        ...
        log_security_event(
            ...
            {"admin_user_id": str(current_user.id), "error": str(e)},
        )
        raise HTTPException(
...
async def get_user_security_activity(
    ...
    try:
        ...
        log_security_event(
            "user_security_activity_accessed",
            {
                "admin_user_id": str(current_user.id),
...
async def get_blacklist_status(current_user: User = Depends(require_admin_access)):
    """
    ...
    try:
        ...
        try:
            ...
            tokens_revoked = await revoke_user_tokens(
                user_id=user_id, reason="admin", revoked_by=str(current_user.id)
            )

            # Log the successful admin action
            log_security_event(
                "admin_user_token_revocation",
                {
                    "admin_user_id": str(current_user.id),
            ...
            logger.critical(
                f"ADMIN ACTION: User {current_user.email} revoked {tokens_revoked} tokens for user {target_user.email}. Reason: {reason}"
            )
            ...
            log_security_event(
                "admin_user_token_revocation_failed",
                {
                    "admin_user_id": str(current_user.id),
                    "admin_email": current_user.email,
                    "target_user_id": user_id,
...
@router.get("/health")
async def security_system_health(current_user: User = Depends(require_admin_access)):
    """
    ...
    try:
        ...
        audit_summary = {
            ...
            "token_operations": {
                ...
                "verification_failures": metrics["token_metrics"][
            ...
            "security_alerts": {
                ...
                "critical_alerts": sum(
                    1 for a in time_filtered_alerts if a["severity"] == "CRITICAL"
                ),
                "high_alerts": sum(
                    1 for a in time_filtered_alerts if a["severity"] == "HIGH"
                ),
                "medium_alerts": sum(
                    1 for a in time_filtered_alerts if a["severity"] == "MEDIUM"
                ),
                "low_alerts": sum(
                    1 for a in time_filtered_alerts if a["severity"] == "LOW"
                ),
            },
            "system_performance": {
                "redis_operations": metrics.get("redis_metrics", {}),
                "active_monitored_users": metrics["suspicious_activity"][
                    "active_users"
                ],
