...
@router.get(
    "/health", response_model=DatabaseHealthResponse, summary="Database Health Check"
)
    ...
    try:
        ...
        return DatabaseHealthResponse(
            status=(
                ...
                else "degraded" if analysis["health_score"] >= 60 else "critical"
            ),
            health_score=analysis["health_score"],
            connection_pool_status=analysis["performance_stats"]
            .get("connection_metrics", {})
            .get("health_status", "unknown"),
            query_performance=(
                "good" if analysis["health_score"] >= 80 else "needs_attention"
            ),
...
async def get_performance_metrics(
    ...
    query_limit: int = Query(
        10, description="Number of queries to return", ge=1, le=50
    ),
    current_user: User = Depends(get_current_user),
    _: None = Depends(require_admin_access),
):
    ...
    try:
        ...
        response = {
            "performance_summary": {
                "total_queries_executed": stats.get("query_statistics", {}).get(
                    "total_queries", 0
                ),
                "unique_query_patterns": stats.get("query_statistics", {}).get(
                    "unique_query_patterns", 0
                ),
                "connection_pool_health": stats.get("connection_metrics", {}).get(
...
async def get_slow_queries(
    limit: int = Query(
        20, description="Number of slow queries to analyze", ge=1, le=100
    ),
    severity: Optional[str] = Query(
        None, description="Filter by severity: warning, critical, emergency"
    ),
    current_user: User = Depends(get_current_user),
    _: None = Depends(require_admin_access),
):
    ...
    try:
        ...
        if severity:
            ...
            slow_queries = [
                q for q in slow_queries if q["query"].get("severity") == severity_upper
            ]
        ...
        for query_analysis in slow_queries:
            ...
            analyzed_query = {
                ...
                "performance_impact": _assess_performance_impact(
        ...
        return {
            ...
            "global_recommendations": _generate_global_recommendations(
                analyzed_queries
            ),
            "generated_at": datetime.now().isoformat(),
        }
...
@router.get(
    "/connections",
    response_model=ConnectionPoolResponse,
...
async def get_optimization_report(
    ...
    try:
        ...
        report = {
            ...
            "detailed_analysis": {
                "query_performance": analysis["performance_stats"].get(
                    "query_statistics", {}
                ),
                "connection_efficiency": analysis["performance_stats"].get(
                    "connection_metrics", {}
                ),
                "resource_utilization": analysis["performance_stats"].get(
                    "system_resources", {}
                ),
...
async def optimize_query_performance(
    auto_apply: bool = Query(
        False, description="Automatically apply safe optimizations"
    ),
    current_user: User = Depends(get_current_user),
    _: None = Depends(require_admin_access),
):
    ...
    try:
        ...
        optimizations = {
            ...
            "applied_optimizations": (
                [] if not auto_apply else ["Example: Added index on user_id column"]
            ),
        }
...
def _generate_global_recommendations(queries: List[Dict]) -> List[str]:
    ...
    if len(queries) > 10:
        recommendations.append(
            "High number of slow queries detected - consider comprehensive database optimization"
        )

    critical_queries = [q for q in queries if q.get("severity") == "emergency"]
    if critical_queries:
        recommendations.append(
            f"{len(critical_queries)} emergency-level slow queries require immediate attention"
        )
    ...
    if len(select_queries) > len(queries) * 0.8:
        recommendations.append(
            "Most slow queries are SELECT statements - consider adding database indexes"
        )
...
def _count_critical_issues(analysis: Dict) -> int:
    ...
    critical_queries = [
        q for q in slow_queries if q["query"].get("severity") == "emergency"
    ]
...
def _get_immediate_actions(analysis: Dict) -> List[str]:
    ...
    if analysis["health_score"] < 60:
        actions.append(
            "Investigate and resolve critical performance issues immediately"
        )

    emergency_queries = [
        q
        for q in analysis.get("slow_query_analysis", [])
        if q["query"].get("severity") == "emergency"
    ]

    if emergency_queries:
        actions.append(
            f"Optimize {len(emergency_queries)} emergency-level slow queries"
        )
...
def _get_short_term_improvements(analysis: Dict) -> List[str]:
    ...
    connection_health = (
        analysis["performance_stats"].get("connection_metrics", {}).get("health_status")
    )
