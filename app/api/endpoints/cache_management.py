...
@router.get(
    "/health", response_model=CacheHealthResponse, summary="Cache System Health Check"
)
    ...
    try:
        ...
        performance_metrics = {
            ...
            "performance_indicators": {
                "cache_effectiveness": (
                    "high"
                    if hit_ratios["total"] > 70
                    else "medium" if hit_ratios["total"] > 40 else "low"
                ),
                "memory_pressure": (
                    "high"
                    if stats["memory_cache"].get("utilization", 0) > 80
                    else "normal"
                ),
                "redis_health": (
                    "healthy"
                    if stats["redis_cache"].get("connected", False)
                    else "degraded"
                ),
            },
        }
        ...
        if include_detailed:
            ...
            performance_metrics["optimization_suggestions"] = (
                _generate_optimization_suggestions(stats, hit_ratios)
            )
...
async def clear_all_cache(
    confirm: bool = Query(
        False, description="Confirmation required to clear all cache"
    ),
    current_user: User = Depends(get_current_user),
    _: None = Depends(require_admin_access),
):
    ...
    try:
        ...
        return {
            ...
            "analysis": {
                ...
                "avg_access_count": (
                    sum(entry["access_count"] for entry in top_keys) / len(top_keys)
                    if top_keys
                    else 0
                ),
            },
    ...
    try:
        ...
        report = {
            ...
            "optimization_recommendations": {
                ...
                "architectural_improvements": _get_architectural_recommendations(
                    stats, hit_ratios
                ),
            },
            "capacity_planning": {
...
def _generate_optimization_suggestions(
    stats: Dict[str, Any], hit_ratios: Dict[str, float]
) -> List[str]:
    ...
    if hit_ratios["memory"] < 30:
        suggestions.append(
            "Optimize memory cache configuration for better L1 performance"
        )
...
def _get_immediate_optimization_actions(
    stats: Dict[str, Any], hit_ratios: Dict[str, float]
) -> List[str]:
...
def _get_architectural_recommendations(
    stats: Dict[str, Any], hit_ratios: Dict[str, float]
) -> List[str]:
