...
async def task_system_health():
    ...
    try:
        ...
        overall_health = _determine_overall_health(
            health_metrics, redis_health, worker_health
        )
...
async def task_system_readiness():
    ...
    try:
        ...
        workers_available = (
            len([w for w in get_task_queue().redis_conn.smembers("rq:workers")]) > 0
        )
...
def _test_redis_connectivity() -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            "status": (
                "healthy"
                if ping_result and read_value == b"test_value"
                else "unhealthy"
            ),
    ...
    try:
        ...
        return {
            ...
            "utilization_percentage": round(
                (active_workers / max(total_workers, 1)) * 100, 1
            ),
        }
...
def _determine_overall_health(
    ...
    if system_load["status"] == "high":
        ...
        recommendations.append(
            "Monitor queue processing and consider adding more workers"
        )
