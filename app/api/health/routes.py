...
async def get_system_health():
    ...
    try:
        ...
        try:
            ...
            services_health["gpt_service"] = {"status": "error", "message": str(e)}
...
async def reset_circuit_breaker(
    ...
    try:
        ...
        logger.error(
            f"Error resetting circuit breaker {service_name}: {str(e)}", exc_info=True
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def test_external_connections():
    ...
    return {
        ...
        "summary": {
            ...
            "connectivity_rate": (
                (connected_services / total_services * 100) if total_services > 0 else 0
            ),
            "overall_status": (
                "healthy"
                if connected_services == total_services
                else "degraded" if connected_services > 0 else "unhealthy"
            ),
        },
    }
