...
async def check_specific_service(service_name: str):
    ...
    try:
        ...
        service_status = (
            "healthy" if connected else "unhealthy" if service.enabled else "disabled"
        )
...
async def test_service_connection(
    service_name: str, admin_user=Depends(require_admin_role)
):
    ...
    try:
        ...
        services_score = (
            (
                ...
                / services_health["enabled_services"]
                * 100
            )
            if services_health["enabled_services"] > 0
            else 0
        )
        api_keys_score = (
            ((api_keys_health["active_keys"] / api_keys_health["total_keys"]) * 100)
            if api_keys_health["total_keys"] > 0
            else 100
        )
        critical_services_score = (
            (sum(critical_services.values()) / len(critical_services) * 100)
            if critical_services
            else 0
        )

        overall_health_score = (
            services_score + api_keys_score + critical_services_score
        ) / 3
        ...
        return {
            ...
            "alerts": _generate_alerts(
                services_health, api_keys_health, critical_services
            ),
...
def _generate_service_recommendations(results: Dict[str, Any]) -> List[str]:
    ...
    for service_name, result in results.items():
        ...
        elif not result.get("enabled") and service_name in [
            "openai",
            "sentry",
            "redis",
        ]:
            recommendations.append(
                f"Consider enabling {service_name} service for full functionality"
            )
...
def _generate_api_key_recommendations(results: Dict[str, Any]) -> List[str]:
    ...
    if missing_critical:
        recommendations.append(
            f"URGENT: Fix critical API keys: {', '.join(missing_critical)}"
        )
...
def _generate_alerts(
    services_health: Dict, api_keys_health: Dict, critical_services: Dict
) -> List[Dict[str, Any]]:
    ...
    unhealthy_critical = [
        name for name, healthy in critical_services.items() if not healthy
    ]
