...
from app.core.middleware_health_monitor import HealthStatus, middleware_health_monitor
...
async def get_comprehensive_health(session: AsyncSession = Depends(get_async_db)):
    ...
    try:
        ...
        middleware_report = (
            await middleware_health_monitor.run_comprehensive_middleware_health_check(
                session
            )
        )
        ...
        try:
            ...
            circuit_breaker_health = {"overall_health": "degraded", "error": str(e)}
        ...
        try:
            if hasattr(settings, "OPENAI_API_KEY") and settings.OPENAI_API_KEY:
                ...
                external_services_health["gpt_service"] = (
                    await gpt_service.get_service_health()
                )
            else:
                external_services_health["gpt_service"] = {
        ...
        try:
            ...
            external_services_health["google_auth_service"] = (
                await google_auth_service.get_service_health()
            )
        except Exception as e:
        ...
        response = {
            ...
            "message": _get_comprehensive_health_message(
                overall_status, middleware_report
            ),
        }
...
async def get_middleware_health(session: AsyncSession = Depends(get_async_db)):
    ...
    try:
        ...
        if middleware_report.overall_status in [
...
async def get_component_health(
    component: str, session: AsyncSession = Depends(get_async_db)
):
    ...
    try:
        ...
        logger.error(
            f"Error checking component {component} health: {str(e)}", exc_info=True
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail={
    ...
    try:
            ...
            if metric.response_time_ms:
                ...
                if metric.response_time_ms > 5000:  # 5 seconds
                    timeout_risks.append(
                        f"{name}: {metric.response_time_ms:.1f}ms (CRITICAL - timeout risk)"
                    )
                elif metric.response_time_ms > 2000:  # 2 seconds
                    timeout_risks.append(
                        f"{name}: {metric.response_time_ms:.1f}ms (WARNING - approaching timeout)"
                    )
        ...
        response = {
            ...
            "alerts": [
                ...
                if "timeout" in alert.lower() or "slow" in alert.lower()
            ],
            "recommendations": [
                rec
                for rec in middleware_report.recommendations
                if "performance" in rec.lower() or "timeout" in rec.lower()
            ],
            "message": _get_performance_message(
...
async def get_health_history(
    ...
    try:
        ...
        recent_statuses = (
            [report.overall_status.value for report in history[-10:]] if history else []
        )
        recent_response_times = (
            [report.response_time_ms for report in history[-10:]] if history else []
        )
        ...
        analysis = {
            ...
            "recent_max_response_time_ms": (
                max(recent_response_times) if recent_response_times else 0
            ),
            "health_stability": (
                "stable" if len(set(recent_statuses)) <= 1 else "unstable"
            ),
            "performance_trend": (
                "improving"
                if recent_response_times
                and recent_response_times[0] > recent_response_times[-1]
                else (
                    "degrading"
                    if recent_response_times
                    and recent_response_times[0] < recent_response_times[-1]
            ...
            formatted_history.append(
                {
                    ...
                    "healthy_components": report.performance_summary.get(
                        "healthy_components", 0
                    ),
                    "total_components": report.performance_summary.get(
        ...
        return {
            ...
            "monitoring_duration": {
                ...
                "duration_hours": (
                    datetime.now(timezone.utc) - middleware_health_monitor.start_time
                ).total_seconds()
                / 3600,
            },
        }
...
async def get_health_metrics(session: AsyncSession = Depends(get_async_db)):
    ...
    try:
        ...
        for name, metric in middleware_report.metrics.items():
            ...
            metrics.append(
                f'mita_middleware_component_health{{component="{name}"}} {component_value}'
            )

            # Response time metrics
            if metric.response_time_ms is not None:
                metrics.append(
                    f'mita_middleware_component_response_time_ms{{component="{name}"}} {metric.response_time_ms}'
                )

            # Value metrics (if available)
            if metric.value is not None:
                metrics.append(
                    f'mita_middleware_component_value{{component="{name}"}} {metric.value}'
                )
        ...
        if "healthy_components" in perf:
            metrics.append(
                f"mita_middleware_healthy_components {perf['healthy_components']}"
            )
        if "total_services" in perf:
            metrics.append(
                f"mita_middleware_total_components {perf['components_checked']}"
            )

        # Alert metrics
        metrics.append(f"mita_middleware_alerts_count {len(middleware_report.alerts)}")
        metrics.append(
            f"mita_middleware_issues_count {len(middleware_report.issues_detected)}"
        )

        # Overall response time
        metrics.append(
            f"mita_middleware_health_check_duration_ms {middleware_report.response_time_ms}"
        )
...
def _get_performance_message(
    performance_status: str, performance_summary: Dict[str, Any]
) -> str:
