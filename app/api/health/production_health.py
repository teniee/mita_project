...
class ProductionHealthChecker:
    ...
    async def check_database_health(self, session: AsyncSession) -> Dict[str, Any]:
        ...
        try:
            ...
            long_transactions_result = await session.execute(
                text(
                    """
    ...
    async def run_comprehensive_health_check(
        ...
        try:
            ...
            health_report = {
                ...
                "message": self._get_overall_health_message(
    ...
    def _get_overall_health_message(
        self, status: str, healthy_count: int, total_count: int
    ) -> str:
        ...
        if status == "healthy":
            ...
            return (
                f"System is unhealthy, {total_count - healthy_count} services are down"
            )
...
async def production_health_check(session: AsyncSession = Depends(get_async_session)):
    ...
    try:
        ...
        if health_report["overall_status"] == "unhealthy":
            raise HTTPException(
                status_code=status.HTTP_503_SERVICE_UNAVAILABLE, detail=health_report
            )
...
async def get_health_history():
    ...
    return {
        ...
        "oldest_check": (
            health_checker.health_history[0]["timestamp"]
            if health_checker.health_history
            else None
        ),
        "newest_check": (
            health_checker.health_history[-1]["timestamp"]
            if health_checker.health_history
            else None
        ),
    }
...
async def prometheus_metrics(session: AsyncSession = Depends(get_async_session)):
    ...
    try:
        ...
        for service_name, service_data in health_report.get("services", {}).items():
            ...
            metrics.append(
                f'mita_service_health{{service="{service_name}"}} {service_value}'
            )

            # Response time metrics
            if (
                "response_time_ms" in service_data
                and service_data["response_time_ms"] is not None
            ):
                metrics.append(
                    f'mita_service_response_time_ms{{service="{service_name}"}} {service_data["response_time_ms"]}'
                )
