...
async def complete_habit(
    ...
    result = await db.execute(
        select(HabitCompletion).filter(
            ...
            >= completion_date.replace(hour=0, minute=0, second=0),
            HabitCompletion.completed_at
            < completion_date.replace(hour=23, minute=59, second=59),
        )
    )
    ...
    completion = HabitCompletion(
        habit_id=habit_id, user_id=user.id, completed_at=completion_date
    )
    db.add(completion)
    await db.commit()
