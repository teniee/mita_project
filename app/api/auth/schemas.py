...
class FastRegisterIn(BaseModel):
    ...
    annual_income: Optional[float] = Field(
        0.0, ge=0, le=10000000, description="Annual income"
    )
    timezone: str = Field(
        "UTC", min_length=1, max_length=50, description="User timezone"
    )
...
class RegisterIn(BaseModel):
    ...
    country: str = Field(
        "US", min_length=2, max_length=2, description="Country code (US only)"
    )
    ...
    timezone: str = Field(
        "UTC", min_length=3, max_length=50, description="User timezone"
    )
...
class GoogleAuthIn(BaseModel):
    ...
    id_token: str = Field(
        ..., min_length=10, max_length=2048, description="Google ID token"
    )
