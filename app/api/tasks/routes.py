...
def require_admin_access(user, feature_flag: str = "admin_endpoints_enabled"):
    ...
    if not is_feature_enabled(feature_flag, getattr(user, "id", None), default=True):
        raise HTTPException(
            status_code=403, detail="Admin endpoints are currently disabled"
        )

    # Check if user has admin role
    if not hasattr(user, "is_admin") or not user.is_admin:
        raise HTTPException(
            status_code=403, detail="Admin access required for this operation"
        )


@router.get("/{task_id}", response_model=TaskResponse)
async def get_task_status(task_id: str, user=current_user_dep):
    """
...
async def cancel_task(task_id: str, user=current_user_dep):
    """
...
async def retry_task(task_id: str, user=current_user_dep):
    """
    Retry a failed task.
...
async def submit_notification_task(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to submit notification task: {str(e)}"
        )
...
async def submit_data_export_task(
    ...
    try:
        if request.export_format not in ["json", "csv"]:
            raise HTTPException(
                status_code=400, detail="Export format must be 'json' or 'csv'"
            )
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to submit data export task: {str(e)}"
        )
...
async def submit_ai_analysis_task(
    ...
    try:
        task_info = task_manager.submit_ai_analysis_task(
            user_id=user.id, year=request.year, month=request.month
        )
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to submit AI analysis task: {str(e)}"
        )
...
async def submit_daily_advice_batch(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to submit daily advice batch: {str(e)}"
        )
...
async def submit_cleanup_batch(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to submit cleanup batch: {str(e)}"
        )
