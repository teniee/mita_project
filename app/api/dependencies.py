...
from app.services.auth_jwt_service import TokenScope, verify_token
...
async def get_current_user(
    ...
    try:
        ...
        try:
            ...
            logger.debug(
                f"verify_token returned: {'None' if payload is None else 'valid payload'}"
            )
        except (
            InvalidTokenError,
            ExpiredSignatureError,
            ...
            logger.warning(
                f"Auth: token verification system error: {type(verify_error).__name__}",
                exc_info=True,
            )
            try:
                log_security_event(
        ...
        try:
            ...
            if user:
                ...
                _ = user.is_premium if hasattr(user, "is_premium") else False
                logger.debug(f"User {user_id} attributes preloaded")
        except Exception as db_error:
            # Database errors during user lookup are system errors (500)
            logger.error(
                f"Auth: database error during user lookup: {db_error}", exc_info=True
            )
            try:
                log_security_event(
                    "database_error",
...
async def require_premium_user(user: User = Depends(get_current_user)) -> User:
    ...
    log_security_event(
        ...
        {"user_id": str(user.id), "token_scopes": token_scopes},
    )
...
oauth2_refresh_scheme = OAuth2PasswordBearer(
    tokenUrl="/api/auth/refresh", auto_error=False
)
...
def require_scope_access(required_scope: str):
    ...
    async def scope_check(user: User = Depends(get_current_user)) -> User:
        ...
        if (
            required_scope not in token_scopes
            and TokenScope.ADMIN_SYSTEM.value not in token_scopes
        ):
            log_security_event(
                "scope_access_denied",
                {
