...
from app.api.ocr.schemas import CategorizeReceiptRequest
...
def validate_image_file(file: UploadFile) -> None:
    ...
    if not file.filename:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail={"error": "File has no name", "code": "INVALID_FILE"},
        )
...
def categorize_receipt_data(
    ...
    try:
        ...
        category = categorization_service.categorize(
            merchant=merchant, amount=amount, items=items, hint=hint
        )
...
def get_ocr_job_status(
    ...
    try:
        ...
        ocr_job = (
            db.query(OCRJob)
            .filter(and_(OCRJob.job_id == ocr_job_id, OCRJob.user_id == user.id))
            .first()
        )
        ...
        return success_response(
            {
                ...
                "created_at": (
                    ocr_job.created_at.isoformat() if ocr_job.created_at else None
                ),
                "completed_at": (
                    ocr_job.completed_at.isoformat() if ocr_job.completed_at else None
                ),
                "result": result_data,
        ...
        return success_response(
            {"job_id": ocr_job_id, "status": "error", "progress": 0, "error": str(e)}
        )
...
def get_receipt_image(
    ...
    try:
        ...
        ocr_job = (
            db.query(OCRJob)
            .filter(and_(OCRJob.job_id == job_id, OCRJob.user_id == user.id))
            .first()
        )
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to retrieve receipt image: {str(e)}"
        )
...
def delete_receipt_image(
    ...
    try:
        ...
        if not ocr_job:
            raise HTTPException(
                status_code=404, detail="Receipt not found or does not belong to user"
            )
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to delete receipt image: {str(e)}"
        )
