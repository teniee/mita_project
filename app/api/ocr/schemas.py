...
class CategorizeReceiptRequest(BaseModel):
    ...
    total: Optional[float] = Field(
        None, ge=0, description="Total amount (alternative field)"
    )
    items: Optional[List[Dict[str, Any]]] = Field(
        default_factory=list, description="List of items on receipt"
    )
...
class ConfidenceScores(BaseModel):
    ...
    merchant: Optional[float] = Field(
        None, ge=0, le=1, description="Merchant detection confidence"
    )
    amount: Optional[float] = Field(
        None, ge=0, le=1, description="Amount detection confidence"
    )
    date: Optional[float] = Field(
        None, ge=0, le=1, description="Date detection confidence"
    )
    items: Optional[float] = Field(
        None, ge=0, le=1, description="Items detection confidence"
    )
...
class OCRResultData(BaseModel):
    ...
    confidence_scores: Optional[Dict[str, float]] = Field(
        None, description="Field-specific confidence scores"
    )
    fields_needing_review: List[str] = Field(
        default_factory=list, description="Fields requiring manual review"
    )
...
class OCRJobStatusResponse(BaseModel):
    ...
    result: Optional[OCRResultData] = Field(
...
class EnhanceImageResponse(BaseModel):
    ...
    enhancements_applied: List[str] = Field(
        ..., description="List of applied enhancements"
    )
