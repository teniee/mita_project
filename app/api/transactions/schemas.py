...
class TxnIn(BaseModel):
    ...
    goal_id: Optional[str] = Field(
        None, description="Link transaction to a savings goal (UUID)"
    )
    ...
    def validate_receipt_url(cls, v):
        ...
        if not re.match(r"^https?://[^\s]+\.(jpg|jpeg|png|pdf)$", v.lower()):
            raise ValueError(
                "Invalid receipt URL format (must be HTTPS with image/PDF extension)"
            )
...
class BulkTxnIn(BaseModel):
    ...
    source: Optional[str] = Field(
        None, max_length=50, description="Import source (csv, ocr, etc.)"
    )
    ...
    def validate_transactions(cls, v):
        ...
        for txn in v:
            ...
            txn_hash = (
                txn.amount,
                txn.category,
                txn.spent_at.date() if txn.spent_at else None,
            )
            if txn_hash in seen:
