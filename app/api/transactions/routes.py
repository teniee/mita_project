...
async def create_transaction_standardized(
    ...
    validated_amount = validate_amount(txn.amount, min_amount=0.01, max_amount=100000.0)
    ...
    if txn.category not in valid_categories:
        raise ValidationError(
            ...
            details={
                "provided_category": txn.category,
    ...
    try:
        ...
        if not result:
            raise BusinessLogicError(
                "Failed to create transaction", ErrorCode.BUSINESS_INVALID_OPERATION
            )
...
async def get_transactions_standardized(
    ...
    try:
        ...
        return success_response(
            data=transactions, message="Transactions retrieved successfully"
        )
    except Exception as e:
...
async def process_receipt(
    ...
    try:
        ...
        is_premium = hasattr(user, "premium_until") and getattr(
        ...
        task_info = task_manager.submit_ocr_task(
            user_id=user.id, image_path=temp.name, is_premium_user=is_premium
        )
...
async def get_merchant_suggestions(
    ...
    query_stmt = (
        query_stmt.group_by(Transaction.description, Transaction.category)
    ...
    return success_response({"suggestions": suggestions, "query": query})
...
async def get_receipt_image(
    ...
    result = await db.execute(
        select(OCRJob).where(
            and_(OCRJob.job_id == receipt_id, OCRJob.user_id == user.id)
        )
    )
...
async def process_receipts_batch(
    ...
    for idx, file in enumerate(files):
        ...
        try:
            ...
            receipt_ids.append(receipt_id)
            results.append(
                {"receipt_id": receipt_id, "status": "failed", "error": str(e)}
            )
        finally:
            # Cleanup
            try:
...
async def get_receipt_processing_status(
    ...
    if job_id.startswith("batch_"):
        ...
        return success_response(
            {
                "job_id": job_id,
                "status": (
                    "completed" if completed_count == len(ocr_jobs) else "processing"
                ),
                "progress": progress,
                "results": results,
                "errors": errors,
        ...
        result = await db.execute(
            select(OCRJob).where(
                and_(OCRJob.job_id == job_id, OCRJob.user_id == user.id)
            )
        )
        ocr_job = result.scalar_one_or_none()
                ...
                "application/json": {
                    "example": {
                        ...
                        "data": {
                            ...
                            "suggestions": [
                                "This will leave only $5.50 for food today"
                            ],
                        },
                    }
                }
