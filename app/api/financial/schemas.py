...
from app.core.validators import FinancialConstants, InputSanitizer
...
class InstallmentEvalRequest(BaseModel):
    ...
    months: int = Field(
        ..., ge=1, le=360, description="Number of months for installment (max 30 years)"
    )
    down_payment: Optional[condecimal(max_digits=12, decimal_places=2, ge=0)] = Field(
        None, description="Down payment amount"
    )
    interest_rate: Optional[condecimal(max_digits=5, decimal_places=4, ge=0, le=1)] = (
        Field(None, description="Annual interest rate (as decimal, e.g., 0.05 for 5%)")
    )
    current_monthly_income: Optional[
        condecimal(max_digits=12, decimal_places=2, ge=0)
    ] = Field(None, description="Current monthly income for affordability calculation")
    existing_monthly_debt: Optional[
        condecimal(max_digits=12, decimal_places=2, ge=0)
    ] = Field(None, description="Existing monthly debt payments")
    ...
    def validate_interest_rate(cls, v):
        ...
        if v > 1:
            raise ValueError(
                "Interest rate must be expressed as decimal (e.g., 0.05 for 5%)"
            )
...
class GoalIn(BaseModel):
    ...
    currency: Optional[str] = Field(
        "USD", min_length=3, max_length=3, description="Currency code"
    )
