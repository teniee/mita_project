...
@router.post(
    "/installment-evaluate",
    response_model=InstallmentEvalResult,
...
def installment_check_standardized(
    ...
    validated_price = validate_amount(
        payload.price, min_amount=1.0, max_amount=1000000.0
    )
    ...
    if not isinstance(payload.months, int) or payload.months < 1 or payload.months > 60:
        raise ValidationError(
            ...
            details={
                "min_months": 1,
                "max_months": 60,
                "provided_months": payload.months,
            },
        )
        ...
        result.update(
            {
                ...
                "budget_impact_assessment": (
                    "within_limits"
                    if result.get("can_afford", False)
                    else "exceeds_limits"
                ),
            }
        )
...
def get_personalized_thresholds(
    ...
    thresholds = get_dynamic_thresholds(
        threshold_type_mapping[threshold_type], user_context
    )
