...
def get_monthly_analytics(user_id: str, db: Session = Depends(get_db)):
    ...
    return [
        AnalyticsCategory(name=k, amount=v, percentage=v / total_spent if total_spent else 0)
        for k, v in totals.items()
    ]

@router.get("/trend/{user_id}", response_model=List[TrendPoint])
def get_trend(user_id: str, db: Session = Depends(get_db)):
