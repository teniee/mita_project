...
def get_behavioral_allocation(
    ...
    cooldown_thresholds = get_dynamic_thresholds(
        ThresholdType.COOLDOWN_PERIOD, user_context
    )
    ...
    for category, total in budget_plan.items():
        ...
        cooldown = cooldown_thresholds.get(
            category, cooldown_thresholds.get(category.replace(" ", "_"), 0)
        )

        # Use dynamic time bias based on user context
        bias = time_bias_thresholds.get(
            category, time_bias_thresholds.get(category.replace(" ", "_"), [1.0] * 7)
        )
        slots = []
...
def allocate_behavioral_budget(user_id: int, total_budget: float, db: Session) -> dict:
    ...
    try:
        income_tier: str = (
            IncomeClassificationService().classify_income(monthly_income, region).value
        )
    except Exception:
        income_tier = "unknown"
    ...
    return {
        ...
        "allocation_method": (
            "dynamic" if user_context_applied else "hardcoded_fallback"
        ),
        "income_tier": income_tier,
