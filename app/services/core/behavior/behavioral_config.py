...
def _convert_daily_to_time_of_day(
    daily_patterns: Dict[str, List[float]]
) -> Dict[str, Dict[str, float]]:
    ...
    for category, daily_pattern in daily_patterns.items():
        if category in ["coffee"]:
            ...
            time_of_day_patterns[category] = {"afternoon": 0.5, "evening": 0.5}
        elif category in ["transport"]:
            # Transport patterns based on commuting
            time_of_day_patterns[category] = {"morning": 0.4, "evening": 0.6}
