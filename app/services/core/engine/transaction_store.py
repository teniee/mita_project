...
def create_transaction(user_id: str, date: str, tx_type: str, amount: float, category: str, calendar: Dict[str, Dict]) -> Dict:
...
def apply_transaction(user_id: str, date: str, tx_type: str, amount: float, category: str, calendar: Dict[str, Dict]) -> None:
    if date not in calendar:
        calendar[date] = {}

    if tx_type == "expense":
        calendar[date].setdefault("actual_spending", {})
        calendar[date]["actual_spending"][category] = calendar[date]["actual_spending"].get(category, 0.0) + amount
        ...
        CATEGORY_SPENT_TOTAL[user_id][category] = CATEGORY_SPENT_TOTAL[user_id].get(category, 0.0) + amount
