...
class IncomeClassificationService:
            ...
            for profile, name in [(us_profile, "US"), (ca_profile, "US-CA")]:
                ...
                for key in required_keys:
                    if key not in thresholds:
                        ...
                        raise ValueError(
                            f"Invalid income classification config for {name}"
                        )
    ...
    def get_tier_display_info(
        self, tier: IncomeTier, region: str = "US"
    ) -> Dict[str, str]:
        """
    ...
    def is_near_tier_boundary(
        self, monthly_income: float, region: str = "US"
    ) -> Tuple[bool, Optional[IncomeTier]]:
        """
    ...
    def validate_classification_consistency(self, profile: Dict) -> bool:
        ...
        try:
            ...
            if tier == IncomeTier.LOWER_MIDDLE and (
                annual_income <= thresholds["low"]
                or annual_income > thresholds["lower_middle"]
            ):
                return False
            if tier == IncomeTier.MIDDLE and (
                annual_income <= thresholds["lower_middle"]
                or annual_income > thresholds["middle"]
            ):
                return False
            if tier == IncomeTier.UPPER_MIDDLE and (
                annual_income <= thresholds["middle"]
                or annual_income > thresholds["upper_middle"]
            ):
                return False
