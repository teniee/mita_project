...
class IncomeScalingAlgorithms:
    ...
    def scale_housing_ratio(
        ...
        elasticity_adjustment = self._safe_elasticity(
            income_ratio, params.elasticity_coefficient
        )
        ...
        if annual_income > params.diminishing_returns_point:
            ...
            diminishing_factor = (
                1 - (excess_income / 500000) * 0.1
            )  # 10% reduction per 500k
        ...
        # Apply bounds
        final_ratio = max(
            params.minimum_threshold, min(params.maximum_threshold, regional_adjusted)
        )

        logger.debug(
            f"Housing ratio calculation: {monthly_income:.0f}/mo -> {final_ratio:.2%}"
        )
        return final_ratio
    ...
    def scale_food_ratio(
        ...
        logger.debug(
            f"Food ratio calculation: {monthly_income:.0f}/mo -> {final_ratio:.2%}"
        )
        return final_ratio
    ...
    def scale_savings_rate_target(
        ...
        if annual_income > params.diminishing_returns_point:
            diminishing_factor = (
                1 - math.log(annual_income / params.diminishing_returns_point) * 0.05
            )
            scaled_rate *= max(0.8, diminishing_factor)

        # Apply bounds
        final_rate = max(
            params.minimum_threshold, min(params.maximum_threshold, scaled_rate)
        )

        logger.debug(
            f"Savings rate calculation: {monthly_income:.0f}/mo, age {age} -> {final_rate:.2%}"
        )
        return final_rate
    ...
    def scale_small_purchase_threshold(
        ...
        logger.debug(
            f"Small purchase threshold: {monthly_income:.0f}/mo -> ${final_threshold:.2f}"
        )
    ...
    def scale_emergency_fund_months(
        ...
        # Apply bounds
        final_months = max(
            params.minimum_threshold, min(params.maximum_threshold, scaled_months)
        )

        logger.debug(
            f"Emergency fund target: {monthly_income:.0f}/mo -> {final_months:.1f} months"
        )
        return final_months
    ...
    def scale_investment_capacity(
        ...
        available_capacity = max(
            0, 1.0 - existing_savings_rate - 0.6
        )  # Leave 60% for living expenses
        scaled_capacity = min(scaled_capacity, available_capacity)

        # Apply bounds
        final_capacity = max(
            params.minimum_threshold, min(params.maximum_threshold, scaled_capacity)
        )

        logger.debug(
            f"Investment capacity: {monthly_income:.0f}/mo -> {final_capacity:.2%}"
        )
        return final_capacity

    def scale_category_variance_thresholds(
        self, monthly_income: float, spending_tier: IncomeTier, months_of_data: int = 6
    ) -> Dict[str, float]:
        """
    ...
    def scale_goal_timeline_constraints(
        ...
        if monthly_income <= 0:
            raise ValueError(
                "monthly_income must be positive to compute goal constraints"
            )
        ...
        if minimum_timeline_months > maximum_timeline_months:
            ...
            max_monthly_contribution = min(
                required_monthly * 1.1, affordability_ceiling
            )
            minimum_timeline_months = max(1, goal_amount / max_monthly_contribution)
            if minimum_timeline_months > maximum_timeline_months:
    ...
    def calculate_income_elasticity(
        ...
        if (
            params.minimum_threshold is not None
            and params.maximum_threshold is not None
        ):
            scaled_value = max(
                params.minimum_threshold, min(params.maximum_threshold, scaled_value)
            )

        logger.debug(
            f"Elasticity scaling for {threshold_type}: {base_value:.3f} -> {scaled_value:.3f}"
        )
        return scaled_value
...
def scale_threshold_by_income(
    threshold_type: str, monthly_income: float, **kwargs
) -> float:
    """
...
def get_scaled_variance_thresholds(
    monthly_income: float, months_of_data: int = 6
) -> Dict[str, float]:
...
def get_scaled_goal_constraints(
    monthly_income: float, age: int, goal_amount: float, goal_category: str = "general"
) -> Dict[str, float]:
