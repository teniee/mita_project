...
class PerformanceMonitorService:
    ...
    async def monitor_api_request(
        ...
        transaction = sentry_sdk.start_transaction(
            op="http.server", name=operation_name, sampled=True
        )
    ...
    async def monitor_financial_operation(
        ...
        transaction = sentry_sdk.start_transaction(
            op="financial.operation", name=operation_name, sampled=True
        )
    ...
    async def _record_metric(self, metric: PerformanceMetric):
        ...
        # Log slow operations
        if (
            metric.category == PerformanceCategory.API_REQUEST
            and metric.duration_ms > 1000
        ):
            logger.warning(
                f"Slow API request: {metric.operation_name} took {metric.duration_ms:.2f}ms"
            )
        elif (
            metric.category == PerformanceCategory.DATABASE_QUERY
            and metric.duration_ms > 500
        ):
            logger.warning(
                f"Slow database query: {metric.operation_name} took {metric.duration_ms:.2f}ms"
            )
        elif (
            metric.category == PerformanceCategory.FINANCIAL_CALCULATION
            and metric.duration_ms > 2000
        ):
            logger.warning(
    ...
    def get_performance_summary(
        self, last_hours: int = 1, category: Optional[PerformanceCategory] = None
    ) -> Dict[str, Any]:
    ...
    def _get_slowest_operations(
        self, metrics: List[PerformanceMetric], limit: int = 10
    ) -> List[Dict[str, Any]]:
...
async def monitor_api_request(
    request: Request, user_id: Optional[str] = None, **kwargs
):
...
async def monitor_financial_operation(operation_name: str, user_id: str, **kwargs):
    ...
    return performance_monitor.monitor_financial_operation(
        operation_name, user_id, **kwargs
    )
