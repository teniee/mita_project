...
class EmailTemplateManager:
    ...
    def render_template(
        self, email_type: EmailType, variables: Dict[str, Any]
    ) -> Tuple[str, str]:
        ...
        if template_config.required_vars:
            missing_vars = [
                var for var in template_config.required_vars if var not in variables
            ]
...
class EmailService:
    ...
    async def send_email(
        ...
        try:
            ...
            # Render template
            subject, html_content = self.template_manager.render_template(
                email_type, variables
            )
            ...
            return EmailDeliveryResult(
                success=False, error_message=error_msg, status=EmailStatus.FAILED
            )
    ...
    async def send_password_reset_email(
        self, user: User, reset_token: str, db: AsyncSession
    ) -> EmailDeliveryResult:
        ...
        expires_at = datetime.now(timezone.utc) + timedelta(
            hours=PasswordResetTokenManager.TOKEN_EXPIRY_HOURS
        )
    ...
    async def send_security_alert(
        ...
        variables = {
            ...
            "action_taken": (
                additional_context.get("action_taken") if additional_context else None
            ),
            "recommended_actions": (
                additional_context.get("recommended_actions")
    ...
    async def send_transaction_confirmation(
        self, user: User, transaction_details: Dict[str, Any], db: AsyncSession
    ) -> EmailDeliveryResult:
    ...
    async def send_budget_alert(
        self, user: User, budget_details: Dict[str, Any], db: AsyncSession
    ) -> EmailDeliveryResult:
        """Send budget alert email"""

        percentage_used = (
            budget_details["spent_amount"] / budget_details["budget_limit"]
        ) * 100
        ...
        variables = {
            ...
            "recommendations": budget_details.get(
                "recommendations",
                [
...
async def send_password_reset_email(
    user: User, reset_token: str, db: AsyncSession
) -> EmailDeliveryResult:
    """Send password reset email"""
...
async def send_security_alert_email(
    ...
    return await email_service.send_security_alert(
        user, alert_type, alert_details, db, additional_context
    )
