        ...
        return [
            {
                **metrics,
                ...
                - timedelta(seconds=i * self.check_interval),
            }
            for i, metrics in enumerate(reversed(self.metrics_history))
            if datetime.now() - timedelta(seconds=i * self.check_interval)
            >= cutoff_time
        ]
...
class RequestProfiler:
    ...
    def get_slow_requests(self, limit: int = 10) -> List[Dict[str, Any]]:
        ...
        sorted_requests = sorted(
            self.slow_requests, key=lambda x: x["duration"], reverse=True
        )
            ...
            async def async_wrapper(*args, **kwargs):
                ...
                try:
                    ...
                    self.monitor.record_function_call(
                        func.__name__, category, duration, status, memory_diff
                    )
...
class PerformanceMonitor:
    ...
    def _generate_recommendations(self) -> List[str]:
        ...
        if current_metrics["cpu_percent"] > 70:
            recommendations.append(
                "High CPU usage - consider optimizing compute-intensive operations"
            )
        ...
        for cache_name, stats in cache_stats.items():
            if stats["hit_rate_percent"] < 60:
                recommendations.append(
                    f"Low cache hit rate for {cache_name} ({stats['hit_rate_percent']:.1f}%) - review cache strategy"
                )

        # Check endpoint performance
        slow_endpoints = [
            ep
            for ep in self.request_profiler.get_endpoint_stats(5)
            if ep["avg_time"] > 1.0
        ]
        if slow_endpoints:
            recommendations.append(
                f"Slow endpoints detected - optimize: {', '.join([ep['endpoint'] for ep in slow_endpoints[:3]])}"
            )

        # Check memory leaks
        memory_report = self.memory_profiler.get_memory_report()
        if "recent_comparison" in memory_report and memory_report[
            "recent_comparison"
        ].get("potential_leak"):
            recommendations.append(
