...
from app.services.core.income_scaling_algorithms import scale_threshold_by_income
...
class AIFinancialAnalyzer:
    ...
    def analyze_spending_patterns(self) -> Dict:
        ...
        weekend_spending, weekday_spending = self._analyze_weekend_patterns(
            spending_data
        )
        ...
        small_purchases = [
            s for s in spending_data if s["amount"] < small_purchase_threshold
        ]
    ...
    def _analyze_weekend_patterns(
        self, spending_data: List[Dict]
    ) -> Tuple[float, float]:
    ...
    def _analyze_monthly_variance(self, spending_data: List[Dict]) -> float:
        ...
        for expense in spending_data:
            ...
            monthly_totals[month_key] = (
                monthly_totals.get(month_key, 0) + expense["amount"]
            )
    ...
    def _detect_impulse_buying(self, spending_data: List[Dict]) -> float:
        ...
        for expense in spending_data:
            ...
            if any(
                word in description for word in ["sale", "discount", "deal", "offer"]
            ):
                impulse_indicators += 1
            # Use dynamic threshold instead of hardcoded $50
            self._get_user_context()
            medium_purchase_threshold = self._get_dynamic_thresholds()[
                "spending_patterns"
            ]["medium_purchase_threshold"]

            if amount > medium_purchase_threshold and any(
                cat in expense["category"].lower()
                for cat in ["entertainment", "shopping"]
            ):
    ...
    def generate_personalized_feedback(self) -> Dict:
        ...
        if not spending_data:
            return {
                ...
                "tips": [
                    "Begin by logging your daily expenses",
        ...
        top_category = (
            max(category_analysis.items(), key=lambda x: x[1])[0]
            if category_analysis
            else "general"
        )

        # Generate contextual feedback
        feedback_text = self._generate_contextual_feedback(
            spending_data, patterns, category_analysis
        )
    ...
    def _generate_contextual_feedback(
        ...
        avg_daily_spending = total_spending / max(
            1, len(set(expense["date"].date() for expense in spending_data))
        )
        ...
        if "impulse_buying" in detected_patterns:
            feedback_parts.append(
                "Consider implementing a waiting period before non-essential purchases."
            )
        ...
        if category_analysis:
            ...
            if (percentage / 100) > concentration_warning:
                feedback_parts.append(
                    f"Your {top_category} expenses represent {percentage:.0f}% of total spending, consider diversifying."
                )

        return " ".join(feedback_parts)

    def _generate_actionable_tips(
        self, patterns: List[str], category_analysis: Dict[str, float]
    ) -> List[str]:
        ...
        if "frequent_small_purchases" in patterns:
            tips.append(
                "Use the 24-hour rule: wait a day before making purchases under $25"
            )
            tips.append("Set up automatic savings to redirect small purchase amounts")

        if "impulse_buying" in patterns:
            tips.append(
                "Create a wish list and review it weekly instead of buying immediately"
            )
        ...
        if category_analysis:
            ...
            if top_category == "food" or top_category == "dining":
                ...
                user_context = self._get_user_context()
                food_ratio = scale_threshold_by_income(
                    "food_ratio",
                    user_context.monthly_income,
                ...
                if current_food_pct > food_ratio * 100 * 1.2:  # 20% above optimal
                    potential_savings = (
                        (current_food_pct - food_ratio * 100) / current_food_pct * 100
                    )
                    tips.append(
                        f"Try meal planning to reduce food expenses by {potential_savings:.0f}%"
                    )
                else:
                ...
                tips.append(
                    "Set up a monthly entertainment budget and track it closely"
                )
        ...
        if not patterns:
            ...
            user_context = self._get_user_context()
            savings_target = scale_threshold_by_income(
                "savings_rate",
                user_context.monthly_income,
    ...
    def _calculate_spending_score(self, spending_data: List[Dict]) -> float:
        ...
        good_concentration = (
            concentration_thresholds["category_concentration_threshold"] * 0.8
        )
        bad_concentration = (
            concentration_thresholds["category_concentration_threshold"] * 1.2
        )
    ...
    def _generate_health_improvements(
        self, budgeting: float, efficiency: float, saving: float, consistency: float
    ) -> List[str]:
        ...
        if budgeting < budgeting_target:
            improvements.append(
                "Create detailed budget categories and track spending against limits"
            )

        if efficiency < efficiency_target:
            improvements.append(
                "Reduce impulse purchases by implementing a 24-hour waiting period"
            )

        if saving < savings_target:
            improvements.append(
                "Cancel unused subscriptions and redirect savings to emergency fund"
            )

        if consistency < consistency_target:
            improvements.append(
                "Establish regular spending patterns and avoid large monthly variations"
            )

        # If all scores meet tier expectations, provide advanced tips
        all_targets_met = (
            budgeting >= budgeting_target
            and efficiency >= efficiency_target
            and saving >= savings_target
            and consistency >= consistency_target
        )
    ...
    def detect_spending_anomalies(self) -> List[Dict]:
        ...
        for category, amounts in category_data.items():
            ...
            for expense in spending_data:
                if (
                    ...
                    and expense["amount"] > threshold
                    and expense["amount"] > mean_amount * 1.5
                ):

                    severity = (
                        "high" if expense["amount"] > threshold * 1.5 else "medium"
                    )
        ...
        anomalies.sort(
            key=lambda x: (severity_order[x["severity"]], x["amount"]), reverse=True
        )
    ...
    def generate_savings_optimization(self) -> Dict:
        ...
        if subscription_savings > 0:
            suggestions.append(
                f"Review and cancel unused subscriptions - save ${subscription_savings:.2f}/month"
            )
        ...
        if dining_savings > 0:
            suggestions.append(
                f"Reduce dining out by 25% through meal planning - save ${dining_savings:.2f}/month"
            )
        ...
        if "impulse_buying" in patterns:
            ...
            suggestions.append(
                f"Implement 24-hour rule for non-essential purchases - save ${impulse_savings:.2f}/month"
            )
        ...
        if transport_savings > 0:
            suggestions.append(
                f"Optimize transportation through carpooling/public transit - save ${transport_savings:.2f}/month"
            )
            total_potential_savings += transport_savings

        # Analyze small purchases
        if "frequent_small_purchases" in patterns:
            small_purchase_savings = self._calculate_small_purchase_savings(
                spending_data
            )
            suggestions.append(
                f"Reduce small impulse purchases by 40% - save ${small_purchase_savings:.2f}/month"
            )
    ...
    def _calculate_subscription_savings(self, spending_data: List[Dict]) -> float:
        ...
        for expense in spending_data:
            ...
            if any(
                word in description
                for word in ["subscription", "monthly", "netflix", "spotify", "gym"]
            ):
        ...
        monthly_subscription_cost = sum(subscription_expenses) / max(
            1, len(set(expense["date"].strftime("%Y-%m") for expense in spending_data))
        )
    ...
    def _calculate_dining_savings(self, spending_data: List[Dict]) -> float:
        ...
        dining_expenses = [
            expense
            for expense in spending_data
            if "dining" in expense["category"].lower()
            or "restaurant" in expense["category"].lower()
        ]

        if not dining_expenses:
            return 0.0

        monthly_dining = sum(expense["amount"] for expense in dining_expenses) / max(
            1, len(set(expense["date"].strftime("%Y-%m") for expense in spending_data))
        )
    ...
    def _calculate_impulse_savings(self, spending_data: List[Dict]) -> float:
        ...
        total_monthly_spending = sum(
            expense["amount"] for expense in spending_data
        ) / max(
            1, len(set(expense["date"].strftime("%Y-%m") for expense in spending_data))
        )
    ...
    def _calculate_transport_savings(self, spending_data: List[Dict]) -> float:
        ...
        transport_expenses = [
            ...
            for expense in spending_data
            if any(
                word in expense["category"].lower()
                for word in ["transport", "gas", "fuel", "uber", "taxi"]
            )
        ]
        ...
        monthly_transport = sum(
            expense["amount"] for expense in transport_expenses
        ) / max(
            1, len(set(expense["date"].strftime("%Y-%m") for expense in spending_data))
        )
    ...
    def _calculate_small_purchase_savings(self, spending_data: List[Dict]) -> float:
        ...
        small_purchases = [
            expense for expense in spending_data if expense["amount"] < 25
        ]

        if not small_purchases:
            return 0.0

        monthly_small_purchases = sum(
            expense["amount"] for expense in small_purchases
        ) / max(
            1, len(set(expense["date"].strftime("%Y-%m") for expense in spending_data))
        )
    ...
    def generate_weekly_insights(self) -> Dict:
        ...
        biggest_expense = (
            max(current_week_data, key=lambda x: x["amount"])["amount"]
    ...
    def _generate_weekly_insights_text(
        self,
        current_total: float,
        ...
        return (
            f"{base_text} {sentiment} Your largest expense category was {top_category}."
        )

    def _generate_weekly_recommendations(
        self, current_week_data: List[Dict], trend: str, percentage_change: float
    ) -> List[str]:
        ...
        if trend == "increasing":
            recommendations.append(
                "Consider reducing discretionary spending for the remainder of the month"
            )
            recommendations.append(
            ...
            recommendations.append(
                "Excellent progress! Consider putting the savings toward your emergency fund"
            )
            recommendations.append(
                "Continue current spending patterns - they're working well"
            )
        else:
            recommendations.append("Maintain your current spending discipline")
            recommendations.append(
                "Look for small optimization opportunities to improve further"
            )
        ...
        if current_week_data:
            top_category = max(
                ...
                key=lambda cat: sum(
                    exp["amount"] for exp in current_week_data if exp["category"] == cat
                ),
            )

            if top_category.lower() in ["dining", "food"]:
                recommendations.append(
                    "Try meal prep this weekend to reduce food costs next week"
                )
            elif top_category.lower() in ["entertainment", "shopping"]:
                recommendations.append(
                    "Set a specific entertainment budget for next week and track it daily"
                )
