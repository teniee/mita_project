...
try:
    ...
    if not validation_result["valid"]:
        logger.error(
            f"Bcrypt configuration validation failed: {validation_result['issues']}"
        )
    elif validation_result["warnings"]:
        logger.warning(
            f"Bcrypt configuration warnings: {validation_result['warnings']}"
        )
    else:
        logger.info(
            f"Bcrypt configuration validated successfully with {validation_result['configuration']['rounds']} rounds"
        )
...
def decode_token(token: str) -> dict:
    return jwt.decode(
        token, settings.SECRET_KEY, algorithms=[ALGORITHM], audience=JWT_AUDIENCE
    )
...
def _create_token(
    data: dict, expires_delta: timedelta, token_type: str, scopes: List[str] = None
) -> str:
    ...
    security_claims = {
        ...
        "token_version_id": data.get(
            "token_version_id", 1
        ),  # User's token version for revocation
    }
    ...
    logger.debug(
        f"Creating {token_type} with scopes: {scopes or []} for user: {data.get('user_id', 'unknown')}"
    )
...
async def revoke_user_tokens(
    user_id: str, reason: str = "security", revoked_by: Optional[str] = None
) -> int:
    ...
    try:
        ...
        log_security_event(
            "user_token_revocation_requested",
            {"user_id": user_id, "reason": reason, "revoked_by": revoked_by},
        )
        ...
        log_security_event(
            "user_token_revocation_completed",
            {"user_id": user_id, "tokens_revoked": revoked_count, "reason": reason},
        )
        ...
        log_security_event(
            "user_token_revocation_failed", {"user_id": user_id, "error": str(e)}
        )
        return 0
...
def _cleanup_token_cache():
    ...
    expired_keys = [
        token_hash
        for token_hash, cache_entry in _token_cache.items()
    ...
    if len(_token_cache) > _cache_max_size:
        sorted_items = sorted(
            _token_cache.items(), key=lambda x: x[1].get("cache_time", 0)
        )
...
def get_token_info(token: str) -> Optional[Dict[str, Any]]:
    ...
    if (
        ...
        or current_time % _cache_cleanup_interval < 1
    ):
    ...
    for secret in secrets:
        try:
            payload = jwt.decode(
                token, secret, algorithms=[ALGORITHM], audience=JWT_AUDIENCE
            )
            token_info = {
                "jti": payload.get("jti"),
...
async def validate_token_security(token: str) -> Dict[str, Any]:
    ...
    if jti:
        try:
            ...
            blacklist_service = await get_blacklist_service()
            validation["is_blacklisted"] = await blacklist_service.is_token_blacklisted(
                jti
            )
        except Exception as e:
...
def create_access_token(
    ...
    logger.debug(
        f"Created access token for user {data.get('sub')} with scopes: {scopes}"
    )
    return token
...
async def verify_token(
    ...
    for i, secret in enumerate(secrets):
        try:
            ...
            try:
                ...
                logger.debug(
                    f"JWT decode failed with secret #{i + 1}: {type(decode_error).__name__}"
                )
            ...
            if payload.get("iss") != JWT_ISSUER:
                logger.warning(
                    f"Token issuer mismatch: expected {JWT_ISSUER}, got {payload.get('iss')}"
                )
                log_security_event(
            ...
            if payload.get("aud") != JWT_AUDIENCE:
                logger.warning(
                    f"Token audience mismatch: expected {JWT_AUDIENCE}, got {payload.get('aud')}"
                )
                log_security_event(
            ...
            if required_scopes:
                ...
                if missing_scopes:
                    ...
                    raise InvalidTokenError(
                        f"Insufficient token scopes: {missing_scopes}"
                    )
            ...
            if is_fresh_token:
                logger.debug(
                    f"Fresh token (age: {token_age_minutes:.0f}min) — skipping DB validation"
                )
                ...
                try:
                    ...
                    async def _run_version_check(session) -> None:
                        ...
                        if user:
                            ...
                            token_version_id = payload.get(
                                "token_version_id", user.token_version
                            )
                            ...
                            if (
                                "token_version_id" in payload
                                and token_version_id < user.token_version
                            ):
                                logger.warning(
                                    f"Token version mismatch for user {user_id}: token={token_version_id}, current={user.token_version}"
                                )
                                log_security_event(
                            ...
                            logger.warning(
                                f"User not found in database during token version check: {user_id}"
                            )
                    ...
                    logger.error(
                        f"Error checking token version: {version_check_error}",
                        exc_info=True,
                    )
                    # Fail-open for version checks but log the security concern
                    log_security_event(
                        "token_version_check_failed",
                        {"user_id": user_id, "error": str(version_check_error)},
                    )
            ...
            if jti:
                try:
                    ...
                    if is_blacklisted:
                        logger.warning(
                            f"Blacklisted token access attempt: jti={jti[:8]}..."
                        )
                        log_security_event(
                            "blacklisted_token_usage_attempt",
                            {"jti": jti[:8] + "...", "token_type": token_type},
                        )
                    ...
                    logger.error(
                        f"Error checking token blacklist: {blacklist_error}",
                        exc_info=True,
                    )
    ...
    logger.warning(
        f"Token verification failed after trying all secrets: {last_error or 'unknown error'}"
    )
    return None


async def blacklist_token(
    token: str, reason: str = "logout", revoked_by: Optional[str] = None
) -> bool:
    ...
    try:
        ...
        success = await blacklist_service.blacklist_token(
            token=token, reason=blacklist_reason, revoked_by=revoked_by
        )
        ...
        log_security_event("token_blacklist_error", {"error": str(e), "reason": reason})
        return False
...
def create_token_pair(user_data: dict, user_role: str = None) -> Dict[str, str]:
    ...
    if elapsed > 1.0:  # Log if token creation takes more than 1 second
        logger.warning(
            f"Slow token creation: {elapsed:.2f}s for user {user_data.get('sub', 'unknown')}"
        )
