...
class TokenSecurityMonitor:
    ...
    def log_scope_violation(
        ...
        self._check_privilege_escalation(
            user_id, required_scopes, token_scopes, ip_address
        )
    ...
    def _check_suspicious_token_creation(
        self, user_id: str, ip_address: Optional[str], scopes: List[str]
    ):
        ...
        self.rate_limit_violations[user_key] = [
            t
            for t in self.rate_limit_violations[user_key]
    ...
    def _track_failed_attempt(self, user_id: str, ip_address: Optional[str]):
        ...
        self.failed_attempts[user_id] = [
            t
            for t in self.failed_attempts[user_id]
            if current_time - t < 3600  # Last hour
        ]
        ...
        if ip_address:
            ...
            self.failed_attempts[ip_key] = [
                t
                for t in self.failed_attempts[ip_key]
                if current_time - t < 3600  # Last hour
            ]
    ...
    def _check_privilege_escalation(
            ...
            self.detect_suspicious_activity(
                ...
                "admin_privilege_escalation_attempt",
                {
                    "required_admin_scopes": [
                        s for s in required_scopes if s in admin_scopes
                    ],
        ...
        if premium_attempt and not has_premium:
            self.detect_suspicious_activity(
                ...
                {
                    "required_premium_scopes": [
                        s for s in required_scopes if s in premium_scopes
                    ],
                    "user_scopes": token_scopes,
                },
                ip_address,
