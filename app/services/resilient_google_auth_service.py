...
from app.core.circuit_breaker import CircuitBreakerConfig, get_circuit_breaker_manager
...
class ResilientGoogleAuthService:
    ...
    async def authenticate_google_user(self, id_token_str: str) -> User:
        ...
        try:
            ...
            logger.error(
                f"Unexpected error in Google authentication: {str(e)}", exc_info=True
            )
            raise HTTPException(
    ...
    async def _verify_google_token(self, id_token: str) -> dict:
        ...
        try:
            return await self.circuit_breaker_manager.call_service(
                "google_oauth_verify", _make_verification_request
            )
        except HTTPException:
    ...
    async def _get_or_create_user(self, email: str, google_payload: dict) -> User:
        ...
        try:
            ...
            logger.error(
                f"Error creating/retrieving user for Google auth: {str(e)}",
                exc_info=True,
            )
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
    ...
    async def get_service_health(self) -> dict:
        ...
        return {
            ...
            "status": (
                ...
                else "degraded" if stats["state"] == "half_open" else "unhealthy"
            ),
            "circuit_breaker_state": stats["state"],
            "total_requests": stats["total_requests"],
            "success_rate": stats["success_rate"],
            "consecutive_failures": stats["consecutive_failures"],
            "last_request_time": stats["last_success_time"]
            or stats["last_failure_time"],
    ...
    async def test_connection(self) -> bool:
        ...
        try:
            ...
            return await self.circuit_breaker_manager.call_service(
                "google_oauth_verify", _test_request
            )
