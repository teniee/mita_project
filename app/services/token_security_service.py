...
class TokenSecurityService:
    ...
    def record_token_issued(
        self, user_id: str, token_type: str, client_ip: Optional[str] = None
    ):
        ...
        log_security_event(
            "token_issued",
            {"user_id": user_id, "token_type": token_type, "client_ip": client_ip},
        )
    ...
    def record_token_blacklisted(
        self, user_id: str, token_jti: str, reason: str = "logout"
    ):
    ...
    def record_blacklist_hit(
        self, user_id: Optional[str], token_jti: str, client_ip: Optional[str] = None
    ):
        ...
        log_security_event(
            ...
            {"user_id": user_id, "jti": token_jti[:8] + "...", "client_ip": client_ip},
        )

    def record_refresh_token_operation(
        self, user_id: str, success: bool, client_ip: Optional[str] = None
    ):
    ...
    def _check_token_issuance_rate(self, user_id: str, client_ip: Optional[str]):
        ...
        user_tokens_last_hour = sum(
            1
            for activity in self._user_activity[user_id]
            if activity["action"] == "token_issued" and activity["timestamp"] > hour_ago
        )
    ...
    def _check_verification_failures(self, user_id: str, client_ip: Optional[str]):
        ...
        if client_ip and self._suspicious_ips[client_ip] > self.SUSPICIOUS_IP_THRESHOLD:
            alert = SecurityAlert(
                ...
                metadata={
                    "ip": client_ip,
                    "failure_count": self._suspicious_ips[client_ip],
                },
            )
            self._add_alert(alert)
    ...
    def _add_alert(self, alert: SecurityAlert):
        ...
        log_security_event(
            f"security_alert_{alert.alert_type}",
            {
    ...
    def get_user_activity_summary(self, user_id: str) -> Dict:
        ...
        summary = {
            ...
            "tokens_issued": sum(
                1 for a in activities if a["action"] == "token_issued"
            ),
            "successful_verifications": sum(
                1 for a in activities if a["action"] == "verification_success"
            ),
            "failed_verifications": sum(
                1 for a in activities if a["action"] == "verification_failure"
            ),
            "tokens_blacklisted": sum(
                1 for a in activities if a["action"] == "token_blacklisted"
            ),
            "refresh_operations": sum(
                1 for a in activities if a["action"] == "token_refresh"
            ),
            "unique_ips": len(set(a.get("ip") for a in activities if a.get("ip"))),
            "last_activity": (
                max(activities, key=lambda a: a["timestamp"])["timestamp"]
                if activities
                else None
            ),
        }
    ...
    def get_security_metrics(self) -> Dict:
        ...
        return {
            ...
            "alert_summary": {
                ...
                "critical_alerts": sum(
                    1 for a in self._alerts if a.severity == "CRITICAL"
                ),
            ...
            "suspicious_activity": {
                "suspicious_ips": len(
                    [
                        ip
                        for ip, count in self._suspicious_ips.items()
    ...
    async def cleanup_old_data(self):
        ...
        with self._lock:
            ...
            now - 3600
            self._suspicious_ips = defaultdict(
                int,
                {
                    ip: count
                    for ip, count in self._suspicious_ips.items()
    ...
    async def check_token_health(self, token: str) -> Dict:
        ...
        if not token_info:
            return {"status": "invalid", "reason": "Token could not be decoded"}
