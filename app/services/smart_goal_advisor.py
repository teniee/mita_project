...
class SmartGoalAdvisor:
    ...
    def generate_personalized_recommendations(self, user_id: UUID) -> List[Dict]:
        """
        ...
        existing_goals = (
            self.db.query(Goal)
            .filter(Goal.user_id == user_id, Goal.status.in_(["active", "paused"]))
            .all()
        )
        ...
        if "Savings" not in existing_categories or monthly_income > 5000:
            recommendations.append(
                self._recommend_general_savings(monthly_income, spending_analysis)
            )
        ...
        category_recs = self._recommend_category_goals(
            spending_analysis, existing_categories, monthly_income
        )
    ...
    def analyze_goal_health(self, goal_id: UUID, user_id: UUID) -> Dict:
        ...
        if goal.target_date:
            ...
            if total_days > 0:
                ...
                progress_ratio = (
                    progress / expected_progress if expected_progress > 0 else 0
                )
                ...
                if progress > 0 and days_since_created > 0:
                    ...
                    days_to_completion = (
                        int(remaining_progress / daily_progress)
                        if daily_progress > 0
                        else 0
                    )
    ...
    def detect_goal_opportunities(self, user_id: UUID) -> List[Dict]:
        ...
        for category, data in spending_analysis.get(
    ...
    def _analyze_spending_patterns(self, user_id: UUID) -> Dict:
        ...
        small_recurring = [
            ...
            if abs(float(t.amount)) < 50 and t.category in ["Subscriptions", "Services"]
        ]
    ...
    def _recommend_general_savings(
        self, monthly_income: float, spending_analysis: Dict
    ) -> Dict:
    ...
    def _recommend_category_goals(
        self, spending_analysis: Dict, existing_categories: set, monthly_income: float
    ) -> List[Dict]:
        """Recommend goals based on spending patterns"""
        recommendations = []

        # Travel goal if user has travel expenses
        travel_spending = spending_analysis.get("recurring_expenses", {}).get(
            "Travel", {}
        )
