...
from app.services.email_service import EmailPriority, EmailService, EmailType
...
class EmailQueueService:
    ...
    def __init__(self):
        ...
        self.max_processing_time = int(
            os.getenv("EMAIL_MAX_PROCESSING_TIME", "300")
        )  # 5 minutes
        self.batch_size = int(os.getenv("EMAIL_BATCH_SIZE", "10"))
        self.dead_letter_retention_days = int(
            os.getenv("EMAIL_DEAD_LETTER_RETENTION_DAYS", "7")
        )
    ...
    async def enqueue_email(
        ...
        try:
            ...
            await redis_client.hset(
                f"{self.queue_key}:jobs", job_id, json.dumps(job.to_dict())
            )
            ...
            logger.info(
                f"Email job {job_id} enqueued for {to_email} (type: {email_type.value})"
            )
            return job_id
    ...
    def _calculate_priority_score(
        self, priority: EmailPriority, scheduled_at: datetime
    ) -> float:
    ...
    async def _process_email_immediately(self, job: EmailQueueJob) -> str:
        ...
        try:
            ...
            logger.info(
                f"Email processed immediately: {job.id} - Success: {result.success}"
            )
            return job.id
    ...
    async def _process_batch(self) -> int:
        ...
        try:
            ...
            job_ids = await redis_client.zrangebyscore(
                self.queue_key, 0, now_timestamp, start=0, num=self.batch_size
            )
    ...
    async def _complete_job(self, job_id: str, job: EmailQueueJob):
        ...
        try:
            ...
            self.metrics.current_queue_size = max(
                0, self.metrics.current_queue_size - 1
            )
    ...
    async def _handle_job_failure(self, job_id: str, job: EmailQueueJob):
        ...
        if job.retry_count <= job.max_retries:
            ...
            logger.info(
                f"Email job {job_id} scheduled for retry {job.retry_count}/{job.max_retries}"
            )
        else:
            ...
            logger.error(
                f"Email job {job_id} moved to dead letter queue after {job.retry_count} attempts"
            )
    ...
    async def _enqueue_for_retry(self, job_id: str, job: EmailQueueJob):
        ...
        try:
            ...
            await redis_client.zadd(
                self.retry_queue_key, {job_id: job.scheduled_at.timestamp()}
            )
    ...
    async def _process_retry_queue(self) -> int:
        ...
        try:
            ...
            now_timestamp = datetime.now(timezone.utc).timestamp()
            job_ids = await redis_client.zrangebyscore(
                self.retry_queue_key, 0, now_timestamp, start=0, num=self.batch_size
            )
                ...
                try:
                    ...
                    if job_data:
                        ...
                        priority_score = self._calculate_priority_score(
                            job.priority, job.scheduled_at
                        )

                        # Move to main queue
                        await redis_client.zadd(
                            self.queue_key, {job_id: priority_score}
                        )
    ...
    async def _cleanup_old_jobs(self):
        ...
        try:
            ...
            old_jobs = await redis_client.zrangebyscore(
                self.dead_letter_queue_key, 0, cutoff_timestamp
            )
            ...
            stuck_jobs = await redis_client.zrangebyscore(
                self.processing_key, 0, processing_cutoff
            )
    ...
    async def _update_metrics(self):
        ...
        try:
            ...
            total_processed = (
                self.metrics.total_jobs_processed + self.metrics.total_jobs_failed
            )
            if total_processed > 0:
                self.metrics.success_rate = (
                    self.metrics.total_jobs_processed / total_processed
                )
...
async def queue_password_reset_email(
    user_email: str, user_name: str, reset_token: str, user_id: str
) -> str:
