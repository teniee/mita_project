...
class ExpenseRepository(BaseRepository[Expense]):
    ...
    async def get_daily_expenses(self, user_id: int, date: datetime) -> List[Expense]:
        ...
        async with get_async_db_context() as db:
            result = await db.execute(
                select(Expense)
                .where(and_(Expense.user_id == user_id, Expense.date == date.date()))
    ...
    async def get_by_category(
        ...
        async with get_async_db_context() as db:
            result = await db.execute(
                select(Expense)
                .where(and_(Expense.user_id == user_id, Expense.category == category))
                .order_by(desc(Expense.date))
                .limit(limit)
            )
            return list(result.scalars().all())
    ...
    async def get_daily_total(self, user_id: int, date: datetime) -> Decimal:
        ...
        async with get_async_db_context() as db:
            result = await db.execute(
                select(func.sum(Expense.amount)).where(
                    and_(Expense.user_id == user_id, Expense.date == date.date())
                )
            )
            total = result.scalar()
            return total or Decimal("0.00")
    ...
    async def get_largest_expenses(
        self, user_id: int, limit: int = 10, days: int = 30
    ) -> List[Expense]:
        ...
        async with get_async_db_context() as db:
            result = await db.execute(
                select(Expense)
                .where(
                    and_(Expense.user_id == user_id, Expense.date >= start_date.date())
                )
    ...
    async def get_expense_trends(self, user_id: int, days: int = 30) -> Dict[str, Any]:
        ...
        async with get_async_db_context() as db:
            ...
            daily_totals = await db.execute(
                select(Expense.date, func.sum(Expense.amount).label("daily_total"))
                .where(
                    and_(
                        Expense.user_id == user_id,
                        Expense.date >= start_date.date(),
            ...
            return {
                ...
                "most_expensive_day": {
                    ...
                    "amount": (
                        most_expensive_day.daily_total
