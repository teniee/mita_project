...
class GoalRepository(BaseRepository[Goal]):
    ...
    async def get_completed_goals(self, user_id: UUID) -> List[Goal]:
        ...
        async with get_async_db_context() as db:
            result = await db.execute(
                select(Goal)
                .where(and_(Goal.user_id == user_id, Goal.status == "completed"))
    ...
    async def get_goals_by_progress_range(
        self, user_id: UUID, min_progress: float = 0.0, max_progress: float = 100.0
    ) -> List[Goal]:
    ...
    async def get_near_completion_goals(
        self, user_id: UUID, threshold: float = 80.0
    ) -> List[Goal]:
    ...
    async def update_goal_progress(self, goal_id: UUID, progress: float) -> bool:
        ...
        if progress >= 100.0:
            update_data.update({"status": "completed", "completed_at": datetime.now()})
    ...
    async def pause_goal(self, goal_id: UUID) -> bool:
        ...
        update_data = {"status": "paused", "last_updated": datetime.now()}
        return await self.update(goal_id, update_data) is not None

    async def resume_goal(self, goal_id: UUID) -> bool:
        """Resume a paused goal"""
        update_data = {"status": "active", "last_updated": datetime.now()}
        return await self.update(goal_id, update_data) is not None
    ...
    async def get_goal_statistics(self, user_id: UUID) -> Dict[str, Any]:
        ...
        async with get_async_db_context() as db:
            ...
            category_counts = await db.execute(
                select(Goal.category, func.count(Goal.id).label("count"))
                .where(Goal.user_id == user_id)
