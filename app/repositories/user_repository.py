...
class UserRepository(BaseRepository[User]):
    ...
    async def get_premium_users(self) -> List[User]:
        ...
        async with get_async_db_context() as db:
            result = await db.execute(select(User).where(User.is_premium.is_(True)))
    ...
    async def get_inactive_users(self, days: int = 30) -> List[User]:
        ...
        async with get_async_db_context() as db:
            ...
            result = await db.execute(
                select(User)
                .where(or_(User.last_login < cutoff_date, User.last_login.is_(None)))
    ...
    async def bulk_update_premium_status(
        self, user_ids: List[int], is_premium: bool
    ) -> int:
