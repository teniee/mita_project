...
class CohortDriftTracker:
    ...
    def log_month(self, user_id: str, year: int, month: int, cohort: str, country: str, region: str):
        key = f"{year}-{month:02d}"
        self.history[user_id][key] = {"cohort": cohort, "country": country, "region": region}
    ...
    def get_drift(self, user_id: str):
        ...
        for i in range(1, len(months)):
            ...
            if self.history[user_id][prev]["cohort"] != self.history[user_id][curr]["cohort"]:
                drifts.append({
