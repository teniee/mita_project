...
from app.core.security import SecurityConfig, SecurityUtils
...
class TestPasswordStrengthValidation:
        ...
        for password, expected_min_score in password_complexity_tests:
            ...
            if expected_min_score >= 4:  # Strong passwords
                try:
                    ...
                    pytest.fail(
                        f"High complexity password should be accepted: {password}"
                    )
    ...
    def test_password_entropy_calculation(self):
        ...
        for password, expected_min_entropy in entropy_tests:
            ...
            assert (
                actual_entropy >= expected_min_entropy * 0.95
    ...
    def test_password_policy_compliance(self):
        ...
        for policy in financial_policy_tests:
            for password in compliant_passwords:
                ...
                meets_policy = (
                    len(password) >= policy["min_length"]
                    and (
                        not policy["require_mixed_case"]
                        or (
                    ...
                    and (
                        ...
                        or any(c.isdigit() for c in password)
                    )
                    and (
                        not policy["require_special"]
                        or any(c in "!@#$%^&*()_+-=[]{}|;:,.<>?" for c in password)
                    )
                )
                ...
                if meets_policy:
                    try:
                        ...
                        pytest.fail(
                            f"Policy-compliant password should be accepted: {password} for {policy['name']}"
                        )
...
class TestPasswordHashing:
    ...
    def test_timing_attack_resistance(self):
        ...
        ), "Timing variation too large, possible timing attack vulnerability"
...
class TestPasswordResetSecurity:
    ...
    def test_reset_token_expiration(self):
        ...
        def store_reset_token(email: str, token: str, expiry_minutes: int = 30):
            ...
            token_store[token] = {"email": email, "expiry": expiry_time, "used": False}
        ...
        store_reset_token(
            test_email, expired_token, expiry_minutes=-1
        )  # Already expired
    ...
    def test_password_reset_rate_limiting(self):
        ...
        def can_request_reset(
            email: str, max_requests: int = 3, window_minutes: int = 60
        ) -> bool:
            ...
            reset_attempts[email] = [
                attempt_time
                for attempt_time in reset_attempts[email]
