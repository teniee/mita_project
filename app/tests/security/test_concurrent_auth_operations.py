...
class TestConcurrentTokenOperations:
    ...
    def test_concurrent_token_creation(self, user_data):
        ...
        assert (
            len(set(created_tokens)) == 100
        ), "Duplicate tokens created under concurrency"
    ...
    def test_concurrent_token_validation(self, user_data):
        ...
        assert (
            len(incorrect_validations) == 0
        ), f"Incorrect validations: {incorrect_validations}"
    ...
    def test_concurrent_token_blacklisting(self, user_data, blacklist_store):
        ...
        with patch(
            "app.services.token_blacklist_service.get_blacklist_service", mock_get
        ):
            ...
            with ThreadPoolExecutor(max_workers=15) as executor:
                futures = [
                    executor.submit(blacklist_token_worker, token)
        ...
        with patch(
            ...
            def rotate_token_worker(worker_id):
                try:
                    ...
                    payload = _sync_verify_token(
                        original_refresh_token, token_type="refresh_token"
                    )
                    if payload is None:
                        with results_lock:
            ...
            assert (
                len(store) >= 1
            ), "Original refresh token should be in blacklist store"
    ...
    def test_race_condition_prevention_login_logout(self, user_data, blacklist_store):
        ...
        with patch(
            ...
            for session in active_tokens.values():
                ...
                refresh_payload = _sync_verify_token(
                    session["refresh"], token_type="refresh_token"
                )

                # Active sessions should have valid tokens
                assert (
                    access_payload is not None
                ), "Active session has invalid access token"
                assert (
                    refresh_payload is not None
                ), "Active session has invalid refresh token"
...
class TestConcurrentRateLimiting:
    ...
    def test_concurrent_rate_limit_accuracy(self, mock_redis_client):
        ...
        with patch("app.core.security.redis_client", mock_redis_client):
                    ...
                    try:
                        ...
                        with results_lock:
                            rate_limit_results.append(
                                {"worker_id": worker_id, "allowed": True}
                            )
                    except RateLimitException:
                        with results_lock:
                            rate_limit_results.append(
                                {"worker_id": worker_id, "allowed": False}
                            )
            ...
            ), f"Rate limit errors: {rate_limit_errors}"
    ...
    def test_concurrent_progressive_penalties(self, mock_redis_client):
        ...
        with patch("app.core.security.redis_client", mock_redis_client):
            ...
            def penalty_worker(violation_count):
                try:
                    ...
                    with patch.object(
                        ...
                        penalty = rate_limiter._check_progressive_penalties(
                            client_id, "login"
                        )
            ...
            violation_counts = (
                list(range(0, 20)) * 10
            )  # Test same counts multiple times

            with ThreadPoolExecutor(max_workers=20) as executor:
                futures = [
                    executor.submit(penalty_worker, count) for count in violation_counts
                ]
            ...
            # Analyze results
            assert (
                len(penalty_errors) == 0
            ), f"Penalty calculation errors: {penalty_errors}"
    ...
    async def test_concurrent_database_operations(self):
        ...
        successful_auths = [
            r for r in auth_results if r["success"] and r["token_present"]
        ]
        assert (
            len(successful_auths) == 20
                ...
                for _ in range(10):
                    ...
                    if payload is None or payload["sub"] != user_data["sub"]:
                        consistency_errors.append(
                            f"Worker {worker_id}: Token validation failed"
                        )
                        break
