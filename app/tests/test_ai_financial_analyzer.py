...
def sample_expenses_subscriptions():
    ...
    for month in range(6):
        for desc, amount, category in subscriptions:
            ...
            expense.date = base_date + timedelta(
                days=month * 30, hours=subscriptions.index((desc, amount, category))
            )
...
class TestSpendingPatternDetection:
    ...
    def test_weekend_overspending_detection(
        self, mock_db, test_user_id, sample_expenses_weekend_pattern
    ):
    ...
    def test_frequent_small_purchases_detection(
        self, mock_db, test_user_id, sample_expenses_small_purchases
    ):
    ...
    def test_impulse_buying_detection(
        self, mock_db, test_user_id, sample_expenses_impulse_buying
    ):
    ...
    def test_subscription_accumulation_detection(
        self, mock_db, test_user_id, sample_expenses_subscriptions
    ):
...
class TestAnomalyDetection:
    ...
    def test_detect_high_severity_anomaly(
        self, mock_db, test_user_id, sample_expenses_with_anomaly
    ):
...
class TestSavingsOptimization:
    ...
    def test_savings_optimization_with_subscriptions(
        self, mock_db, test_user_id, sample_expenses_subscriptions
    ):
...
class TestDynamicThresholds:
    ...
    def test_thresholds_low_income_user(self, mock_db, low_income_user):
        ...
        with patch.object(AIFinancialAnalyzer, "_get_user_context") as mock_context:
            ...
            small_purchase_threshold = thresholds["spending_patterns"][
    ...
    def test_thresholds_scale_with_income(
        self, mock_db, low_income_user, high_income_user
    ):
        ...
        with patch.object(AIFinancialAnalyzer, "_get_user_context") as mock_context:
            ...
            mock_context.return_value = UserContext(
                ...
                family_size=4,
                housing_status="own",
                life_stage="married_with_children",
            )
            ...
            assert (
                thresholds_high["spending_patterns"]["small_purchase_threshold"]
...
class TestPersonalizedFeedback:
    ...
    def test_personalized_feedback_with_patterns(
        self, mock_db, test_user_id, sample_expenses_weekend_pattern
    ):
