...
class TestIncomeClassificationService:
    ...
    @pytest.mark.parametrize(
        "monthly_income,region,expected_tier",
        [
    ...
    def test_income_classification_accuracy(
        self, monthly_income, region, expected_tier
    ):
    ...
    def test_tier_thresholds_retrieval(self):
        ...
        for key in required_keys:
            ...
            assert (
                us_thresholds[key] > 0
            ), f"Invalid threshold for {key}: {us_thresholds[key]}"
    ...
    def test_behavioral_patterns(self):
        ...
        for tier in IncomeTier:
            ...
            required_attrs = [
                ...
                "savings_rate_target",
                "housing_ratio_target",
            ]

            for attr in required_attrs:
                assert (
                    attr in pattern
                ), f"Missing behavioral attribute {attr} for {tier.value}"

            # Verify savings rate is reasonable (0-40%)
            savings_rate = pattern["savings_rate_target"]
            assert (
                0.0 <= savings_rate <= 0.4
    ...
    def test_budget_weights_completeness(self):
        ...
        for tier in IncomeTier:
            ...
            for category, weight in weights.items():
                assert (
                    weight > 0
                ), f"Negative weight for {category} in {tier.value}: {weight}"
                assert (
                    weight < 1.0
                ), f"Weight > 100% for {category} in {tier.value}: {weight}"
    ...
    def test_validation_function(self):
        ...
        for profile in valid_profiles:
            assert self.service.validate_classification_consistency(
                profile
        ...
        for profile in invalid_profiles:
            assert not self.service.validate_classification_consistency(
                profile
