...
class TestJWTClaimsAndSecurity:
    ...
    def test_token_creation_with_standard_claims(self):
        ...
        user_data = {"sub": "test-user-123", "is_premium": False, "country": "US"}
        ...
        payload = jwt.decode(
            token, settings.SECRET_KEY, algorithms=["HS256"], audience=JWT_AUDIENCE
        )
    ...
    async def test_token_issuer_audience_validation(self):
        ...
        wrong_issuer_token = jwt.encode(
            wrong_issuer_payload, settings.SECRET_KEY, algorithm="HS256"
        )
    ...
    async def test_required_claims_validation(self):
        ...
        incomplete_token = jwt.encode(
            incomplete_payload, settings.SECRET_KEY, algorithm="HS256"
        )
    ...
    async def test_not_before_claim_validation(self):
        ...
        future_token = jwt.encode(
            future_payload, settings.SECRET_KEY, algorithm="HS256"
        )
...
class TestTokenSecurityMonitoring:
    ...
    def test_security_report_generation(self):
        ...
        monitor.log_scope_violation(
            "user2", "/admin", ["admin:system"], ["read:profile"]
        )
