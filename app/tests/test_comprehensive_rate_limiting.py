...
def mock_request():
    ...
    request.headers = {"User-Agent": "TestClient/1.0", "X-Forwarded-For": "10.0.0.1"}
...
class TestSecurityConfiguration:
    ...
    def test_rate_limit_tiers(self):
        ...
        assert (
            tiers["anonymous"]["requests_per_hour"]
            < tiers["basic_user"]["requests_per_hour"]
        )
        assert (
            tiers["basic_user"]["requests_per_hour"]
            < tiers["premium_user"]["requests_per_hour"]
        )
        assert (
            tiers["premium_user"]["requests_per_hour"]
            < tiers["admin_user"]["requests_per_hour"]
        )
...
class TestRateLimitingMiddleware:
    ...
    @pytest.mark.asyncio
    async def test_middleware_applies_general_rate_limiting(
        ...
        for _ in range(100):
            try:
                rate_limiter.check_rate_limit(
                    mock_request, 1000, 3600, "performance_test"
                )
            except RateLimitException:
                pass
