...
from app.core.async_session import close_database, get_async_db_context
...
async def _isolated_db():
    """Clean tables before each test and dispose the engine afterwards.
...
class TestBaseRepository:
    ...
    async def test_update_and_delete(self, user_repo):
        ...
        updated_user = await user_repo.update(
            user.id, {"country": "CA", "is_premium": True}
        )
...
class TestTransactionRepository:
    ...
    async def test_user(self, user_repo):
        ...
        user_data = {"email": "transaction_test@example.com", "password_hash": "hash"}
        user = await user_repo.create_user(user_data)
        yield user
        # Cleanup
        await user_repo.delete(user.id)
...
class TestExpenseRepository:
    ...
    async def test_user(self, user_repo):
        ...
        user_data = {"email": "expense_test@example.com", "password_hash": "hash"}
        user = await user_repo.create_user(user_data)
        yield user
        await user_repo.delete(user.id)
...
class TestGoalRepository:
    ...
    async def test_user(self, user_repo):
        ...
        user_data = {"email": "goal_test@example.com", "password_hash": "hash"}
        user = await user_repo.create_user(user_data)
        yield user
        await user_repo.delete(user.id)
