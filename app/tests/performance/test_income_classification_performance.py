...
class IncomeClassificationPerformanceTests:
    ...
    def test_core_cohort_analyzer_single_classification_performance(
        ...
        assert (
            perf_stats["p95_ms"] <= self.CLASSIFICATION_MAX_MS * 2
        ), f"95th percentile too slow: {perf_stats['p95_ms']:.3f}ms"

        assert (
            perf_stats["p99_ms"] <= self.CLASSIFICATION_MAX_MS * 3
        ...
        print(
            f"   Mean: {perf_stats['mean_ms']:.3f}ms (target: {self.CLASSIFICATION_TARGET_MS}ms)"
        )
    ...
    def test_bulk_classification_performance(
        ...
        print(
            f"   Per item: {per_item_ms:.3f}ms (target: {self.BULK_OPERATION_TARGET_MS}ms)"
        )
        print(
            f"   Throughput: {len(bulk_profiles) / (perf_stats['mean_ms'] / 1000):.0f} classifications/second"
        )
    ...
    def test_multi_state_classification_performance(
        ...
        for state, profile_data in mock_country_profiles.items():
            ...
            perf_stats = self.measure_performance(
                classify_state_profiles, iterations=100
            )
            ...
            assert (
                per_item_ms <= self.CLASSIFICATION_MAX_MS
        ...
        assert (
            performance_variance / average_time <= 0.5
        ...
        for state, result in state_results.items():
            print(
                f"   {state}: {result['per_item_ms']:.3f}ms per item ({result['profiles_tested']} profiles)"
            )
    ...
    def test_analyzer_consistency_performance(
        ...
        for name, analyzer in analyzers.items():
            ...
            assert (
                perf_stats["mean_ms"] <= self.CLASSIFICATION_MAX_MS
    ...
    @patch("app.config.country_profiles_loader.get_profile")
    def test_budget_generation_performance(
        self, mock_get_profile, mock_country_profiles
    ):
        """
        ...
        print(f"   P95:  {perf_stats['p95_ms']:.3f}ms")
        print(
            "   Includes: Income classification, threshold lookup, budget calculation"
        )

    def test_memory_usage_during_classification(
        self, mock_country_profiles, test_profiles
    ):
        """
        ...
        with patch(
            "app.config.country_profiles_loader.get_profile"
        ) as mock_get_profile:
            mock_get_profile.return_value = mock_country_profiles["US-CA"]
    ...
    def test_concurrent_classification_performance(
        ...
        for result in concurrency_results:
            print(
                f"   {result['concurrent_tasks']:2d} tasks: "
