...
class MITALoadTestRunner:
    ...
    def _run_locust_test(
        ...
        try:
            ...
            return {"test_name": test_name, "success": False, "error": "Test timed out"}
    ...
    def _print_test_summary(self, result: Dict[str, Any]):
        ...
        print(
            f"   Average Response Time: {metrics.get('average_response_time', 0):.1f}ms"
        )
        ...
        if (
            metrics.get("failure_rate", 0) < 1
            and metrics.get("average_response_time", 0) < 1000
        ):
    ...
    def generate_comprehensive_report(self):
        ...
        report_file = os.path.join(
            self.report_dir, f"comprehensive_report_{timestamp}.json"
        )
    ...
    def _generate_performance_summary(self) -> Dict[str, Any]:
        ...
        # Aggregate metrics
        total_requests = sum(
            r["metrics"].get("total_requests", 0) for r in successful_tests
        )
        total_failures = sum(
            r["metrics"].get("total_failures", 0) for r in successful_tests
        )
        avg_response_times = [
            r["metrics"].get("average_response_time", 0)
            for r in successful_tests
            if r["metrics"].get("average_response_time", 0) > 0
        ]
        ...
        return {
            ...
            "average_response_time_across_tests": sum(avg_response_times)
            / max(len(avg_response_times), 1),
            "best_performing_test": min(
                successful_tests,
                key=lambda x: x["metrics"].get("average_response_time", float("inf")),
            )["test_name"],
            "worst_performing_test": max(
                successful_tests,
                key=lambda x: x["metrics"].get("average_response_time", 0),
            )["test_name"],
        }
    ...
    def _generate_recommendations(self) -> List[str]:
        ...
        high_failure_rate_tests = [
            r for r in successful_tests if r["metrics"].get("failure_rate", 0) > 5
        ]
        ...
        if auth_tests and any(
            r["metrics"].get("average_response_time", 0) > 500 for r in auth_tests
        ):
            recommendations.append(
        ...
        financial_tests = [
            r for r in successful_tests if "financial" in r["test_name"].lower()
        ]
        if financial_tests and any(
            r["metrics"].get("average_response_time", 0) > 200 for r in financial_tests
        ):
            recommendations.append(
    ...
    def _print_comprehensive_summary(self, report: Dict[str, Any]):
        ...
        print(
            f"Success Rate: {(report['successful_tests'] / max(report['total_tests_run'], 1)) * 100:.1f}%"
        )
        ...
        if "error" not in summary:
            ...
            print(
                f"   Average Response Time: {summary['average_response_time_across_tests']:.1f}ms"
            )
...
def main():
    ...
    parser.add_argument(
        "--host", default="http://localhost:8000", help="Base URL for the API server"
    )
    parser.add_argument(
        "--test",
        choices=[
    ...
    parser.add_argument(
        "--skip-smoke", action="store_true", help="Skip smoke test when running 'all'"
    )
