...
class MITAMobileUser(HttpUser):
    ...
    def login_user(self):
        ...
        headers = {"User-Agent": "MITA-Mobile/1.0 (iOS 15.0)", "X-Platform": "mobile"}

        with self.client.post(
            "/api/auth/login", json=login_data, headers=headers
        ) as response:
            if response.status_code == 200:
                data = response.json()
                self.auth_token = data.get("access_token")
                ...
                self.client.post(
                    "/api/auth/register", json=self.user_data, headers=headers
                )
                self.login_user()
...
def on_test_start(environment, **kwargs):
    ...
    if isinstance(environment.runner, MasterRunner):
        logger.info(
            f"Master runner starting with {environment.runner.worker_count} workers"
        )
...
def on_test_stop(environment, **kwargs):
    ...
    report = {
        "test_duration": (
            stats.last_request_timestamp - stats.start_time
            if stats.last_request_timestamp
            else 0
        ),
            ...
            report["critical_endpoints"].append(
                {
                    ...
                    "failure_rate": (entry.num_failures / max(entry.num_requests, 1))
                    * 100,
    ...
    if report["failure_rate"] > 5:
        logger.error(
            f"RELIABILITY FAILURE: Failure rate {report['failure_rate']:.2f}% > 5%"
        )
    ...
    for endpoint in report["critical_endpoints"]:
        if endpoint["endpoint"] == "auth_login" and endpoint["avg_response_time"] > 500:
            logger.error(
                f"AUTH PERFORMANCE FAILURE: Login took {endpoint['avg_response_time']}ms"
            )

        if (
            endpoint["endpoint"] == "budget_dashboard"
            and endpoint["avg_response_time"] > 1000
        ):
            logger.error(
                f"DASHBOARD PERFORMANCE FAILURE: Dashboard took {endpoint['avg_response_time']}ms"
            )
