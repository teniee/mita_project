...
class DatabasePerformanceTests:
    ...
    def _classify_operation_type(self, operation_name: str) -> str:
        ...
        if any(keyword in operation_lower for keyword in ["insert", "create", "add"]):
            ...
            for keyword in ["select", "query", "find", "lookup"]
        ):
            return "SELECT"
    ...
    async def test_user_authentication_query_performance(self, async_session_factory):
        ...
        benchmark = self.measure_database_operation_performance(
            user_lookup_operation, "user_authentication_lookup", iterations=1000
        )
    ...
    @pytest.mark.asyncio
    async def test_financial_transaction_insert_performance(
        self, async_session_factory, test_users
    ):
        """
        ...
        benchmark = self.measure_database_operation_performance(
            transaction_insert_operation, "financial_transaction_insert", iterations=500
        )
    ...
    @pytest.mark.asyncio
    async def test_bulk_transaction_query_performance(
        self, async_session_factory, test_users
    ):
        """
    ...
    @pytest.mark.asyncio
    async def test_complex_financial_analytics_query_performance(
        self, async_session_factory, test_users
    ):
        """
        ...
        benchmark = self.measure_database_operation_performance(
            complex_analytics_query, "complex_financial_analytics_query", iterations=50
        )
    ...
    async def test_concurrent_database_operations_performance(
        ...
        for result in concurrency_results:
            print(
                f"   {result['concurrent_users']:2d} users: "
        ...
        async def rapid_connection_operations():
            ...
            for i in range(50):  # 50 rapid operations
                async with async_session_factory() as session:
                    ...
                    result = await session.execute(select(func.now()))  # Simple query
    ...
    @pytest.mark.asyncio
    async def test_database_query_plan_optimization(
        self, async_session_factory, test_users
    ):
        """
        ...
        async with async_session_factory() as session:
            ...
            critical_queries = [
                ...
                # Recent transactions query (test-only interpolation of our own
                # fixture's integer id — not user input)
                f"SELECT * FROM transactions WHERE user_id = {user.id} "  # nosec B608
                "ORDER BY date DESC LIMIT 20",
                # Monthly spending aggregation
                f"SELECT category, SUM(amount) as total "  # nosec B608
                f"FROM transactions WHERE user_id = {user.id} "
    ...
    @pytest.mark.asyncio
    async def test_database_memory_usage_under_load(
        self, async_session_factory, test_users
    ):
        """
        ...
        for cycle in range(20):  # 20 cycles of operations
            users_batch = test_users[
                cycle
                % len(test_users) : min((cycle % len(test_users)) + 5, len(test_users))
            ]
