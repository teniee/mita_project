...
class MemoryResourceTests:
    ...
    def detect_memory_leaks(
        self, operation_func, operation_name: str, iterations: int = None
    ) -> MemoryLeakDetection:
        """
        ...
        memory_growth_mb = (
            final_snapshot.memory_rss_mb - baseline_snapshot.memory_rss_mb
        )
    ...
    def test_income_classification_memory_leaks(self):
        ...
        with patch("app.config.country_profiles_loader.get_profile") as mock_profile:
            ...
            leak_result = self.detect_memory_leaks(
                classification_operation, "income_classification"
            )
            ...
            print(
                f"   Leak Status: {'NONE DETECTED' if not leak_result.potential_leak else 'POTENTIAL LEAK'}"
            )
    ...
    def test_cache_memory_management(self):
        ...
        leak_result = self.detect_memory_leaks(
            cache_operations, "cache_operations", iterations=50  # 50 cache cycles
        )
    ...
    def test_performance_monitor_memory_impact(self, performance_monitor):
        ...
        monitoring_growth = (
            with_monitoring.memory_rss_mb - without_monitoring.memory_rss_mb
        )
    ...
    def _generate_memory_recommendations(
        self, snapshot: ResourceUsageSnapshot
    ) -> List[str]:
