...
class SecurityPerformanceTests:
    ...
    def measure_operation_performance(
        self, operation_func, iterations: int = None, measure_memory: bool = True
    ) -> Dict[str, float]:
        """
        ...
        return {
            ...
            "throughput_ops_per_sec": (
                iterations / (sum(times) / 1000) if sum(times) > 0 else 0
            ),
        }
    ...
    def test_rate_limiter_performance_impact(self, mock_redis, mock_request):
        ...
        with patch("app.core.security.redis_client", mock_redis):
            ...
            rate_limited_perf = self.measure_operation_performance(
            ...
            throughput_impact = (
                (
                    baseline_perf["throughput_ops_per_sec"]
                    - rate_limited_perf["throughput_ops_per_sec"]
                )
                / baseline_perf["throughput_ops_per_sec"]
            ) * 100
    ...
    def test_token_blacklist_performance_impact(self, mock_redis):
        ...
        with patch("app.services.token_security_service.redis_client", mock_redis):
            ...
            baseline_perf = self.measure_operation_performance(
                baseline_token_validation
            )
            ...
            blacklist_perf = self.measure_operation_performance(
                token_validation_with_blacklist
            )
            ...
            print(f"   Overhead: {overhead_ms:.3f}ms ({overhead_percent:.1f}%)")
            print(
                f"   Blacklist Throughput: {blacklist_perf['throughput_ops_per_sec']:.0f} checks/sec"
            )
    ...
    def test_audit_logging_performance_impact(self):
        ...
        with patch("app.core.audit_logging.logger"):
            audit_perf = self.measure_operation_performance(
                financial_operation_with_audit
            )
    ...
    def test_comprehensive_auth_security_performance(self, mock_redis, mock_request):
        ...
        with patch("app.core.security.redis_client", mock_redis):
            ...
            comprehensive_perf = self.measure_operation_performance(
                comprehensive_auth_check
            )
    ...
    async def test_concurrent_security_operations_performance(self, mock_redis):
        ...
        with patch("app.core.security.redis_client", mock_redis):
            ...
            async def concurrent_rate_limiting_test(concurrent_requests: int):
                ...
                async def single_rate_check(request):
                    return rate_limiter.check_rate_limit(
                        request, 100, 60, "concurrent_test"
                    )
            ...
            for concurrent in [1, 10, 25, 50]:
                ...
                assert (
                    result["per_request_ms"] <= self.MAX_RATE_LIMITER_OVERHEAD_MS * 2
                ), (
            ...
            for result in concurrency_results:
                print(
                    f"   {result['concurrent_requests']:2d} concurrent: "
    ...
    def test_security_health_monitoring_performance(self, mock_redis):
        ...
        with patch("app.core.security.redis_client", mock_redis):
            ...
            print(
                f"   Throughput: {perf_stats['throughput_ops_per_sec']:.0f} checks/sec"
            )
    ...
    def test_security_features_comprehensive_impact_assessment(self):
        ...
        # Overall assessment
        assert (
            impact_report["combined_worst_case_overhead"] <= 50.0
