...
class AuthenticationPerformanceTests:
    ...
    def test_password_verification_performance(self):
        ...
        perf_stats = self.measure_auth_performance(
            verify_password_operation, iterations=200
        )
    ...
    def test_jwt_token_generation_performance(self, auth_test_data):
        ...
        assert (
            access_perf["mean_ms"] <= self.TOKEN_GENERATION_MAX_MS
        ...
        assert (
            refresh_perf["mean_ms"] <= self.TOKEN_GENERATION_MAX_MS
    ...
    def test_jwt_token_validation_performance(self, auth_test_data):
        ...
        assert (
            perf_stats["p95_ms"] <= self.TOKEN_VALIDATION_MAX_MS * 1.5
        ), f"JWT validation P95 too slow: {perf_stats['p95_ms']:.3f}ms"

        print("\n✅ JWT Validation Performance:")
        print(
            f"   Mean: {perf_stats['mean_ms']:.3f}ms (target: {self.TOKEN_VALIDATION_TARGET_MS}ms)"
        )
        print(f"   P95:  {perf_stats['p95_ms']:.3f}ms")
    ...
    @pytest.mark.asyncio
    async def test_login_flow_performance(
    ...
    @pytest.mark.asyncio
    async def test_registration_flow_performance(
        self, mock_database_session, auth_test_data
    ):
        """
        ...
        print(f"   P95:  {p95_time:.3f}ms")
        print(
            "   Includes: Validation, password hashing, DB operations, token generation"
        )
    ...
    def test_token_security_service_performance(self):
        ...
        with patch("app.services.token_security_service.redis_client") as mock_redis:
            ...
            assert (
                blacklist_perf["mean_ms"] <= MAX_SECURITY_OVERHEAD_MS
            ), f"Token blacklist check too slow: {blacklist_perf['mean_ms']:.3f}ms"

            assert (
                tracking_perf["mean_ms"] <= MAX_SECURITY_OVERHEAD_MS
            ...
            print(
                f"   Blacklist throughput: {blacklist_perf['throughput']:.0f} checks/sec"
            )
