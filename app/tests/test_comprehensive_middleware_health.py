...
class TestMiddlewareHealthMonitor:
    ...
    async def test_comprehensive_health_check_healthy_system(
        self, health_monitor, mock_session
    ):
        """Test health check on a healthy system"""
        # Run comprehensive health check
        report = await health_monitor.run_comprehensive_middleware_health_check(
            mock_session
        )
    ...
    async def test_health_check_detects_slow_database(
        ...
        assert any(
            "database" in alert.lower() or "timeout" in alert.lower()
            for alert in report.alerts
        )

    async def test_health_check_detects_system_resource_issues(
        self, health_monitor, mock_session
    ):
        ...
        with patch("psutil.cpu_percent") as mock_cpu, patch(
            ...
            assert len(report.alerts) > 0
            assert any(
                "cpu" in alert.lower() or "memory" in alert.lower()
                for alert in report.alerts
            )
    ...
    async def test_health_check_timeout_detection(self, health_monitor, mock_session):
        ...
        with patch(
            ...
            timeout_alerts = [
                alert
                for alert in report.alerts
                if "timeout" in alert.lower() or "critical" in alert.lower()
            ]

            assert (
                timeout_risk_detected or len(timeout_alerts) > 0
    ...
    def test_health_history_tracking(self, health_monitor):
        ...
        for i in range(5):
            ...
            report.overall_status = (
                HealthStatus.HEALTHY if i < 3 else HealthStatus.DEGRADED
            )
    ...
    def test_performance_threshold_configuration(self, health_monitor):
        ...
        assert (
            thresholds["response_time_critical"] <= 5000
        ), "Critical threshold too high to detect timeout risk"
        assert (
            thresholds["database_connection_critical"] <= 3000
        ), "DB threshold too high"
        ...
        assert (
            thresholds["response_time_warning"] <= 2000
        ), "Warning threshold should be lower"
        assert (
            thresholds["database_connection_warning"] <= 1000
        ), "DB warning threshold should be lower"
...
class TestSecurityMiddlewareHealthChecker:
    ...
    async def test_security_event_logging_health(self, security_checker):
        ...
        with patch("app.core.audit_logging.AuditLogger") as mock_logger, patch(
            "os.path.exists"
        ) as mock_exists, patch("os.access") as mock_access:
...
class TestDatabaseMiddlewareHealthChecker:
    ...
    async def test_connection_pool_health_check(
        self, database_checker, mock_session_advanced
    ):
        """Test database connection pool health check"""
        metric = await database_checker.check_connection_pool_health(
            mock_session_advanced
        )
    ...
    async def test_database_performance_degradation_detection(
        self, database_checker, mock_session_advanced
    ):
    ...
    async def test_query_performance_check(
        self, database_checker, mock_session_advanced
    ):
...
class TestCacheMiddlewareHealthChecker:
    ...
    async def test_redis_cache_health_check_with_redis(self, cache_checker):
        ...
        assert details["data_integrity"] is True
    ...
    async def test_redis_cache_slow_response_detection(self, cache_checker):
        ...
        mock_redis.get.side_effect = lambda key: "health_test_value_" + str(
            int(time.time())
        )
...
class TestHealthAlertManager:
    ...
    async def test_alert_generation_for_unhealthy_system(self, alert_manager):
        ...
        system_alerts = [
            alert for alert in triggered_alerts if "system" in alert.title.lower()
        ]
    ...
    def test_alert_cooldown_mechanism(self, alert_manager):
        ...
        # Set cooldown
        alert_manager.cooldown_tracker[rule_name] = datetime.now(
            timezone.utc
        ) + timedelta(minutes=5)
...
class TestHealthCheckIntegration:
    ...
    async def test_mobile_app_integration_compatibility(self):
        ...
        for field in required_fields:
            ...
            ), f"Missing required field for mobile app: {field}"
