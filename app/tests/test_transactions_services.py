...
def test_parse_receipt_image(monkeypatch, tmp_path):
    ...
    class DummyService:
        def process_image(self, path, is_premium_user=False):
            called["args"] = (path, is_premium_user)
            return {"store": "Test", "amount": 1.0}

    monkeypatch.setattr(
        "app.api.transactions.services.AdvancedOCRService",
        lambda: DummyService(),
    )

    fake_file = SimpleNamespace(filename="test.jpg", file=io.BytesIO(b"123"))
    result = parse_receipt_image(fake_file, is_premium_user=True)
