...
class TestDynamicThresholdEconomicSoundness:
    ...
    def test_savings_rates_increase_with_income(self):
        ...
        assert (
            savings_rates["high_income_executive"] > savings_rates["low_income_single"]
        )
        assert (
            savings_rates["upper_middle_california"]
            > savings_rates["lower_middle_family"]
        )
    ...
    def test_regional_cost_adjustments(self):
        ...
        base_context = UserContext(
            monthly_income=7000, age=35, family_size=2, debt_to_income_ratio=0.15
        )
        ...
        assert (
            housing_ratios["US-CA"] > housing_ratios["US-TX"]
        ), "California should have higher housing ratio than Texas"
        assert (
            housing_ratios["US-NY"] > housing_ratios["US"]
    ...
    def test_life_stage_adjustments(self):
        ...
        contexts = {
            "young_single": UserContext(
                monthly_income=base_income, age=25, life_stage="single", family_size=1
            ),
            "family": UserContext(
                monthly_income=base_income, age=35, life_stage="family", family_size=4
            ),
            "senior": UserContext(
        ...
        for life_stage, context in contexts.items():
            ...
            if life_stage == "family":
                ...
                assert (
                    priorities["healthcare"] <= 5
                ), "Families should prioritize healthcare"
                assert (
                    priorities.get("education", 10) <= 9
                ), "Families should prioritize education"

            elif life_stage == "senior":
                # Seniors should prioritize healthcare
                assert (
                    priorities["healthcare"] <= 3
                ), "Seniors should highly prioritize healthcare"
    ...
    def test_debt_impact_on_allocations(self):
        ...
        low_debt_allocations = self.service.get_budget_allocation_thresholds(
            low_debt_context
        )
        high_debt_allocations = self.service.get_budget_allocation_thresholds(
            high_debt_context
        )
        ...
        if "debt_payment" in high_debt_allocations:
            assert (
                high_debt_allocations["debt_payment"] > 0.2
    ...
    def test_spending_pattern_thresholds_scale_with_income(self):
        ...
        for profile_name in profiles:
            ...
            small_purchase_thresholds[profile_name] = thresholds[
                "small_purchase_threshold"
            ]
            large_purchase_thresholds[profile_name] = thresholds[
                "large_purchase_threshold"
            ]

        # Higher income should have higher thresholds
        assert (
            small_purchase_thresholds["high_income_executive"]
            > small_purchase_thresholds["low_income_single"]
        )

        assert (
            large_purchase_thresholds["high_income_executive"]
        ...
        for profile_name in profiles:
            assert (
                large_purchase_thresholds[profile_name]
    ...
    def test_dynamic_budget_method_appropriateness(self):
        ...
        for profile_name, user_context in self.user_profiles.items():
            ...
            assert (
                30 <= needs_pct <= needs_cap
            ), f"Needs % for {profile_name} should be 30-{needs_cap}%, got {needs_pct}"
            assert (
                5 <= wants_pct <= 40
            ), f"Wants % for {profile_name} should be 5-40%, got {wants_pct}"
            assert (
                2 <= savings_pct <= 30
            ), f"Savings % for {profile_name} should be 2-30%, got {savings_pct}"

            # Total should be reasonable (allowing for investments category)
            total_pct = needs_pct + wants_pct + savings_pct
            assert (
                80 <= total_pct <= 120
            ), f"Total % for {profile_name} should be 80-120%, got {total_pct}"
            ...
            if user_context.monthly_income < 4000:
                assert method in [
                    "needs_first"
                ], f"Low income should use needs_first method, got {method}"
            elif user_context.monthly_income > 15000:
                assert method in [
                    "wealth_building"
                ], f"High income should use wealth_building method, got {method}"
    ...
    def test_cooldown_periods_make_economic_sense(self):
        ...
        for profile_name, user_context in self.user_profiles.items():
            ...
            assert (
                cooldowns.get("groceries", 0) <= 1
            ), "Groceries should have minimal cooldown"
            assert (
                cooldowns.get("transport", 0) == 0
            ), "Transport should have no cooldown"
            assert (
                cooldowns.get("healthcare", 0) == 0
            ), "Healthcare should have no cooldown"

            # Discretionary categories should have meaningful cooldowns
            assert (
                cooldowns.get("entertainment", 0) >= 1
            ), "Entertainment should have some cooldown"
            assert (
                cooldowns.get("clothing", 0) >= 3
            ), "Clothing should have meaningful cooldown"
    ...
    def test_economic_environment_adjustments(self):
        ...
        service_high_inflation.economic_context.recession_risk = (
            0.4  # High recession risk
        )

        normal_allocations = self.service.get_budget_allocation_thresholds(base_context)
        crisis_allocations = service_high_inflation.get_budget_allocation_thresholds(
            base_context
        )
        ...
        if "savings" in normal_allocations and "savings" in crisis_allocations:
            assert (
                crisis_allocations["savings"] >= normal_allocations["savings"]
    ...
    def test_threshold_consistency_validation(self):
        ...
        assert validate_threshold_consistency(
            valid_budget_thresholds, self.user_profiles["middle_income_couple"]
        )

        # Test invalid thresholds (sum too high)
        invalid_thresholds = {"housing": 0.6, "food": 0.3, "savings": 0.3, "other": 0.2}
        assert not validate_threshold_consistency(
            invalid_thresholds, self.user_profiles["middle_income_couple"]
        )
    ...
    def test_income_scaling_smoothness(self, income_level):
        ...
        context = UserContext(
            monthly_income=income_level, age=35, region="US", family_size=2
        )
        ...
        assert (
            0.8 <= total_budget <= 1.2
        ), f"Budget total should be reasonable for income {income_level}"
...
class TestAPIEndpointIntegration:
    ...
    def test_api_response_structure(self):
        ...
        for field in required_fields:
            assert (
                field in budget_method
            ), f"Budget method response missing field: {field}"
