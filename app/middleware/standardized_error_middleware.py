                        ...
                        if isinstance(body, bytes):
                            ...
                            if not self._is_standardized_response(body_content):
                                ...
                                standardized_error = StandardizedAPIException(
                                    message=body_content.get(
                                        "detail", "An error occurred"
                                    ),
                                    error_code=self._map_status_to_error_code(
...
class ResponseValidationMiddleware(BaseHTTPMiddleware):
        ...
        try:
            ...
            if hasattr(response, "body") and response.body:
                ...
                validation_issues = self._validate_response_structure(
                    body_content, response.status_code
                )
    ...
    def _validate_response_structure(
        self, response_content: dict, status_code: int
    ) -> list[str]:
        """
        ...
        if success is True:
            ...
            for field in required_error_fields:
                if field not in error:
                    issues.append(
                        f"Error response missing required field: error.{field}"
                    )
