...
class ComprehensiveRateLimitMiddleware(BaseHTTPMiddleware):
    ...
    def __init__(self, app, enable_rate_limiting: bool = True):
        ...
        self.admin_paths = {"/admin/", "/auth/security/status"}
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        try:
            ...
            if getattr(SecurityConfig, "RATE_LIMIT_FAIL_SECURE", True) and any(
                request.url.path.startswith(auth_path) for auth_path in self.auth_paths
            ):
    ...
    async def _apply_auth_rate_limiting(self, request: Request, user_tier: str) -> None:
        ...
        limits = {
            "anonymous": {
                "limit": 20,
                "window": 300,
            },  # 20 requests per 5 minutes (increased from 10)
    ...
    async def _apply_admin_rate_limiting(
        ...
        if user_tier != "admin_user":
            ...
            self.rate_limiter.check_rate_limit(
                request, 15, 3600, "admin_non_admin"
            )  # Increased from 5
        else:
            # Admin users have higher limits
            self.rate_limiter.check_rate_limit(
                request, 200, 3600, "admin_admin"
            )  # Increased from 100

    async def _apply_general_rate_limiting(
        self, request: Request, user_tier: str
    ) -> None:
        """Apply general API rate limiting"""

        tier_config = SecurityConfig.RATE_LIMIT_TIERS.get(
            user_tier, SecurityConfig.RATE_LIMIT_TIERS["anonymous"]
        )
        ...
        try:
            ...
            try:
                status = self.rate_limiter.get_rate_limit_status(
                    request, f"general_{user_tier}"
                )
                if "api_requests" in status:
                    remaining = max(
                        0,
                        tier_config["requests_per_hour"]
                        - status["api_requests"]["count"],
                    )
    ...
    def _create_rate_limit_response(
        self, request: Request, detail: str
    ) -> JSONResponse:
