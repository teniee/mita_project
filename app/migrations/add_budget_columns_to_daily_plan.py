...
def upgrade():
    ...
    op.add_column(
        "daily_plan", sa.Column("planned_amount", sa.Numeric(12, 2), nullable=True)
    )

    # Add spent_amount column for actual spending
    op.add_column(
        "daily_plan", sa.Column("spent_amount", sa.Numeric(12, 2), nullable=True)
    )

    # Add daily_budget column for total daily budget limit
    op.add_column(
        "daily_plan", sa.Column("daily_budget", sa.Numeric(12, 2), nullable=True)
    )
    ...
    op.execute(
        "UPDATE daily_plan SET planned_amount = 0.00 WHERE planned_amount IS NULL"
    )
    ...
    op.create_index(
        "idx_daily_plan_user_date_category",
        "daily_plan",
