...
def upgrade() -> None:
    ...
    op.add_column(
        "transactions", sa.Column("tags", postgresql.ARRAY(sa.String()), nullable=True)
    )
    ...
    op.add_column(
        "transactions", sa.Column("confidence_score", sa.Float(), nullable=True)
    )

    # Add receipt_url column
    op.add_column(
        "transactions", sa.Column("receipt_url", sa.String(500), nullable=True)
    )
