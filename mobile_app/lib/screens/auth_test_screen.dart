...
class _AuthTestScreenState extends State<AuthTestScreen> {
  ...
  Future<void> _testLoginEndpoint() async {
    ...
    try {
      ...
      if (e is DioException && e.response?.statusCode == 401) {
        ...
        _addTestResult('Login Endpoint', true, 'Endpoint accessible (status: ${e.response?.statusCode})');
    ...
    try {
      ...
      if (e is DioException) {
        ...
        if (statusCode == 409 || statusCode == 422) {
          _addTestResult('Registration Endpoint', true, 'Endpoint accessible (conflict as expected)');
        } else if (statusCode == 400) {
          _addTestResult('Registration Endpoint', true, 'Endpoint accessible (validation working)');
        } else if (statusCode != null) {
          _addTestResult('Registration Endpoint', true, 'Endpoint accessible (status: $statusCode)');
        } else if (e.toString().contains('timeout') || e.toString().contains('TimeoutException')) {
  ...
  Future<void> _testTimeoutHandling() async {
    ...
    try {
      ...
      await testDio.post<Map<String, dynamic>>('/auth/login', data: {'email': 'test', 'password': 'test'});
      ...
      if (e is DioException && 
        ...
        _addTestResult('Timeout Handling', true, 'Error handling working (different error: ${e.runtimeType})');
      }
    }
  }
  ...
  Future<void> _testManualLogin() async {
    ...
    try {
      ...
      if (e is DioException && e.response?.statusCode == 401) {
        _addTestResult('Manual Login', true, 'Login endpoint working (401 for invalid credentials)');
      } else {
  ...
  Future<void> _testManualRegistration() async {
    ...
    try {
      ...
      _addTestResult('Manual Registration', true, 'Registration successful: ${response.statusCode}');
      ...
      if (e is DioException && (e.response?.statusCode == 409 || e.response?.statusCode == 422)) {
        _addTestResult('Manual Registration', true, 'Registration endpoint working (conflict expected)');
      } else {
