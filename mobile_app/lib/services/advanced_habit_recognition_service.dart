...
class AdvancedHabitRecognitionService {
  static final AdvancedHabitRecognitionService _instance =
  ...
  Future<double> predictHabitFormationRisk(
    ...
    final categoryTransactions =
        recentTransactions.where((t) => t['category'] == category).toList();
  ...
  Future<DetectedHabit?> _detectImpulseBuying(List<Map<String, dynamic>> transactions) async {
    ...
    if (nonEssentialSpending > 0.3) {
      indicators.add(
          'High spending in non-essential categories (${(nonEssentialSpending * 100).round()}%)');
      confidence += 0.3;
      impactScore += 0.2;
    }
  ...
  Map<String, dynamic> _generateInsights(
    ...
    return {
      ...
      'dataQuality': transactions.length >= 50
          ? 'good'
