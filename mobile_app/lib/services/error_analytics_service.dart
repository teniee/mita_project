...
class ErrorAnalyticsService {
  ...
  void recordError({
    ...
    try {
      ...
      // Update metrics
      final metrics = _errorMetrics[errorKey] ?? ErrorMetrics(
        errorKey: errorKey,
  ...
  List<ErrorPattern> getErrorPatterns({Duration? timeWindow}) {
    ...
    for (final entry in patternCounts.entries) {
      if (entry.value >= 2) { // At least 2 occurrences
  ...
  ErrorImpactAssessment assessErrorImpact(String errorKey) {
    ...
    if (hoursSinceLastOccurrence < 1) {
      ...
      impactScore += 15;
    } else if (hoursSinceLastOccurrence < 168) { // 1 week
      impactScore += 10;
    } else {
      impactScore += 5;
    }
    ...
    final criticalCount = metrics.occurrences.where((o) => o.severity == ErrorSeverity.critical).length;
    ...
    final authCount = metrics.occurrences.where((o) => o.category == ErrorCategory.authentication).length;
  ...
  String _generateErrorKey(dynamic error, String? operationName) {
    ...
    coreMessage = coreMessage.replaceAll(RegExp(r'[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}'), 'UUID');
  ...
  Future<void> _loadStoredMetrics() async {
    try {
      ...
      if (metricsData != null) {
        ...
        logInfo('Loaded ${_errorMetrics.length} error metrics from storage', tag: 'ERROR_ANALYTICS');
      }
    } catch (e) {
