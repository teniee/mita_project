...
class SmartCategorizationService {
  ...
  Future<List<CategorySuggestion>> _getAISuggestions({
    ...
    return suggestions.map((suggestion) => CategorySuggestion(
  ...
  List<CategorySuggestion> _applyUserLearning(
    ...
    return suggestions.map((suggestion) {
      ...
      return CategorySuggestion(
        ...
        reason: suggestion.reason + (confirmations > 0 ? ' (User confirmed $confirmations times)' : ''),
  ...
  List<CategorySuggestion> _getFallbackSuggestions(String description, double amount) {
    ...
    if (desc.contains('coffee') || desc.contains('starbucks') || desc.contains('cafe')) {
      suggestions.add(CategorySuggestion(category: 'Food & Dining', confidence: 0.8, reason: 'Coffee shop pattern', source: 'fallback'));
    } else if (desc.contains('gas') || desc.contains('fuel') || desc.contains('shell')) {
      suggestions.add(CategorySuggestion(category: 'Transportation', confidence: 0.8, reason: 'Gas station pattern', source: 'fallback'));
    } else if (desc.contains('grocery') || desc.contains('supermarket') || desc.contains('walmart')) {
      suggestions.add(CategorySuggestion(category: 'Food & Dining', confidence: 0.8, reason: 'Grocery pattern', source: 'fallback'));
    } else if (desc.contains('restaurant') || desc.contains('dining') || desc.contains('pizza')) {
      suggestions.add(CategorySuggestion(category: 'Food & Dining', confidence: 0.8, reason: 'Restaurant pattern', source: 'fallback'));
    } else if (desc.contains('movie') || desc.contains('cinema') || desc.contains('netflix')) {
      suggestions.add(CategorySuggestion(category: 'Entertainment', confidence: 0.7, reason: 'Entertainment pattern', source: 'fallback'));
    } else {
      suggestions.add(CategorySuggestion(category: 'General', confidence: 0.3, reason: 'Default category', source: 'fallback'));
    }
  ...
  void _updateAmountPattern(double amount, String category) {
    ...
    _amountPatterns[amountRange]![category] = (_amountPatterns[amountRange]![category] ?? 0.0) + 1.0;
  }
...
class CategorySuggestion {
  ...
  @override
  String toString() => 'CategorySuggestion(category: $category, confidence: ${confidence.toStringAsFixed(2)}, reason: $reason)';
}
