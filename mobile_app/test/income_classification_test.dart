  ...
  group('Income Classification Tests', () {
    ...
    group('Boundary Value Testing', () {
      test('California boundary values', () {
        ...
        expect(
            incomeService.classifyIncomeForLocation(44935 / 12, 'US',
                stateCode: 'CA'),
            equals(IncomeTier.low),
            reason:
                'Income exactly at low threshold should be classified as low');

        expect(
            incomeService.classifyIncomeForLocation(44936 / 12, 'US',
                stateCode: 'CA'),
            equals(IncomeTier.lowerMiddle),
            reason: 'Income just above low threshold should be lowerMiddle');

        expect(
            incomeService.classifyIncomeForLocation(71896 / 12, 'US',
                stateCode: 'CA'),
            equals(IncomeTier.lowerMiddle),
            reason:
                'Income exactly at lower_middle threshold should be lowerMiddle');

        expect(
            incomeService.classifyIncomeForLocation(71897 / 12, 'US',
                stateCode: 'CA'),
            equals(IncomeTier.middle),
            reason:
                'Income just above lower_middle threshold should be middle');
      });
      ...
      test('Mississippi vs California comparison', () {
        ...
        final msClassification = incomeService
            .classifyIncomeForLocation(monthlyIncome, 'US', stateCode: 'MS');
      ...
      test('All states have valid thresholds', () {
        ...
        expect(states.length, equals(50),
            reason: 'Should have all 50 US states');
    ...
    group('Input Validation Tests', () {
      test('Handle negative income', () {
        expect(
            () => incomeService.classifyIncomeForLocation(-1000, 'US',
                stateCode: 'CA'),
            returnsNormally,
            reason: 'Should handle negative income gracefully');

        final result = incomeService.classifyIncomeForLocation(-1000, 'US',
            stateCode: 'CA');
        expect(result, equals(IncomeTier.low),
      ...
      test('Handle extremely large income', () {
        final result = incomeService.classifyIncomeForLocation(1000000, 'US',
            stateCode: 'CA');
    ...
    group('Performance Tests', () {
      test('Classification performance benchmark', () {
        ...
        for (int i = 0; i < 1000; i++) {
          incomeService.classifyIncomeForLocation(3000.0 + (i * 10), 'US',
              stateCode: 'CA');
        }
    ...
    group('Data Consistency Tests', () {
      ...
      test('Income range string accuracy', () {
        ...
        final locationRange = incomeService
            .getIncomeRangeStringForLocation(tier, 'US', stateCode: 'CA');
    ...
    group('Edge Cases', () {
      test('State with lowest thresholds (MS)', () {
        ...
        final thresholds =
            countryService.getIncomeThresholds('US', stateCode: 'MS');
        expect(thresholds['low'], equals(26000));

        // Test classification
        final result = incomeService.classifyIncomeForLocation(2500, 'US',
            stateCode: 'MS');
        expect(
            result, equals(IncomeTier.lowerMiddle)); // 2500*12 = 30000 > 26000
      });
