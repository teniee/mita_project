...
/// MITAApp is built under a MultiProvider in main(); mirror that here.
Widget buildApp() => MultiProvider(
      providers: buildTestProviders(),
      child: const MITAApp(),
    );

/// The welcome screen runs a timed init sequence on startup; advance the
/// fake clock past it so no timers are left pending at teardown.
Future<void> flushStartupTimers(WidgetTester tester) async {
...
void main() {
  ...
  group('Performance Tests', () {
    group('App Startup Performance', () {
      testWidgets('App should start quickly', (WidgetTester tester) async {
        ...
        try {
          ...
          await flushStartupTimers(tester);
      ...
      testWidgets('App should handle initial API failures gracefully',
    ...
    group('Memory and Resource Management', () {
      testWidgets('App should not leak widgets during navigation',
          (WidgetTester tester) async {
        await tester.pumpWidget(buildApp());
        await flushStartupTimers(tester);
        await tester.pump();
        ...
        expect(widgetGrowth, lessThan(100),
            reason:
                'Widget count should not grow excessively during navigation');
      });
    });

    group('UI Responsiveness', () {
      testWidgets('UI should remain responsive during data loading',
          (WidgetTester tester) async {
        await tester.pumpWidget(buildApp());
        await flushStartupTimers(tester);
      ...
      testWidgets('Scroll performance should be smooth',
        ...
        if (scrollableWidgets.isNotEmpty) {
          final scrollableFinder = scrollViews.evaluate().isNotEmpty
              ? scrollViews.first
    ...
    group('Error Recovery Performance', () {
      testWidgets('App should recover quickly from network errors',
          (WidgetTester tester) async {
        await tester.pumpWidget(buildApp());
        await flushStartupTimers(tester);
        await tester.pump();
    ...
    group('Animation Performance', () {
      testWidgets('Animations should not cause frame drops',
          (WidgetTester tester) async {
        await tester.pumpWidget(buildApp());
        await flushStartupTimers(tester);
        await tester.pump();
    ...
    group('Resource Cleanup', () {
      testWidgets('App should clean up resources properly',
          (WidgetTester tester) async {
        await tester.pumpWidget(buildApp());
        await flushStartupTimers(tester);
        await tester.pump();
        ...
        for (int i = 0; i < 3; i++) {
          ...
          await tester.pumpWidget(buildApp());
          await flushStartupTimers(tester); // Remount
          await tester.pump();
        }
