...
void main() {
  group('Secure Token Storage Tests', () {
    ...
    // Stateful in-memory backing store so write -> read round-trips like the
    ...
    tearDown(() async {
      ...
      // clearAllUserData deliberately preserves device-level metadata (e.g.
      // last-rotation time); wipe the whole mock store so each test starts
      // from a genuinely fresh-install state.
      secureStore.clear();
  ...
  group('Token Lifecycle Manager Tests', () {
    ...
    test('should check if immediate action is required', () async {
      final requiresAction =
          await TokenLifecycleManager.instance.requiresImmediateAction();
