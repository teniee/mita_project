...
void main() {
  group('Financial Precision Edge Cases', () {
    ...
    group('Floating Point Precision Tests', () {
      test('Exact threshold boundary precision', () {
        ...
        // Test exact boundary
        final resultExact = incomeService
            .classifyIncomeForLocation(exactMonthlyLow, 'US', stateCode: 'CA');
        ...
        final resultAbove = incomeService
            .classifyIncomeForLocation((44935.01) / 12, 'US', stateCode: 'CA');
        ...
        final resultBelow = incomeService
            .classifyIncomeForLocation((44934.99) / 12, 'US', stateCode: 'CA');
      ...
      test('Recurring decimal precision handling', () {
        ...
        for (final testCase in recurringCases) {
          ...
          final tier2 = incomeService.classifyIncomeForLocation(
              monthlyFromAnnual, 'US',
              stateCode: 'CA');

          expect(tier1, equals(tier2),
              reason:
                  'Conversion precision should not affect classification for income $annual');
        }
      });
      ...
      test('Large number precision maintenance', () {
        ...
        for (final income in largeIncomes) {
          ...
          final tier = incomeService.classifyIncomeForLocation(monthly, 'US',
              stateCode: 'CA');
    ...
    group('State Boundary Edge Cases', () {
      test('Extreme state differences for same income', () {
        ...
        for (final state in states) {
          final thresholds =
              countryService.getIncomeThresholds('US', stateCode: state);
      ...
      test('All states handle extreme incomes consistently', () {
        ...
        for (final incomeCase in extremeIncomes) {
          ...
          for (final state in states.take(5)) {
            // Test first 5 states for performance
            expect(
            ...
            final tier = incomeService.classifyIncomeForLocation(income, 'US',
                stateCode: state);
            expect(tier, isA<IncomeTier>(),
                reason: 'Should return valid tier for $desc in $state');
          }
        }
      });
    });
    ...
    group('Data Validation Edge Cases', () {
      test('Invalid state codes fallback gracefully', () {
        ...
        for (final invalidState in invalidStates) {
          expect(
              () => incomeService.classifyIncomeForLocation(5000, 'US',
                  stateCode: invalidState),
              returnsNormally,
              reason: 'Should handle invalid state code: $invalidState');

          final tier = incomeService.classifyIncomeForLocation(5000, 'US',
              stateCode: invalidState);
          expect(tier, isA<IncomeTier>(),
              reason:
                  'Should return valid tier for invalid state $invalidState');
        }
      });
      ...
      test('Malformed input handling', () {
        final malformedInputs = [
          ...
          {
            'income': double.negativeInfinity,
            'desc': 'negative infinite income'
          },
        ];
      ...
      test('Threshold data integrity across all states', () {
        ...
        for (final state in states) {
          ...
          expect(thresholds['low']!, greaterThan(15000),
              reason:
                  'State $state low threshold unusually low: ${thresholds['low']}');
          expect(thresholds['low']!, lessThan(100000),
              reason:
                  'State $state low threshold unusually high: ${thresholds['low']}');
          expect(thresholds['upper_middle']!, lessThan(500000),
              reason:
                  'State $state upper_middle threshold unusually high: ${thresholds['upper_middle']}');
        }
      });
    });
  });
  ...
  group('Location Service Edge Cases', () {
    ...
    test('Handle location permission edge cases', () async {
      ...
      final supportedCountries =
          locationService.getSupportedCountriesForSelection();
    ...
    test('Location formatting edge cases', () {
      final testCases = [
        {
          'country': 'US',
          'state': 'CA',
          'expected': '🇺🇸 United States, California'
        },
        {
          'country': 'US',
          'state': 'TX',
          'expected': '🇺🇸 United States, Texas'
        },
        {
          'country': 'US',
          'state': 'INVALID',
          'expected': '🇺🇸 United States, INVALID'
        },
      ...
      for (final testCase in testCases) {
        ...
        expect(result, equals(testCase['expected']),
            reason:
                'Formatting failed for ${testCase['country']}, ${testCase['state']}');
      }
    });
  });
  ...
  group('Service Integration Edge Cases', () {
    ...
    test('Tier name and description consistency', () {
      for (final tier in IncomeTier.values) {
        ...
        expect(name.isNotEmpty, isTrue,
            reason: 'Tier $tier should have non-empty name');
        expect(description.isNotEmpty, isTrue,
            reason: 'Tier $tier should have non-empty description');
        expect((color.a * 255.0).round() & 0xff, greaterThan(0),
            reason: 'Tier $tier should have visible primary color');
        expect((secondaryColor.a * 255.0).round() & 0xff, greaterThan(0),
            reason: 'Tier $tier should have visible secondary color');
    ...
    test('Budget weight consistency across tiers', () {
      for (final tier in IncomeTier.values) {
        ...
        expect(totalWeight, greaterThan(0.8),
            reason: 'Budget weights too low for tier $tier');
        expect(totalWeight, lessThan(1.2),
            reason: 'Budget weights too high for tier $tier');

        // Verify all weights are positive
        for (final category in weights.keys) {
          expect(weights[category]!, greaterThan(0),
              reason:
                  'Category $category should have positive weight for tier $tier');
    ...
    test('Financial tips quality and consistency', () {
      for (final tier in IncomeTier.values) {
        ...
        expect(tips.isNotEmpty, isTrue,
            reason: 'Tier $tier should have financial tips');
        expect(tips.length, greaterThan(3),
            reason: 'Tier $tier should have sufficient tips');

        for (final tip in tips) {
          expect(tip.isNotEmpty, isTrue,
              reason: 'All tips should be non-empty for tier $tier');
          expect(tip.length, greaterThan(10),
              reason: 'Tips should be meaningful length for tier $tier');
    ...
    test('Goal suggestions appropriateness by tier', () {
      ...
      for (int i = 0;
          i < testIncomes.length && i < IncomeTier.values.length;
          i++) {
        ...
        expect(goals.isNotEmpty, isTrue,
            reason: 'Tier $tier should have goal suggestions');

        for (final goal in goals) {
          expect(goal['title'], isA<String>(),
              reason: 'Goal should have string title');
          expect(goal['description'], isA<String>(),
              reason: 'Goal should have string description');
          expect(goal['target_amount'], isA<double>(),
              reason: 'Goal should have numeric target');
          expect(goal['monthly_target'], isA<double>(),
              reason: 'Goal should have monthly target');
          expect(goal['priority'], isA<String>(),
              reason: 'Goal should have priority level');
          ...
          expect(targetAmount, greaterThan(0),
              reason: 'Goal target should be positive');
          expect(monthlyTarget, greaterThan(0),
              reason: 'Monthly target should be positive');
  ...
  group('Performance and Memory Edge Cases', () {
    ...
    test('Concurrent classification consistency', () {
      ...
      for (int i = 0; i < 100; i++) {
        final result = incomeService.classifyIncomeForLocation(testIncome, 'US',
            stateCode: 'CA');
        results.add(result);
      }
