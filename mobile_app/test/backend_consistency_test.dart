  ...
  group('Backend Consistency Tests', () {
    ...
    group('Data Format Consistency', () {
      ...
      test('Threshold key names are consistent', () {
        final thresholds =
            countryService.getIncomeThresholds('US', stateCode: 'CA');
        ...
        for (final state in testStates) {
          ...
          for (final monthlyIncome in testIncomes) {
            ...
            expect(frontendTierName, equals(expectedTier),
                reason:
                    'Classification mismatch for $monthlyIncome monthly in $state');
          }
        }
      });
    });
    ...
    group('Critical Backend Issues Detection', () {
      test('Detect outdated 3-tier system usage', () {
        ...
        for (final monthlyIncome in problematicMonthlyIncomes) {
          ...
          final fiveTierResult = incomeService
        ...
        expect(true, isTrue,
            reason: 'This test documents backend inconsistency');
      });
      ...
      test('Floating point consistency across conversions', () {
        ...
        for (final annualValue in testValues) {
          ...
          expect(precision, lessThan(0.01),
              reason:
                  'Precision loss of \$${precision.toStringAsFixed(2)} for \$$annualValue');

          // Test classification consistency
          final directClassification =
              _simulateBackendClassification(annualValue, 'CA');
          final convertedClassification =
              _simulateBackendClassification(backToAnnual, 'CA');

          expect(directClassification, equals(convertedClassification),
              reason:
                  'Classification changed due to conversion precision loss');
        }
      });
    ...
    group('API Contract Validation', () {
      ...
      test('Error handling for edge cases', () {
        ...
        expect(() => countryService.getIncomeThresholds('INVALID'),
            returnsNormally);
        expect(
            () =>
                countryService.getIncomeThresholds('US', stateCode: 'INVALID'),
            returnsNormally);
      });
    });
  });
