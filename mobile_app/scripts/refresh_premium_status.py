...
class SubscriptionManager:
    ...
    async def get_subscriptions_to_verify(
        self, limit: int = BATCH_SIZE
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    ...
                    if response.status != 200:
                        self.logger.error(
                            f"Apple verification failed with status {response.status}"
                        )
                        return False, {}
                    ...
                    if status == 0:
                        ...
                        if not self.config.get("use_sandbox", False):
                            self.logger.info(
                                "Receipt is sandbox, retrying with sandbox URL"
                            )
                            return await self._verify_apple_sandbox_receipt(
                                subscription
                            )
    ...
    async def _parse_apple_receipt(
        self, apple_response: Dict[str, Any], subscription: SubscriptionInfo
    ) -> Tuple[bool, Dict[str, Any]]:
        ...
        try:
            ...
            renewal_info = next(
                (
                    info
                    for info in pending_renewal_info
            ...
            return True, {
                ...
                "is_trial_period": matching_transaction.get("is_trial_period")
                == "true",
                "original_transaction_id": matching_transaction.get(
                    "original_transaction_id"
                ),
                "web_order_line_item_id": matching_transaction.get(
    ...
    async def _verify_apple_sandbox_receipt(
        self, subscription: SubscriptionInfo
    ) -> Tuple[bool, Dict[str, Any]]:
    ...
    async def _parse_google_receipt(
        self, google_response: Dict[str, Any], subscription: SubscriptionInfo
    ) -> Tuple[bool, Dict[str, Any]]:
        ...
        try:
            ...
            expires_at = datetime.fromtimestamp(
                expiry_time_millis / 1000, tz=timezone.utc
            )
    ...
    async def update_subscription_status(
        self, subscription: SubscriptionInfo, verification_result: Dict[str, Any]
    ) -> bool:
        """
        ...
        try:
            async with self.db_pool.acquire() as conn:
                async with conn.transaction():
                    ...
                    if not is_premium and old_status in [
                        SubscriptionStatus.ACTIVE,
                        SubscriptionStatus.GRACE_PERIOD,
                    ]:
                    ...
                    self.logger.info(
                        f"Updated subscription {subscription.subscription_id} for user {subscription.user_id}: {old_status.value} -> {new_status.value}"
                    )
                    return True
    ...
    async def process_subscription_batch(self) -> Dict[str, int]:
        ...
        results = await asyncio.gather(
            *[rate_limited_task(task) for task in tasks], return_exceptions=True
        )
    ...
    async def _process_ios_subscription(self, subscription: SubscriptionInfo) -> bool:
        ...
        for attempt in range(MAX_RETRIES):
            try:
                is_valid, verification_result = await self.verify_app_store_receipt(
                    subscription
                )
                ...
                if is_valid:
                    ...
                    self.logger.warning(
                        f"Invalid iOS receipt for subscription {subscription.subscription_id}"
                    )
                    return False

            except Exception as e:
                self.logger.error(
                    f"iOS subscription processing error (attempt {attempt + 1}): {e}"
                )
    ...
    async def _process_android_subscription(
        self, subscription: SubscriptionInfo
    ) -> bool:
        """Process Android subscription with retry logic"""
        for attempt in range(MAX_RETRIES):
            try:
                is_valid, verification_result = await self.verify_google_play_receipt(
                    subscription
                )
                ...
                if is_valid:
                    ...
                    self.logger.warning(
                        f"Invalid Android receipt for subscription {subscription.subscription_id}"
                    )
                    return False

            except Exception as e:
                self.logger.error(
                    f"Android subscription processing error (attempt {attempt + 1}): {e}"
                )
                if attempt < MAX_RETRIES - 1:
                    await asyncio.sleep(
                        RETRY_DELAY * (2**attempt)
    ...
    def generate_metrics_report(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "runtime_info": {"python_version": sys.version, "platform": sys.platform},
        }
...
async def main():
    ...
    try:
        ...
        try:
            ...
            manager.logger.info(
                f"Processing complete: {json.dumps(metrics_report, indent=2)}"
            )
