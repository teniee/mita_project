    ...
    def upload_to_s3(self, backup_path: Path) -> bool:
        ...
        try:
            ...
            self.s3_client.upload_file(
                str(backup_path), self.backup_bucket, s3_key, ExtraArgs=extra_args
            )

            logger.info(
                f"Upload completed successfully. Size: {file_size / 1024 / 1024:.1f} MB"
            )
            return True
    ...
    def cleanup_old_backups(self, retention_days: int = 30):
        ...
        try:
            ...
            pages = paginator.paginate(
                Bucket=self.backup_bucket, Prefix="database_backups/"
            )
                ...
                for obj in page["Contents"]:
                    if obj["LastModified"].replace(tzinfo=None) < cutoff_date:
                        self.s3_client.delete_object(
                            Bucket=self.backup_bucket, Key=obj["Key"]
                        )
                        deleted_count += 1
