...
class SecretEncryptionValidator:
    ...
    def __init__(self, environment: str):
        ...
        logger.info(
            f"Initialized Secret Encryption Validator for environment: {environment}"
        )
    ...
    def _validate_secrets_manager_encryption(self) -> None:
        ...
        try:
            ...
            for secret in secrets:
                ...
                try:
                    # Get secret details
                    secret_details = self.secrets_client.describe_secret(
                    ...
                    # Check rotation
                    encryption_info["rotation_enabled"] = secret_details.get(
                    ...
                    compliance_tags = [
                        tag["Value"] for tag in tags if tag["Key"] == "Compliance"
                    ]
                    encryption_info["compliance_tags"] = (
                        compliance_tags[0].split(",") if compliance_tags else []
                    )
                    ...
                    logger.error(
                        f"Failed to validate encryption for secret {secret_name}: {e}"
                    )
                    self.validation_results["violations"].append(
                        {
                            "type": "VALIDATION_ERROR",
            ...
            self.validation_results["encryption_at_rest"][
                "secrets_manager"
            ] = encryption_results
            ...
            logger.info(
                f"Secrets Manager validation: {encryption_results['encrypted_secrets']}/{encryption_results['total_secrets']} secrets properly encrypted"
            )

        except Exception as e:
            raise EncryptionValidationError(
                f"Secrets Manager encryption validation failed: {e}"
            )
    ...
    def _validate_kms_configuration(self) -> None:
        ...
        try:
            ...
            try:
                ...
                kms_results["key_rotation_enabled"] = rotation_status[
                    "KeyRotationEnabled"
                ]
                ...
                key_policy = self.kms_client.get_key_policy(
                    KeyId=key_metadata["KeyId"], PolicyName="default"
                )
                ...
                kms_results["key_usage_permissions"] = self._test_kms_permissions(
                    key_metadata["KeyId"]
                )
    ...
    def _validate_tls_configuration(self) -> None:
        ...
        try:
            ...
            tls_results["secrets_manager_tls"] = self._test_aws_tls_connection(
                f"secretsmanager.{self.aws_region}.amazonaws.com", 443
            )
            ...
            tls_results["application_tls"] = self._test_tls_connection(app_host, 443)
            ...
            for trail in trails["trailList"]:
                if trail.get("IsMultiRegionTrail", False):
                    ...
                    for selector in event_selectors.get("EventSelectors", []):
                        if any(
                            "secretsmanager" in resource.get("ResourceType", "")
    ...
    def _determine_compliance_status(self) -> None:
        ...
        critical_violations = [
            v
            for v in self.validation_results["violations"]
            if v["severity"] == "CRITICAL"
        ]
        high_violations = [
            v for v in self.validation_results["violations"] if v["severity"] == "HIGH"
        ]
    ...
    def _generate_recommendations(self) -> None:
        ...
        secrets_results = self.validation_results["encryption_at_rest"].get(
            "secrets_manager", {}
        )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="MITA Finance Secret Encryption Validator"
    )
    parser.add_argument(
        "--environment",
        required=True,
    ...
    try:
        ...
        if args.fail_on_violations and results["violations"]:
            logger.error(
                f"Validation failed with {len(results['violations'])} violations"
            )
            sys.exit(1)
