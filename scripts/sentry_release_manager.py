...
class SentryReleaseManager:
    ...
    def get_git_info(self) -> Dict[str, str]:
        ...
        try:
            ...
            commit_message = subprocess.check_output(
                ["git", "log", "-1", "--pretty=%s"], universal_newlines=True
            ).strip()

            # Get author
            commit_author = subprocess.check_output(
                ["git", "log", "-1", "--pretty=%an"], universal_newlines=True
            ).strip()
    ...
    def create_release(
        self, version: str, projects: Optional[List[str]] = None
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                f"   Commit: {git_info['commit_sha'][:8]} - {git_info['commit_message']}"
            )
            ...
            if e.response and e.response.status_code == 400:
                ...
                if "already exists" in str(error_data):
                    logger.warning(
                        f"⚠️  Release {version} already exists, skipping creation"
                    )
    ...
    def create_deployment(
        ...
        url_path = (
            f"{self.base_url}/organizations/{self.org_slug}/releases/{version}/deploys/"
        )

        try:
            response = requests.post(
                url_path, headers=self.headers, json=deployment_data
            )
            response.raise_for_status()

            deployment_info = response.json()
            logger.info(
                f"✅ Created deployment for release {version} in {env} environment"
            )
    ...
    def finalize_release(self, version: str) -> Dict[str, Any]:
        ...
        finalize_data = {"dateReleased": datetime.now(timezone.utc).isoformat() + "Z"}
    ...
    def set_commits(
        self, version: str, commits: Optional[List[Dict[str, Any]]] = None
    ) -> None:
        ...
        try:
            for commit in commits:
                ...
                if not response.ok:
                    logger.warning(
                        f"Failed to add commit {commit['id']}: {response.text}"
                    )
    ...
    def notify_deployment(
        self, version: str, status: str = "success", webhook_url: Optional[str] = None
    ) -> None:
            ...
            "attachments": [
                {
                    ...
                    "fields": [
                        ...
                        {"title": "Branch", "value": git_info["branch"], "short": True},
                        {
                            "title": "Timestamp",
    ...
    def full_release_cycle(
        ...
        try:
            ...
            deployment_info = self.create_deployment(
                version, environment=self.environment, url=deployment_url
            )
...
def main():
    ...
    parser.add_argument(
        "--token", help="Sentry auth token (or use SENTRY_AUTH_TOKEN env var)"
    )
    parser.add_argument(
        "--version", help="Release version (auto-generated if not provided)"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--no-notify", action="store_true", help="Skip deployment notifications"
    )
    parser.add_argument(
        "--projects", nargs="+", help="Additional projects to include in release"
    )
    ...
    if not auth_token:
        logger.error(
            "❌ Sentry auth token required. Use --token or set SENTRY_AUTH_TOKEN environment variable"
        )
        sys.exit(1)
