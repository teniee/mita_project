...
class SentryAlertsManager:
    ...
    def setup_all_alerts(self, environment: str = "production"):
        ...
        logger.info(
            f"Alert setup complete: {success_count}/{total_count} rules created successfully"
        )
    ...
    def _create_alert_rule(
        self, rule_name: str, rule_config: Dict[str, Any], environment: str
    ) -> bool:
        ...
        alert_rule_data = {
            ...
            "query": self._build_query_from_conditions(
                rule_config.get("conditions", [])
            ),
        ...
        try:
            ...
            if response.status_code == 201:
                ...
                if "already exists" in str(error_data).lower():
                    logger.warning(
                        f"Alert rule '{rule_name}' already exists, updating..."
                    )
    ...
    def _create_slack_action(self, alert_rule_id: str, action: Dict[str, Any]):
        ...
        action_data = {
            ...
            "params": {"channel": target_identifier, "tags": tags},
        }
    ...
    def _update_existing_alert_rule(
        self, rule_name: str, alert_rule_data: Dict[str, Any]
    ) -> bool:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.error(
                    f"Failed to update alert rule: {response.status_code} - {response.text}"
                )
                return False
    ...
    def _setup_slack_integration(self, slack_config: Dict[str, Any]):
        ...
        for channel_name, channel_config in channels.items():
            ...
            if webhook_url and webhook_url != "${SLACK_WEBHOOK_URL}":
                logger.info(
                    f"Slack webhook configured for {channel_name}: {webhook_url[:50]}..."
                )
            else:
    ...
    def _setup_pagerduty_integration(self, pagerduty_config: Dict[str, Any]):
        ...
        for service_name, service_config in services.items():
            ...
            if integration_key and not integration_key.startswith("${"):
                ...
                logger.warning(
                    f"PagerDuty integration key not configured for {service_name}"
                )
...
def main():
    ...
    parser.add_argument(
        "--config",
        default="config/sentry/alert_rules.yaml",
        help="Configuration file path",
    )
    parser.add_argument(
        "--environment", default="production", help="Environment to set up alerts for"
    )
    parser.add_argument(
        "--validate-only", action="store_true", help="Only validate configuration"
    )
    parser.add_argument(
        "--list-rules", action="store_true", help="List existing alert rules"
    )
    ...
    try:
        if args.validate_only:
            ...
            for rule in rules:
                logger.info(
                    f"  - {rule['name']} (ID: {rule['id']}) - {rule.get('environment', 'N/A')}"
                )
