...
class ProductionAPIConfigurator:
    ...
    def _check_environment_setup(self):
        ...
        required_env_vars = ["ENVIRONMENT", "SECRET_KEY", "JWT_SECRET", "DATABASE_URL"]
    ...
    async def _validate_all_apis(self):
        ...
        logger.info(
            f"API key validation completed. Valid: {len(self.validation_results) - len(self.invalid_keys) - len(self.missing_keys)}, Invalid: {len(self.invalid_keys)}, Missing: {len(self.missing_keys)}"
        )
    ...
    async def _test_openai_connection(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"status": "error", "service": "OpenAI GPT", "error": str(e)}
    ...
    async def _test_sentry_connection(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # Initialize Sentry
            sentry_sdk.init(
                dsn=dsn, environment="production_test", traces_sample_rate=0.0
            )
            ...
            with sentry_sdk.push_scope() as scope:
                ...
                event_id = sentry_sdk.capture_message(
                    "API configuration test", level="info"
                )
            ...
            return {"status": "error", "service": "Sentry", "error": str(e)}
    ...
    async def _test_redis_connection(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"status": "error", "service": "Upstash Redis", "error": str(e)}
    ...
    async def _test_sendgrid_connection(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"status": "error", "service": "SendGrid", "error": str(e)}
    ...
    async def _test_aws_s3_connection(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"status": "error", "service": "AWS S3", "error": str(e)}
    ...
    def _generate_configuration_report(self) -> Dict[str, Any]:
        ...
        report = {
            ...
            "summary": {
                ...
                "services_connected": len(
                    [r for r in connection_summary.values() if r == "connected"]
                ),
            },
    ...
    def _generate_recommendations(self) -> List[str]:
        ...
        if self.missing_keys:
            recommendations.append(
                f"Configure missing API keys: {', '.join(self.missing_keys)}"
            )

        if self.invalid_keys:
            recommendations.append(
                f"Fix invalid API keys: {', '.join(self.invalid_keys)}"
            )
        ...
        missing_critical = [
            ...
            if key in self.missing_keys or key in self.invalid_keys
        ]

        if missing_critical:
            recommendations.append(
                f"CRITICAL: Fix these essential services immediately: {', '.join(missing_critical)}"
            )
        ...
        if hasattr(self, "connection_tests"):
            for service, result in self.connection_tests.items():
                if result["status"] != "connected":
                    recommendations.append(
                        f"Fix {service} connection: {result.get('error', 'Unknown error')}"
                    )
...
async def main():
    ...
    parser = argparse.ArgumentParser(
        description="Configure MITA Finance Production APIs"
    )
    parser.add_argument(
        "--validate-only",
        action="store_true",
        help="Only validate APIs, don't test connections",
    )
    parser.add_argument(
        "--output-format",
