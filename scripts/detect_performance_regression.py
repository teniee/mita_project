...
class PerformanceRegressionDetector:
                    ...
                    try:
                        with open(summary_file, "r") as f:
                            ...
                            current_metrics.update(
                                self._extract_metrics_from_summary(data)
                            )
                    except Exception as e:
                        logger.warning(
                            f"Could not extract metrics from {summary_file}: {e}"
                        )
                ...
                with open(csv_file, "r") as f:
                    ...
                    if total_requests > 0:
                        ...
                        metrics["requests_per_second"] = (
                            total_requests / 60
                        )  # Assume 1 minute test

            except Exception as e:
                logger.warning(
                    f"Could not extract load test metrics from {csv_file}: {e}"
                )
    ...
    def _extract_benchmark_metrics(self, report_dir: Path, metrics: Dict[str, float]):
        ...
        for benchmark_file in benchmark_files:
            try:
                ...
                logger.warning(
                    f"Could not extract benchmark metrics from {benchmark_file}: {e}"
                )
        ...
        for metric_key in self.key_metrics:
            if metric_key in baseline_perf and metric_key in current_perf:
                comparison = self._compare_metric(
                    metric_key, baseline_perf[metric_key], current_perf[metric_key]
                )
                ...
                if comparison["regression_level"] != "none":
                    ...
                    < -self.thresholds["improvement_threshold_percent"]
                ):
                    improvements.append(comparison)

        # Categorize regressions by severity
        critical_regressions = [
            r for r in regressions if r["regression_level"] == "critical"
        ]
        significant_regressions = [
            r for r in regressions if r["regression_level"] == "significant"
        ]
        ...
        self.regression_report.update(
            {
                ...
                "summary": {
                    ...
                    "metrics_stable": len(comparisons)
                    - len(regressions)
                    - len(improvements),
                },
            }
    ...
    def _flatten_metrics(self, metrics_data: Dict[str, Any]) -> Dict[str, float]:
        ...
        if "performance_metrics" in metrics_data:
            ...
            security = perf_metrics.get("security", {})
            if security.get("rate_limiter_overhead_ms"):
                flattened["rate_limiter_overhead_ms"] = security[
                    "rate_limiter_overhead_ms"
                ]
    ...
    def _compare_metric(
        self, metric_key: str, baseline_value: float, current_value: float
    ) -> Dict[str, Any]:
        ...
        change_percent = (
            (change_absolute / baseline_value) * 100 if baseline_value != 0 else 0
        )
    ...
    def _print_regression_summary(self, report: Dict[str, Any]):
        ...
        print(
            f"   Significant Regressions: {len(report.get('significant_regressions', []))}"
        )
        ...
        print(
            f"   Production Ready: {'✅ YES' if impact.get('production_readiness', False) else '❌ NO'}"
        )
        print(
            f"   User Experience: {impact.get('user_experience_impact', 'unknown').upper()}"
        )
...
def main():
    parser = argparse.ArgumentParser(
        description="MITA Performance Regression Detection"
    )
    parser.add_argument(
        "--reports-dir",
        required=True,
    ...
    parser.add_argument(
        "--baseline-branch", default="main", help="Baseline branch for comparison"
    )
    parser.add_argument(
        "--current-branch", default="current", help="Current branch being analyzed"
    )
    parser.add_argument(
        "--output",
    ...
    detector = PerformanceRegressionDetector(
        args.reports_dir, args.baseline_branch, args.current_branch
    )
