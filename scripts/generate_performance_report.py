...
class PerformanceReportGenerator:
        ...
        if "benchmarks" in data:
            ...
            if parsed["benchmarks"]:
                ...
                parsed["summary"] = {
                    ...
                    "passed_tests": len(
                        parsed["benchmarks"]
                    ),  # All benchmarks that ran are considered passed
                    "failed_tests": 0,
    ...
    def collect_load_test_results(self) -> Dict[str, Any]:
        ...
        for subdir in self.input_dir.iterdir():
            if subdir.is_dir() and "load-test-reports" in subdir.name:
                ...
                if stats_files:
                    try:
                        load_test_results[scenario_name] = self._parse_load_test_csv(
                            stats_files[0]
                        )
                    except Exception as e:
                        print(
                            f"Warning: Could not parse load test results for {scenario_name}: {e}"
                        )
        ...
        try:
            with open(csv_file, "r") as f:
                ...
                results["summary"] = {
                    ...
                    "average_response_time": weighted_response_time
        ...
        for category, data in benchmark_results.items():
            if "benchmarks" in data:
                for benchmark in data["benchmarks"]:
                    ...
                    target_key = self._match_benchmark_to_target(
                        benchmark_name, targets
                    )
                    ...
                    if target_key:
                        ...
                        result = {
                            ...
                            "within_acceptable": mean_time
                            <= target["max_acceptable_ms"],
    ...
    def _match_benchmark_to_target(
        self, benchmark_name: str, targets: Dict[str, Any]
    ) -> Optional[str]:
        ...
        if "classification" in name_lower and "income" in name_lower:
            ...
            return "auth_login"
        elif "token" in name_lower and (
            "validation" in name_lower or "verify" in name_lower
        ):
    ...
    def generate_recommendations(
        ...
        income_results = performance_analysis.get("results", {}).get(
            "income_classification"
        )
        ...
        self.report_data.update(
            {
                "summary": {
                    "targets_met": performance_analysis.get("targets_met", 0),
                    "targets_failed": performance_analysis.get("targets_failed", 0),
                    "critical_failures": performance_analysis.get(
                        "critical_failures", 0
                    ),
                ...
                "performance_metrics": {
                    "income_classification": self._extract_metric(
                        performance_analysis, "income_classification"
                    ),
