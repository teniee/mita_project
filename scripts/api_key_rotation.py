...
from app.core.api_key_manager import ServiceType, api_key_manager
...
class APIKeyRotationManager:
    ...
    async def analyze_rotation_needs(self) -> Dict[str, Any]:
        ...
        analysis = {
            ...
            "summary": {
                ...
                "total_needing_attention": len(rotation_overdue)
                + len(rotation_needed)
                + len(rotation_recommended),
            },
            "next_actions": self._generate_rotation_plan(
                rotation_overdue + rotation_needed + rotation_recommended
            ),
        }
    ...
    async def rotate_key(
        self, key_name: str, new_key_value: str = None
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if new_key_value:
                ...
                if service_type:
                    valid, error = await api_key_manager._validate_key(
                        service_type, new_key_value, key_name
                    )
                    if valid:
                        rotation_result["steps"][-1]["status"] = "completed"
                    else:
            ...
            if self.dry_run:
                ...
                rotation_result["steps"][-1][
                    "message"
                ] = "Would update environment variable in production"
            else:
                ...
                rotation_result["steps"][-1][
                    "message"
                ] = "Environment variable update simulated"
            ...
            if self.dry_run:
                ...
                rotation_result["steps"][-1][
                    "message"
                ] = "Would test service connectivity"
            else:
                # Validate connectivity with new key
                connectivity_ok = await self._test_service_connectivity(
                    key_name, new_key_value
                )
                if connectivity_ok:
                    rotation_result["steps"][-1]["status"] = "completed"
                else:
            ...
            if self.dry_run:
                ...
                success = await api_key_manager.rotate_key(
                    key_name, new_key_value or "simulated_new_key"
                )
                if success:
                    rotation_result["steps"][-1]["status"] = "completed"
                    rotation_result["success"] = True
    ...
    async def batch_rotate_keys(
        self, keys_to_rotate: List[str], new_keys: Dict[str, str] = None
    ) -> Dict[str, Any]:
        ...
        batch_result = {
            ...
            "dry_run": self.dry_run,
            "results": [],
            "summary": {"successful": 0, "failed": 0, "simulated": 0},
        }
    ...
    def generate_rotation_report(self, analysis: Dict[str, Any]) -> str:
        ...
        report.append(
            f"  📊 Total Needing Attention: {summary['total_needing_attention']} keys"
        )
        report.append("")
        ...
        if analysis["rotation_overdue"]:
            ...
            for key_info in analysis["rotation_overdue"]:
                ...
                report.append(
                    f"    Days since rotation: {key_info['days_since_rotation']}"
                )
                report.append(
                    f"    Recommended interval: {key_info['recommended_interval']} days"
                )
            report.append("")
        ...
        if analysis["next_actions"]:
            ...
            for i, action in enumerate(analysis["next_actions"][:5], 1):
                report.append(
                    f"{i}. {action['key_name']} ({action['service']}) - Priority: {action['priority']}"
                )
                ...
                if action["zero_downtime_capable"]:
                    ...
                    report.append(
                        f"   ⏱️  Estimated downtime: {action['estimated_downtime_minutes']} minutes"
                    )
                report.append("")
    ...
    def save_rotation_report(
        self, analysis: Dict[str, Any], batch_results: Dict[str, Any] = None
    ):
...
async def main():
    ...
    parser = argparse.ArgumentParser(
        description="MITA Finance API Key Rotation Manager"
    )
    parser.add_argument(
        "--analyze", action="store_true", help="Analyze rotation needs only"
    )
    parser.add_argument("--rotate", nargs="*", help="Rotate specific keys")
    parser.add_argument(
        "--rotate-all-needed",
        action="store_true",
        help="Rotate all keys that need rotation",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--execute",
        action="store_true",
        help="Execute actual rotation (disables dry-run)",
    )
    parser.add_argument(
    ...
    if args.execute and args.dry_run:
        logger.warning(
            "Both --execute and --dry-run specified. Using --execute (real rotation)"
        )
        dry_run = False
