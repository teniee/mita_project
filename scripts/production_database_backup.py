...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("/var/log/mita-backup.log"), logging.StreamHandler()],
)
...
class ProductionBackupSystem:
    ...
    def _create_database_dump(self, backup_id: str, backup_type: str) -> Dict[str, str]:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            # Compress the dump
            with open(sql_path, "rb") as f_in, gzip.open(
            ...
            logger.info(
                f"Database dump created: {sql_size} bytes -> {gz_size} bytes "
        ...
        try:
            ...
            with tempfile.TemporaryDirectory() as tmpdir:
                ...
                if downloaded_checksum != metadata["gz_checksum"]:
                    logger.error(
                        f"Checksum mismatch for backup {metadata['backup_id']}"
                    )
                    return False
                ...
                if test_db_url:
                    success = self._test_restore_to_temp_db(
                        local_path, test_db_url, metadata
                    )
                    if not success:
                        return False
    ...
    def _test_restore_to_temp_db(
        self, backup_path: str, test_db_url: str, metadata: Dict[str, str]
    ) -> bool:
        ...
        try:
            ...
            with tempfile.TemporaryDirectory() as tmpdir:
                ...
                with gzip.open(backup_path, "rb") as f_in, open(
                    sql_path, "wb"
                ) as f_out:
                    shutil.copyfileobj(f_in, f_out)
    ...
    def list_backups(self) -> List[Dict[str, str]]:
        ...
        try:
            ...
            for obj in response["Contents"]:
                try:
                    metadata_content = self.s3.get_object(
                    ...
                    logger.warning(
                        f"Failed to load metadata for {obj['Key']}: {str(e)}"
                    )
    ...
    def restore_backup(self, backup_id: str, target_db_url: str = None) -> bool:
        ...
        try:
            ...
            with tempfile.TemporaryDirectory() as tmpdir:
                ...
                if downloaded_checksum != metadata["gz_checksum"]:
                    raise RuntimeError(
                        f"Backup checksum verification failed for {backup_id}"
                    )
    ...
    def verify_migration_readiness(self) -> bool:
        ...
        try:
            ...
            with conn.cursor() as cur:
                ...
                if long_running > 0:
                    logger.warning(
                        f"Long-running transactions detected: {long_running}"
                    )
                    return False
...
def main():
    parser = argparse.ArgumentParser(
        description="MITA Production Database Backup System"
    )
    ...
    restore_parser.add_argument(
        "--backup-id", required=True, help="Backup ID to restore"
    )
    ...
    validate_parser.add_argument(
        "--backup-id", required=True, help="Backup ID to validate"
    )
        ...
        if args.command == "backup":
            ...
            if backups:
                ...
                for backup in backups:
                    ...
                    print(
                        f"  {backup['backup_id']} - {backup['timestamp']} "
