...
class MigrationPerformanceTester:
                    ...
                    lock_data.append(
                        {
                            ...
                            "blocked_locks": len(
                                [lock for lock in locks if not lock["granted"]]
                            ),
                        }
                    )
    ...
    def test_migration_performance(self, target: str = "head") -> Dict[str, any]:
        ...
        performance_results = {
            ...
            "resource_usage": {
                ...
                "max_cpu_percent": max(
                    (r["cpu_percent"] for r in resource_data), default=0
                ),
                "max_memory_percent": max(
                    (r["memory_percent"] for r in resource_data), default=0
                ),
                "avg_cpu_percent": sum(r["cpu_percent"] for r in resource_data)
                / max(len(resource_data), 1),
                "peak_memory_gb": max(
                    (r["memory_used_gb"] for r in resource_data), default=0
                ),
            },
            "lock_analysis": {
                "samples": len(lock_data),
                "max_concurrent_locks": max(
                    (entry["total_locks"] for entry in lock_data), default=0
                ),
                "max_blocked_locks": max(
                    (entry["blocked_locks"] for entry in lock_data), default=0
                ),
        ...
        logger.info(
            f"  Max CPU: {performance_results['resource_usage']['max_cpu_percent']:.1f}%"
        )
        logger.info(
            f"  Max Memory: {performance_results['resource_usage']['max_memory_percent']:.1f}%"
        )
                ...
                while load_active:
                    try:
                        with conn.cursor() as cur:
                            ...
                            cur.execute(
                                "SELECT SUM(amount) FROM transactions LIMIT 100"
                            )
                            cur.execute(
    ...
    def generate_performance_report(self, results: Dict[str, any], output_file: str):
        ...
        report = {
            "test_summary": {
                ...
                "test_database": (
                    self.test_db_url.split("@")[1]
                    if "@" in self.test_db_url
                    else "masked"
                ),
                "total_tests": len(
                    [k for k in results.keys() if k.endswith("_results")]
                ),
            },
    ...
    def _generate_recommendations(self, results: Dict[str, any]) -> List[str]:
        ...
        if "migration_results" in results:
            max_cpu = results["migration_results"]["resource_usage"].get(
                "max_cpu_percent", 0
            )
            max_memory = results["migration_results"]["resource_usage"].get(
        ...
        if "migration_results" in results:
            ...
            max_blocked = results["migration_results"]["lock_analysis"].get(
                "max_blocked_locks", 0
            )
        ...
        if not recommendations:
            recommendations.append(
                "Migration performance appears to be within acceptable limits."
            )
...
def main():
    ...
    parser.add_argument(
        "--with-load", action="store_true", help="Run concurrent load during migration"
    )
    parser.add_argument(
        "--rollback-test", action="store_true", help="Test rollback performance"
    )
    parser.add_argument("--target", default="head", help="Migration target")
    parser.add_argument(
        "--rollback-target", default="0003_goals", help="Rollback target"
    )
    parser.add_argument(
        "--output",
    ...
    try:
        ...
        if "migration_results" in results:
            ...
            print(
                f"Peak Memory Usage: {mr['resource_usage']['max_memory_percent']:.1f}%"
            )
            print(
                f"Max Concurrent Locks: {mr['lock_analysis']['max_concurrent_locks']}"
            )
