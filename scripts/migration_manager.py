...
class ProductionMigrationManager:
    ...
    def __init__(self):
        ...
        self.max_migration_time = int(
            os.environ.get("MAX_MIGRATION_TIME", "1800")
        )  # 30 minutes
        self.lock_timeout = int(
            os.environ.get("MIGRATION_LOCK_TIMEOUT", "3600")
        )  # 1 hour
            ...
            with conn.cursor() as cur:
                ...
                try:
                    ...
                    if existing_lock:
                        ...
                        if expires_at_existing > datetime.datetime.now(
                            datetime.timezone.utc
                        ).replace(tzinfo=expires_at_existing.tzinfo):
                            ...
                            cur.execute(
                                "DELETE FROM migration_locks WHERE lock_name = %s",
                                (lock_name,),
                            )
                            cur.execute(
                                """
                            ...
                            logger.info(
                                f"Forcibly acquired {operation} lock after expiration"
                            )
                ...
                try:
                    ...
                    # Release lock
                    cur.execute(
                        "DELETE FROM migration_locks WHERE lock_name = %s", (lock_name,)
                    )
    ...
    def _validate_pre_migration_state(self, stats: Dict[str, any]) -> bool:
        ...
        if stats.get("user_count", 0) == 0 and stats.get("transaction_count", 0) == 0:
            ...
            logger.info(
                f"Database contains {stats['user_count']} users and "
        ...
        try:
            with self.acquire_migration_lock("migration"):
                ...
                if not self._validate_post_migration_state(pre_stats, post_stats):
                    logger.error(
                        "Post-migration validation failed - initiating rollback"
                    )
                ...
                logger.info(
                    f"Migration completed successfully in {migration_duration:.2f}s"
                )
                logger.info(
                    f"Database migrated from {current_revision} to {new_revision}"
                )
    ...
    def _create_pre_migration_backup(self) -> Dict[str, str]:
        ...
        backup_script = os.path.join(
            os.path.dirname(__file__), "production_database_backup.py"
        )
        ...
        backup_id = "pre-migration-" + datetime.datetime.now(
            datetime.timezone.utc
    ...
    def _run_alembic_migration(self, target: str) -> bool:
        ...
        env["MIGRATION_TIMESTAMP"] = datetime.datetime.now(
            datetime.timezone.utc
        ).isoformat()
    ...
    def _validate_post_migration_state(
        self, pre_stats: Dict[str, any], post_stats: Dict[str, any]
    ) -> bool:
        ...
        for count_key in critical_counts:
            ...
            if post_count < pre_count:
                logger.error(
                    f"Data loss detected in {count_key}: {pre_count} -> {post_count}"
                )
                return False
...
def main():
    ...
    rollback_parser = subparsers.add_parser(
        "rollback", help="Rollback database migration"
    )
    rollback_parser.add_argument(
        "--target", required=True, help="Target migration to rollback to"
    )
    ...
    try:
        if args.command == "migrate":
            ...
            if args.release:
                ...
                print(
                    f"Use 'with acquire_migration_lock(\"{args.operation}\")' in your code"
                )
