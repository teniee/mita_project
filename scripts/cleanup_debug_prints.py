...
class DebugPrintCleanup:
    ...
    def categorize_print_statements(
        self, statements: List[Tuple[str, int, str]]
    ) -> Dict[str, List]:
        ...
        for file_path, line_num, line in statements:
            ...
            if any(
                ...
                pattern in line_lower
                for pattern in ["starting", "startup", "initializ", "🚀", "✅"]
            ):
                categories["startup"].append((file_path, line_num, line))
            elif any(
                pattern in line_lower for pattern in ["debug", "🔍", "step", "checking"]
            ):
                categories["debug"].append((file_path, line_num, line))
            elif any(
                pattern in line_lower
                for pattern in ["test", "mock", "temp", "todo", "fixme"]
            ):
    ...
    def replace_print_with_logging(
        self, file_path: str, line_num: int, line: str, log_level: str
    ) -> bool:
        ...
        try:
            ...
            if not print_match:
                logger.warning(
                    f"Could not parse print statement at {file_path}:{line_num}"
                )
                return False
            ...
            if "logger" not in "".join(lines[:line_num]):
                ...
                logger_import = (
                    f"{indentation}from app.core.logging_config import get_logger\n"
                )
            ...
            self.changes_made.append(
                f"{file_path}:{line_num} - Replaced print with logger.{log_level}"
            )
            return True
    ...
    def cleanup_backend_prints(self):
        ...
        for file_path, line_num, line in categories["remove"]:
            ...
            if any(
                pattern in line.lower() for pattern in ["temp", "test", "debug", "todo"]
            ):
