...
def create_jwt_secret(secret_arn: str) -> None:
    ...
    try:
        ...
        new_secret = {
            ...
            "previous_key": current_secret.get(
                "current_key"
            ),  # Keep previous key for validation
            "algorithm": JWT_ALGORITHM,
            "created_at": datetime.now(timezone.utc).isoformat(),
            "expires_at": (
                datetime.now(timezone.utc)
                + timedelta(minutes=TRANSITION_PERIOD_MINUTES)
            ).isoformat(),
...
def set_jwt_secret(secret_arn: str) -> None:
    ...
    try:
        ...
        token = jwt.encode(
            test_payload, pending_secret["current_key"], algorithm=JWT_ALGORITHM
        )

        # Test decoding with new key
        decoded = jwt.decode(
            token, pending_secret["current_key"], algorithms=[JWT_ALGORITHM]
        )
...
def test_with_application_endpoint(endpoint: str, token: str) -> None:
    ...
    try:
        ...
        if response.status_code not in [200, 401]:  # 401 is expected for test token
            raise JWTRotationError(
                f"Application endpoint test failed: {response.status_code}"
            )
...
def schedule_jwt_key_cleanup(secret_arn: str, old_version_id: str) -> None:
    ...
    cleanup_time = datetime.now(timezone.utc) + timedelta(
        minutes=TRANSITION_PERIOD_MINUTES
    )

    logger.info(
        f"Scheduled JWT key cleanup for version {old_version_id} at {cleanup_time}"
    )
...
def send_jwt_rotation_alert(secret_arn: str, step: str, error_message: str) -> None:
    ...
    """
    logger.error(
        f"JWT ROTATION ALERT: {secret_arn} failed at step {step}: {error_message}"
    )
