...
def lambda_handler(event: Dict[str, Any], context) -> Dict[str, Any]:
    ...
    try:
        ...
        if not secret_arn or not step:
            raise DatabaseRotationError(
                "Missing required parameters: SecretId and Step"
            )
        ...
        try:
            with admin_connection.cursor() as cursor:
                ...
                if cursor.fetchone():
                    ...
                    cursor.execute(
                        f"ALTER USER {username} WITH PASSWORD %s", (password,)
                    )
                    ...
                    grant_user_permissions(
                        cursor, username, pending_secret.get("permissions", [])
                    )
...
def connect_to_database(
    secret_data: Dict[str, Any], use_admin: bool = False
...
def schedule_credential_cleanup(secret_arn: str, old_version_id: str) -> None:
    ...
    logger.info(
        f"Scheduled cleanup for old credentials: {old_version_id} in {GRACE_PERIOD_HOURS} hours"
    )
